...
public:
		...
		{
			{
				...
				{					
					if (bCanAutoSubmitDone)
					{
						...
						UE_LOG(LogSony, Log, TEXT("GGnmManager.SubmitDone by SubmitDoneTaskRunnable (%f s since last submit) in RT frame %u, RHI frame %u"), TimeSinceLastSubmission, GGnmManager.GetFrameCount());
#endif
					...
					{
#if PS4_GNM_SLOW_FRAME_DEBUGGING
						UE_LOG(LogSony, Warning, TEXT("Cannot call GGnmManager.SubmitDone because bCanAutoSubmitDone is false (%f s since last submit), RT frame %u, RHI frame %u"), TimeSinceLastSubmission, GGnmManager.GetFrameCountRT(), GGnmManager.GetFrameCount());
#endif
						}
					}
				}
	...
	void UpdateSubmitDoneTime()
	{
		...
		{
			UE_LOG(LogSony, Warning, TEXT("Gap between SubmitDone calls of %f s"), DeltaTime);
		}
	}
...
public:
	...
	void Start()
	{
		...
		SubmitCommandsAsyncTaskThread = FRunnableThread::Create(this, TEXT("SubmitGPUCommandsAsyncTaskThreadPS4"), 0, TPri_AboveNormal, ThreadAffinity);
		...
		if (bWillSubmitGfx)
		{
			for (int32 i = 0; i < ActiveSubmissionBuffers.Num(); ++i)
			{
				...
				if (Ret != SCE_OK)
				{
					...
					{
						UE_LOG(LogSony, Warning, TEXT("Submission: %i of group: %i, Addr: %p, Size: %i\n"), j, i, Submission.SubmissionAddrs[j], Submission.SubmissionSizesBytes[j]);						
					}
					UE_LOG(LogSony, Fatal, TEXT("async submitCommandBuffers failed: 0x%x, %i, %p"), Ret, Submission.SubmissionAddrs.Num(), &Submission);
				}				
			}
			...
			for (int32 i = 0; i < ActiveComputeSubmissionBuffers.Num(); ++i)
			{
				...
				do
				{
					...
					if (Status != Gnmx::ComputeQueue::kSubmitOK)
					{
						...
						{
							UE_LOG(LogSony, Warning, TEXT("AsyncCompute Submission failing because queue is full. GPU hang?"));
						}
				...
				if (Status != Gnmx::ComputeQueue::kSubmitOK)
				{
					...
					{
						UE_LOG(LogSony, Warning, TEXT("ComputeSubmission: %i of group: %j, Addr: %p, Size: %i\n"), j, i, Submission.SubmissionAddrs[j], Submission.SubmissionSizesBytes[j]);
					}
					UE_LOG(LogSony, Fatal, TEXT("async compute submit failed: 0x%x, %i, %p"), Status, Submission.SubmissionCount, &Submission);
				}
			}
...
public:
	...
	void WaitForGPUFrameCompletion()
	{
		...
		while (GGnmManager.GetEndOfFrameLabel(ActiveReprojectionData.RHIContextIndex) < ActiveReprojectionData.FrameCounter)
		{
			if (GPS4EventBasedFrameEnd != 0)
			{
				...
				{
					UE_LOG(LogSony, Warning, TEXT("EOF Event Wait Timed Out"));
				}
			}
			else
			{
...
FGnmManager::FGnmManager() 
	...
	, VideoOutHandle(-1)
	, VideoOutBufferRegistrationHandle(-1)
	, VideoOutQueue(nullptr)
...
bool FGnmManager::Initialize()
{
	...
	Ret = sceVideoOutAddVblankEvent(VBlankEventQueue, VideoOutHandle, this);
	...
	// Determine if HDR output is supported.
	SceVideoOutDeviceCapabilityInfo VideoCaps;
	GRHISupportsHDROutput =
		(sceVideoOutGetDeviceCapabilityInfo(VideoOutHandle, &VideoCaps) == SCE_OK) &&
		((VideoCaps.capability & SCE_VIDEO_OUT_DEVICE_CAPABILITY_BT2020_PQ) != 0);

	// This format is used as a proxy for the display output, needs to be fixed
	...
	*BlockUntilIdleLabel = 0xFFFFFFFF;	
	...
	static FRHIUniformBufferLayout DummyLayout(TEXT("DummyLayout"));
...
void FGnmManager::CreateBackBuffers(uint32 SizeX, uint32 SizeY)
{
	...
	// We have to pause flip tracking whilst we're reallocating buffers, since we do manual flips to blank.
	RHIShutdownFlipTracking();

	// We must flip to blank and wait for all flips to clear before we can unregister an old backbuffers.
	int32 Ret = sceVideoOutSubmitFlip(VideoOutHandle, SCE_VIDEO_OUT_BUFFER_INDEX_BLANK, SCE_VIDEO_OUT_FLIP_MODE_VSYNC, 0);
	...
	// Completely flush and free the existing back buffers. This allows us to reuse their memory when we reallocate.
	if (VideoOutBufferRegistrationHandle >= 0)
	{
		...
		// This must be done by the rendering thread.
		ENQUEUE_RENDER_COMMAND(Flush)([this](FRHICommandList&)
		{
			for (int32 BufferIndex = 0; BufferIndex < NUM_RENDER_BUFFERS; ++BufferIndex)
			{
				RenderBuffers[BufferIndex] = nullptr;
#if HAS_MORPHEUS
				ReprojectionBuffers[BufferIndex] = nullptr;
#endif
			}
		...
		// Flush again to ensure the above command is complete.
		FlushRenderingCommands();
	}

	// HDR formats
	const bool bEnableHDROutput = IsHDRAllowed() && IsHDREnabled();
	...
	for (int32 BufferIndex = 0; BufferIndex < NUM_RENDER_BUFFERS; BufferIndex++)
	{
		...
		RenderBuffers[BufferIndex] = (FGnmTexture2D*)(FRHITexture2D*)RHICreateTexture2D(SizeX, SizeY, BackBufferFormat, 1, 1, TexCreate_RenderTargetable | TexCreate_Presentable, CreateInfo);

#if HAS_MORPHEUS
		ReprojectionBuffers[BufferIndex] = (FGnmTexture2D*)(FRHITexture2D*)RHICreateTexture2D(SizeX, SizeY, BackBufferFormat, 1, 1, TexCreate_RenderTargetable | TexCreate_Presentable, CreateInfo);
#endif
	...
	// Restart flip tracking now we've got a valid swap chain again.
...
{
#if HAS_MORPHEUS
	UE_LOG(LogSony, Log, TEXT("FGnmManager::CreateBackBuffers creating social screen buffers."));
	...
	{
		FGnmAuxBuffer& Aux = AuxBuffers[BufferIndex];

		FRHIResourceCreateInfo CreateInfo;
		Aux.RenderBuffer = (FGnmTexture2D*)(FRHITexture2D*)RHICreateTexture2D(AuxSizeX, AuxSizeY, PF_B8G8R8A8, 1, 1, TexCreate_RenderTargetable, CreateInfo);
		...
		Aux.VideoOutBuffer = (FGnmTexture2D*)(FRHITexture2D*)RHICreateTexture2D(AuxSizeX, AuxVideoOutHeight, PF_L8, 1, 1, TexCreate_RenderTargetable | TexCreate_NoTiling | TexCreate_Presentable | TexCreate_UAV, CreateInfo);
		VoidAuxVideoOutBuffers[BufferIndex] = GetGnmSurfaceFromRHITexture(Aux.VideoOutBuffer).BufferMem.GetPointer();
		Aux.UAV = RHICreateUnorderedAccessView(Aux.VideoOutBuffer);
	}

	FGnmSurface* Surface = &GetGnmSurfaceFromRHITexture(AuxBuffers[0].VideoOutBuffer);
...
void FGnmManager::UpdateFlipRate(bool bForce)
{
	...
	if (bForce || LastFlipRate != TargetFlipRate)
	{
		...
		{
			UE_LOG(LogSony, Fatal, TEXT("sceVideoOutSetFlipRate return error code 0x%08x"), RetVal);
		}
...
void FGnmManager::ChangeVideoOutputMode(EPS4OutputMode InMode)
{
...
#endif
		{
			if (InMode != EPS4OutputMode::Standard2D)
			{
				UE_LOG(LogSony, Error, TEXT("ChangeVideoOutputMode: Attempted to change Video Output Mode to %i but UE4 was not built with HAS_MORPHEUS so only Standard2D(%i) is supported.  Rebuild UE4 (not just your project)."), (int)InMode, (int)EPS4OutputMode::Standard2D);
			}
			checkf(false, TEXT("sceVideoOutConfigureOutputMode failed: 0x%08X\n"), Ret);
		}
	}
	...
	{
		UE_LOG(LogSony, Log, TEXT("ChangeVideoOutputMode: changed video output mode to %i refreshRate to 0x%x."), (int)InMode, VOut.refreshRate);
	}
...

		{
			...
			if (bDo2DVRReprojection)
			{
				if (bReprojectionData2DVRDirty)
				{
					...
					ReprojectionData2DVR.FrameCounter = FrameCounter - 1;  // it has already been incremented in FGnmManager::SubmitFrame
			...
			else
			{
				...
				ReprojectionData.FrameCounter = FrameCounter - 1;  // it has already been incremented in FGnmManager::SubmitFrame
...
{
#if HAS_MORPHEUS
	LLM_SCOPE_SONY(ELLMTagSony::GnmMisc);
	...
	parameter.label = (uint64_t*)ReprojectionLabels[DistortionData.RHIContextIndex];
	if (*parameter.label != 0)
	{
		...
		UE_LOG(LogSony, Warning, TEXT("ApplyMorpheusReprojection parameter.label != 0.  Ok if you just switched to/from previous context mode."));
	}
	*parameter.label = 1;  // set to 0 when reprojection is done
	parameter.sampler = &ReprojectionSampler;
	parameter.timing = SCE_HMD_REPROJECTION_REPROJECTION_TIMING_3000USEC;
...
void FGnmManager::StartMorpheus2DVRReprojection(GnmBridge::Morpheus2DVRReprojectionData& ReprojectionData)
{
	...
	if (bDo2DVRReprojection)
	{
		UE_LOG(LogSony, Warning, TEXT("StartMorpheus2DVRReprojection called, but already in 2DVR mode! May need a stack of 2DVR requests or something."));
	}
...
void FGnmManager::ApplyMorpheus2DVRReprojection(GnmBridge::Morpheus2DVRReprojectionData& ReprojectionData)
{
	...
	{
		UE_LOG(LogSony, Warning, TEXT("ApplyMorpheus2DVRReprojection: reprojection texture is invalid. Falling back to GWhiteTexture."));
		ReprojectionData.Texture = GWhiteTexture->TextureRHI->GetTexture2D();
	}
	if (ReprojectionData.Texture->GetFormat() != PF_B8G8R8A8)
	{
		UE_LOG(LogSony, Warning, TEXT("ApplyMorpheus2DVRReprojection: reprojection texture format is incorrect. Must be B8G8R8A8. Falling back to GWhiteTexture."));
		ReprojectionData.Texture = GWhiteTexture->TextureRHI->GetTexture2D();
	}
	...
	parameter.label = (uint64_t *)ReprojectionLabels[ReprojectionData.RHIContextIndex];;
	if (*parameter.label != 0)
	{
		// label should always be clear here, but the cvar switch between previous and current context index break that (for one frame)
		UE_LOG(LogSony, Warning, TEXT("ApplyMorpheus2DVRReprojection parameter.label != 0.  Ok if you just switched to/from previous context mode."));
	}
	*parameter.label = 1;  // set to 0 when reprojection is done
...
void FGnmManager::ChangeSocialScreenOutputMode(EPS4SocialScreenOutputMode InPS4SocialScreenOutputMode)
{
	...
	UE_LOG(LogSony, Log, TEXT("ChangeSocialScreenOutputMode from %i to %i."), static_cast<uint32>(CurrentSocialScreenOutputMode), static_cast<uint32>(InPS4SocialScreenOutputMode));
	...
	if (ConfiguredSocialScreenOutputMode != InPS4SocialScreenOutputMode && InPS4SocialScreenOutputMode != EPS4SocialScreenOutputMode::Mirroring)
	{
		if (ConfiguredSocialScreenOutputMode != EPS4SocialScreenOutputMode::Mirroring)
		{
			...
			if (CurrentSocialScreenOutputMode == ConfiguredSocialScreenOutputMode)
			{
				...
				if (Ret == SCE_OK)
				{
					UE_LOG(LogSony, Log, TEXT("ChangeSocialScreenOutputMode set mode to mirroring while we close the configured mode."));
					CurrentSocialScreenOutputMode = EPS4SocialScreenOutputMode::Mirroring;
				}
				else
				{
					UE_LOG(LogSony, Error, TEXT("sceSocialScreenSetMode failed with code 0x%x.  Mode not changed."), Ret);
					return;
				}
			}
			...
			if (Ret == SCE_OK)
			{
				UE_LOG(LogSony, Log, TEXT("ChangeSocialScreenOutputMode previously configured mode closed."));
			}
			else
			{
				UE_LOG(LogSony, Error, TEXT("sceSocialScreenCloseSeparateMode failed with code 0x%x.  Mode not changed."), Ret);
				return;
			}
		}
		...
		if (Ret != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSocialScreenConfigureSeparateMode failed with code 0x%x.  Separate mirror mode will not start."), Ret);
			return;
		}
		...
		if (Ret == SCE_OK)
		{
			UE_LOG(LogSony, Log, TEXT("ChangeSocialScreenOutputMode opened mode %i."), static_cast<uint32>(InPS4SocialScreenOutputMode));
		...
		else
		{
			...
			{
				UE_LOG(LogSony, Error, TEXT("sceSocialScreenOpenSeparateMode failed with code SCE_SOCIAL_SCREEN_ERROR_CANNOT_OPEN_SEPARATE_MODE(0x%x).  The dialog to tell you the user how to fix this is supposed to pop up now."), Ret);
				return;
			}
			else
			{
				UE_LOG(LogSony, Error, TEXT("sceSocialScreenOpenSeparateMode failed with code 0x%x.  Separate mirror mode will not start."), Ret);
				return;
			}
		}
	}
	...
	// The mode we want to switch to is Mirroring or is configured now
	{
		...
		if (Ret == SCE_OK)
		{
			UE_LOG(LogSony, Log, TEXT("ChangeSocialScreenOutputMode switched mode to %i."), static_cast<uint32>(InPS4SocialScreenOutputMode));
...
const FGnmAuxBuffer& FGnmManager::GetSocialScreenAuxBuffer() const
{
#if HAS_MORPHEUS
	check(IsInRenderingThread);
	// Get the *next* render buffer, the current index one is the one that will be copied to output this frame.
	return AuxBuffers[(CurrentAuxBufferIndex_RenderThread + 1) % NUM_AUX_RENDER_BUFFERS];
#else
	static FGnmAuxBuffer Instance;
	return Instance;
#endif
}
...
inline bool SurfaceFormatIsValidForSrgb(Gnm::SurfaceFormat SurfaceFormat)
{
	switch (SurfaceFormat)
	{
	case Gnm::SurfaceFormat::kSurfaceFormat8:
	case Gnm::SurfaceFormat::kSurfaceFormat8_8:
	case Gnm::SurfaceFormat::kSurfaceFormat8_8_8_8:
	case Gnm::SurfaceFormat::kSurfaceFormatBc1:
	case Gnm::SurfaceFormat::kSurfaceFormatBc2:
	case Gnm::SurfaceFormat::kSurfaceFormatBc3:
	case Gnm::SurfaceFormat::kSurfaceFormatBc7:
		return true;
	default:
		return false;
	}
 ...
 * @param SrgbMode Will configure for sRGB or disable as requested if default not chosen
 *
 * @return a valid DataFormat object
 */
Gnm::DataFormat FGnmManager::GetDataFormat(EPixelFormat PixelFormat, EGnmSrgbMode SrgbMode)
{
	// look it up
	Gnm::DataFormat Format;
	Format.m_asInt = GPixelFormats[PixelFormat].PlatformFormat;

	if (SrgbMode != EGnmSrgbMode::Default)
	{
		// Override the format, only if the original format was kUNorm or kSrgb.
		switch (Format.getTextureChannelType())
		{
		case Gnm::kTextureChannelTypeUNorm:
			if (SrgbMode == EGnmSrgbMode::Enable && SurfaceFormatIsValidForSrgb((Gnm::SurfaceFormat)Format.m_bits.m_surfaceFormat))
			{
				Format.m_bits.m_channelType = Gnm::kTextureChannelTypeSrgb;
			}
			break;

		case Gnm::kTextureChannelTypeSrgb:
			if (SrgbMode == EGnmSrgbMode::Disable)
			{
				Format.m_bits.m_channelType = Gnm::kTextureChannelTypeUNorm;
			}
			break;
		}
	}
...
#endif
			FMemBlock::DumpDeletedMemory(*GLog, *MemBlockFreeLabel);
...
void FGnmManager::SubmitQueuedDCBs(bool bVsync, bool bPresent, bool bAuxFrame)
{
	...
	if (bAddedFinalSubmit)
	{
		...
		if (RetVal != SCE_OK)
		{
			UE_LOG(LogSony, Fatal, TEXT("Final submission failed with: 0x%x, %p"), RetVal, &FinalFrameSubmission);
		}
...
void FGnmManager::EndFrame(bool bPresent, bool bUpdatingCubemap)
{
	...
	// Time this lock. The RHI thread gets blocked here by the async
	// submission thread whilst it waits for GPU to finish the previous frame.
	uint32 IdleStart = FPlatformTime::Cycles();
	FScopeLock Lock(&SubmissionMutex);
	uint32 IdleCycles = FPlatformTime::Cycles() - IdleStart;
	...
	{
		GRenderThreadIdle[ERenderThreadIdleTypes::WaitingForGPUPresent] += IdleCycles;
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(FGnmDynamicRHI_CustomPresentTime);
...
//#endif

		{
			...
			//if (VideoOutCount >= 1)
			//{
			//	UE_LOG(LogSony, Warning, TEXT("EndFrame found that %i frames have flip sinced the last EndFrame.  We are probably missing framerate."));
			//}
			sceKernelWaitEqueue(VBlankEventQueue, &Event, 1, &NumEvents, nullptr);
		}
		break;
	...
	uint64 Start = *StartOfFrameTimestampLabel[PreviousContext];
	uint64 End = *EndOfFrameTimestampLabel[PreviousContext];
	if (Start > 0)
	{
	...
	if (bUseVSync)
	{
		uint64 FlipDifference = 0;
		for (int32 i = 0; i < FlipMarkersIndex[PreviousContext]; i++)
		{
			FFlipMarker& CurrentMarker = FlipMarkers[PreviousContext][i];

			uint64 FlipWaitStart = *CurrentMarker.StartTimestamp;
			uint64 FlipWaitEnd = *CurrentMarker.EndTimestamp;
	...
	if (!bUpdatingCubemap)
	{
		uint64 TotalSubmitWaitGPUCycles = SubmissionGapRecorder.SubmitSubmissionTimestampsForFrame(FrameCounter, StartOfSubmissionTimestamp[PreviousContext], EndOfSubmissionTimestamp[PreviousContext]);
	...
	Start = *StartOfDefragTimestampLabel[PreviousContext];
	End = *EndOfDefragTimestampLabel[PreviousContext];
...
void FGnmManager::FreePendingMemBlocks(bool bImmediate)
{
#if USE_NEW_PS4_MEMORY_SYSTEM

	check(IsInRenderingThread());

	uint64 CapturedRTValue = FMemBlock::RTFenceValue;
	FMemBlock::RTFenceValue++;

	FRHICommandListImmediate& RHICmdList = FRHICommandListExecutor::GetImmediateCommandList();
	RHICmdList.EnqueueLambda([this, CapturedRTValue, bImmediate](FRHICommandList&)
		{
			// Free all pending mem blocks whose GPU fence has signaled.
			FMemBlock::ProcessDelayedFrees(*MemBlockFreeLabel);

			if (bImmediate)
			{
				// In immediate mode, RHI + parallel threads are flushed. Signal the GPU to be at the same level as RHIT Fence
				FMemBlock::RHITFenceValue = CapturedRTValue;
			}

			// Enqueue the next fence signal on the GPU.
			GnmContextType& Context = ImmediateContext->GetContext();
			Context.writeImmediateAtEndOfPipe(Gnm::kEopFlushCbDbCaches, (void*)MemBlockFreeLabel, FMemBlock::RHITFenceValue, Gnm::kCacheActionNone);
	...
	if (bImmediate)
	{
		// Ensure the above lambda has run to completion
...
#if PS4_GNM_SLOW_FRAME_DEBUGGING
	UE_LOG(LogSony, Log, TEXT("Flushing and calling submit done"))
#endif
...
void* FGnmManager::AllocateFromTempRingBuffer(uint32 Size, uint32 Alignment, ETempRingBufferAllocateMode Mode)
{
	...
	if (Size > ChunkSize)
	{
		...
		{
		default:
			UE_LOG(LogSony, Fatal, TEXT("Cannot allocate %d bytes in temp ring buffer. Max size allowed is %d."), Size, ChunkSize);
			// Fall through
	...
	void* Buffer;
	{
		...
		Buffer = FrameTempAllocator[Index].Allocate(Size, Alignment);
	}
	check(Buffer);
...
FRHICOMMAND_MACRO(FRHICommandStartRHIThreadTempBufferFrame)
{
	int32 FrameIndex;
...
void FGnmManager::StartRenderThreadTempBufferFrame()
{
	...
	{
		check(IsInRenderingThread());
		ALLOC_COMMAND_CL(FRHICommandListExecutor::GetImmediateCommandList(), FRHICommandStartRHIThreadTempBufferFrame)(RenderThreadTempBufferIndex);
	}
	else
	{
...
uint64 FGnmSubmissionGapRecorder::SubmitSubmissionTimestampsForFrame(uint32 FrameCounter, const TArray<uint64*>& PrevFrameBeginSubmissionTimestamps, const TArray<uint64*>& PrevFrameEndSubmissionTimestamps)
{
