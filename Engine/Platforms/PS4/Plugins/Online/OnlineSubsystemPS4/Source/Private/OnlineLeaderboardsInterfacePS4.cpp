...
SceNpScoreBoardId FindScoreBoardId(const FName& LeaderboardName, const FName& StatName)
{
	...
	if (Ret == false)
	{
		UE_LOG_ONLINE_LEADERBOARD(Error,
...
SceNpScoreValue GetNpScoreFromVariant(const FVariantData& Stat)
{
	...
	{
		UE_LOG_ONLINE_LEADERBOARD(Warning, TEXT("Unsuppported score type for PS4 scoreboards: %s"), *Stat.ToString());
	}
...
FVariantData GetVarientFromNpScore(const EOnlineKeyValuePairDataType::Type StatType, const SceNpScoreValue Score)
{
	...
	{
		UE_LOG_ONLINE_LEADERBOARD(Warning, TEXT("Unsuppported score type for PS4 scoreboards: %s"), *EOnlineKeyValuePairDataType::ToString(StatType));
	}
...
public:
	...
	virtual void Tick() override
	{
		if (!bInit)
		{
			...
			{
				UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScoreCreateRequest error = 0x%08x, user is not signed in to PSN"), RequestId);
				bInit = true;
			...
			{
				UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScoreCreateRequest error = 0x%08x"), RequestId);
				bInit = true;
				bWasSuccessful = false;
				bIsComplete = true;
			}
		...
		else
		{
			...
			{
				// We've hit an error
				UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScorePollAsync error = 0x%08x"), Waiting);
				bIsComplete = true;
				bWasSuccessful = false;
	...
	virtual void Finalize() override
	{
		...
		{
			UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("FOnlineAsyncTaskPS4Leaderboard RequestId error = 0x%08x"), RequestId);
		}
		AsyncFinalize();
	}
...
public:
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4WriteLeaderboards bWasSuccessful: %d"), WasSuccessful());
	}
	...
	virtual bool AsyncBegin(int32 InRequestId) override
	{
		...
		if (Result < 0)
		{
			UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScoreRecordScoreAsync error = 0x%08x"), Result);
			return false;
		}
		return true;
	}
	...
	virtual void AsyncCheckResult(int32 Result) override
	{
		...
		if (Result == SCE_NP_COMMUNITY_SERVER_ERROR_NOT_BEST_SCORE)
		{
			...
			UE_LOG_ONLINE_LEADERBOARD(Display, TEXT("sceNpScoreRecordScore -- not best score"));
		}
		else if (Result == SCE_NP_COMMUNITY_SERVER_ERROR_INVALID_ANTICHEAT_DATA)
		{
			UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScoreRecordScore error = 0x%08x, Invalid passphrase (check that proper Title Id and Secret is used in sceNpSetNpTitleId)"), Result);
			bWasSuccessful = false;
		}
		else if (Result < 0)
		{
			// Recording the score failed for some reason.
			UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScoreRecordScore error = 0x%08x"), Result);
			bWasSuccessful = false;
		}
	}
...
public:
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4ReadLeaderboards bWasSuccessful: %d"), WasSuccessful());
	}
	...
	virtual void AsyncCheckResult(int32 Result) override
	{
		...
		{
			// Recording the score failed for some reason.
			UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceNpScoreGetRankingByAccountIdAsync error = 0x%08x"), Result);
			bWasSuccessful = false;
		}
		else
		{
	...
	virtual void AsyncFinalize() override
	{
		...
		for (int32 Idx = 0; Idx < UserIdCount; ++Idx)
		{
			...
			if (UserRow == nullptr)
			{
				...
				{
					UE_LOG_ONLINE_LEADERBOARD(Warning, TEXT("Leaderboards: Cannot determine nickname for account '%s'."), *PS4AccountIdToString(CurrentUserId->GetAccountId()));
				}
			...
			if (bWasSuccessful && RankData.hasData)
			{
				...
				if (!ReadObject->SortedColumn.GetComparisonIndex() || ReadObject->SortedColumn == ColumnMetaData.ColumnName)
				{
...
public:
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4ReadLeaderboardsForFriends bWasSuccessful: %d"), WasSuccessful());
	}
	...
	, TitleCtxId(0)
{
	...
	{
		UE_LOG_ONLINE_LEADERBOARD(Fatal, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_NP_SCORE_RANKING) failed: 0x%x"), Result);
	}
	...
	{
		UE_LOG_ONLINE_LEADERBOARD(Fatal, TEXT("sceUserServiceGetInitialUser failed: 0x%x"), Result);
	}

	if ((Result = sceNpScoreCreateNpTitleCtxA(ServiceLabel, InitialUserId)) <= 0)
	{
		UE_LOG_ONLINE_LEADERBOARD(Log, TEXT("sceNpScoreCreateNpTitleCtx failed: 0x%x"), Result);
	}
...
bool FOnlineLeaderboardsPS4::ReadLeaderboards(const TArray<TSharedRef<const FUniqueNetId>>& Players, FOnlineLeaderboardReadRef& ReadObject)
{
	...
	{
		UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("sceUserServiceGetInitialUser failed with error code 0x%08x"), Result);
		return false;
	}

	int32 LocalUserIndex = FSonyApplication::GetSonyApplication()->GetUserIndex(InitialUser);
	...
	Delegate.AddLambda([=](FOnlineIdMap ResolvedIds, bool bNameResolutionSuccessful, FString ErrorString)
	{
		...
		{
			UE_LOG_ONLINE_LEADERBOARD(Error, TEXT("Leaderboard name resolution failed: %s"), *ErrorString);
		}
...
{
	UE_LOG_ONLINE_LEADERBOARD(Warning, TEXT("FOnlineLeaderboardsPS4::ReadLeaderboardsAroundRank is currently not supported."));
	return false;
}
...
{
	UE_LOG_ONLINE_LEADERBOARD(Warning, TEXT("FOnlineLeaderboardsPS4::ReadLeaderboardsAroundUser is currently not supported."));
	return false;
}

void FOnlineLeaderboardsPS4::FreeStats(FOnlineLeaderboardRead& ReadObject)
{
	UE_LOG_ONLINE_LEADERBOARD(Display, TEXT("FOnlineLeaderboardsPS4::FreeStats()"));
}
...
{
	UE_LOG_ONLINE_LEADERBOARD(Display, TEXT("FOnlineLeaderboardsPS4::FlushLeaderboards() not supported"));
...
{
	UE_LOG_ONLINE_LEADERBOARD(Display, TEXT("FOnlineLeaderboardsPS4::WriteOnlinePlayerRatings() not supported"));
	return false;
}
