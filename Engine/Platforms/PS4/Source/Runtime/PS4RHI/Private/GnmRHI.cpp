...
inline void SetInvalid(EPixelFormat Format)
{
	const uint32 INVALID = Gnm::kDataFormatInvalid.m_asInt;
	GPixelFormats[Format].PlatformFormat = INVALID;
	GPixelFormats[Format].Supported = false;
}

inline void SetFormat(EPixelFormat Format, uint32 AsInt)
{
	GPixelFormats[Format].PlatformFormat = AsInt;
	GPixelFormats[Format].Supported = true;
}

inline void SetFormat(EPixelFormat Format, uint32 AsInt, int32 BlockBytes)
{
	GPixelFormats[Format].PlatformFormat = AsInt;
	GPixelFormats[Format].BlockBytes = BlockBytes;
	GPixelFormats[Format].Supported = true;
}
...
FGnmDynamicRHI::FGnmDynamicRHI()
{
	...
	GRHISupportsDepthUAV = true;
	GRHISupportsAtomicUInt64 = true;
	GRHISupportsResummarizeHTile = true;
	GRHISupportsExplicitHTile = true;
	GRHISupportsDepthUAV = true;
	...
	GRHISupportsDirectGPUMemoryLock = true;
	...
	SetInvalid(PF_Unknown);
	// @todo gnm: 96 and 128 bit render targets aren't supported yet, so always use 16 bits when 32 are requested
//	SetFormat(PF_A32B32G32R32F		, Gnm::DataFormat::build(Gnm::kSurfaceFormat16_16_16_16, Gnm::kTextureChannelTypeFloat, Gnm::kTextureChannelZ, Gnm::kTextureChannelY, Gnm::kTextureChannelX, Gnm::kTextureChannelW).m_asInt);
	SetFormat(PF_A32B32G32R32F		, Gnm::kDataFormatR32G32B32A32Float.m_asInt);
	SetFormat(PF_B8G8R8A8			, Gnm::kDataFormatB8G8R8A8Unorm.m_asInt);
	SetFormat(PF_G8					, Gnm::kDataFormatR8Unorm.m_asInt);
	SetFormat(PF_G16				, Gnm::kDataFormatR16Unorm.m_asInt);
	SetFormat(PF_DXT1				, Gnm::kDataFormatBc1Unorm.m_asInt);
	SetFormat(PF_DXT3				, Gnm::kDataFormatBc2Unorm.m_asInt);
	SetFormat(PF_DXT5				, Gnm::kDataFormatBc3Unorm.m_asInt);
	SetInvalid(PF_UYVY); // @todo gnm: Is it supported? I don't think so. Maybe kFormatBG_RG ?
	SetFormat(PF_FloatRGB			, Gnm::kDataFormatR11G11B10Float.m_asInt, 4);
	SetFormat(PF_FloatRGBA			, Gnm::kDataFormatR16G16B16A16Float.m_asInt, 8);
	SetFormat(PF_DepthStencil		, Gnm::kDataFormatR32Float.m_asInt, 4);
	SetFormat(PF_X24_G8				, Gnm::kDataFormatR8Uint.m_asInt, 1);
	SetFormat(PF_ShadowDepth		, Gnm::kDataFormatR16Unorm.m_asInt, 2);
	SetFormat(PF_R32_FLOAT			, Gnm::kDataFormatR32Float.m_asInt);
	SetFormat(PF_G16R16				, Gnm::kDataFormatR16G16Unorm.m_asInt);
	SetFormat(PF_G16R16F			, Gnm::kDataFormatR16G16Float.m_asInt);
	SetFormat(PF_G16R16F_FILTER		, Gnm::kDataFormatR16G16Float.m_asInt);
	SetFormat(PF_G32R32F			, Gnm::kDataFormatR32G32Float.m_asInt);
	SetFormat(PF_A2B10G10R10		, Gnm::kDataFormatR10G10B10A2Unorm.m_asInt);
	SetFormat(PF_ETC1				, Gnm::kDataFormatB10G10R10A2Unorm.m_asInt);
	SetFormat(PF_A16B16G16R16		, Gnm::kDataFormatR16G16B16A16Unorm.m_asInt);
	SetInvalid(PF_D24); // @todo gnm: Do we need this?
	SetFormat(PF_R16F				, Gnm::kDataFormatR16Float.m_asInt);
	SetFormat(PF_R16F_FILTER		, Gnm::kDataFormatR16Float.m_asInt);
	SetFormat(PF_BC5				, Gnm::kDataFormatBc5Unorm.m_asInt);
	SetFormat(PF_V8U8				, Gnm::kDataFormatR8G8Snorm.m_asInt);
	SetInvalid(PF_A1); // Not supported for rendering.
	SetFormat(PF_FloatR11G11B10		, Gnm::kDataFormatR11G11B10Float.m_asInt, 4);
	SetFormat(PF_L8					, Gnm::kDataFormatL8Unorm.m_asInt);
	SetFormat(PF_A8					, Gnm::kDataFormatA8Unorm.m_asInt);
	SetFormat(PF_R32_UINT			, Gnm::kDataFormatR32Uint.m_asInt);
	SetFormat(PF_R32_SINT			, Gnm::kDataFormatR32Sint.m_asInt);
	SetFormat(PF_R16G16B16A16_UINT	, Gnm::kDataFormatR16G16B16A16Uint.m_asInt);
	SetFormat(PF_R16G16B16A16_SINT	, Gnm::kDataFormatR16G16B16A16Sint.m_asInt);
	SetFormat(PF_R5G6B5_UNORM		, Gnm::kDataFormatB5G6R5Unorm.m_asInt);
	SetFormat(PF_R8G8B8A8			, Gnm::kDataFormatR8G8B8A8Unorm.m_asInt);
	SetFormat(PF_R8G8B8A8_UINT		, Gnm::kDataFormatR8G8B8A8Uint.m_asInt);
	SetFormat(PF_R8G8B8A8_SNORM		, Gnm::kDataFormatR8G8B8A8Snorm.m_asInt);
	SetFormat(PF_R8G8				, Gnm::kDataFormatR8G8Unorm.m_asInt);
	SetFormat(PF_BC4				, Gnm::kDataFormatBc4Unorm.m_asInt);
	SetFormat(PF_R16G16_UINT        , Gnm::kDataFormatR16G16Uint.m_asInt);
	SetFormat(PF_R32G32_UINT        , Gnm::kDataFormatR32G32Uint.m_asInt);
	SetFormat(PF_R32G32B32A32_UINT	, Gnm::kDataFormatR32G32B32A32Uint.m_asInt);
	SetFormat(PF_R16_UINT			, Gnm::kDataFormatR16Uint.m_asInt);
	SetFormat(PF_R16_SINT			, Gnm::kDataFormatR16Sint.m_asInt);
	SetFormat(PF_BC6H				, Gnm::kDataFormatBc6Unorm.m_asInt);
	SetFormat(PF_BC7				, Gnm::kDataFormatBc7Unorm.m_asInt);
	SetFormat(PF_R8_UINT			, Gnm::kDataFormatR8Uint.m_asInt);
	SetFormat(PF_R16G16B16A16_UNORM	, Gnm::kDataFormatR16G16B16A16Unorm.m_asInt);
	SetFormat(PF_R16G16B16A16_SNORM	, Gnm::kDataFormatR16G16B16A16Snorm.m_asInt);
...
void* FGnmDynamicRHI::RHIGetNativeInstance()
{
	return nullptr;
}
...
FRHICOMMAND_MACRO(FRHICommandUnlockTexture2DUpdate)
{
...
FRHICOMMAND_MACRO(FRHICommandUpdateTexture2D)
{
...
FRHICOMMAND_MACRO(FRHICommandUpdate3DTexture)
{
...
FRHICOMMAND_MACRO(FRHICommandGnmEndUpdate3DTexture)
{
