...
void FMobileSceneRenderer::Render(FRHICommandListImmediate& RHICmdList)
{
	...
	FIntPoint RenderTargetSize = ViewFamily.RenderTarget->GetRenderTargetTexture().IsValid() ? ViewFamily.RenderTarget->GetRenderTargetTexture()->GetSizeXY() : ViewFamily.RenderTarget->GetSizeXY();
	...
	FTextureRHIParamRef VariableRes = nullptr;
	
	if (SceneContext.IsVariableResolutionTextureAllocated())
	{
		VariableRes = SceneContext.GetVariableResolutionTexture();
	}
