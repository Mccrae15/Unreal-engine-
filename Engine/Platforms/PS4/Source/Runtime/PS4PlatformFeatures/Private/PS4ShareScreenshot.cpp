...
void FPS4ShareScreenshot::EnableScreenshots(bool Enable)
{
	if (bIsInitialized)
	{
		...
		if (bIsEnabled)
		{
			...
			if (Result != SCE_OK)
			{
				UE_LOG(LogSony, Error, TEXT("sceScreenShotEnable failed. Error code: 0x%08x"), Result);
			}
		}
		else
		{
			...
			if (Result != SCE_OK)
			{
				UE_LOG(LogSony, Error, TEXT("sceScreenShotDisable failed. Error code: 0x%08x"), Result);
			}
	...
	{
		UE_LOG(LogSony, Error, TEXT("FPS4ShareScreenshot not initialized"));
	}
}
...
bool FPS4ShareScreenshot::Initialize()
{
	if (!bIsInitialized)
	{
		...
		int32 Result = sceSysmoduleLoadModule(SCE_SYSMODULE_SCREEN_SHOT);
  		...
  		if (Result != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_SCREEN_SHOT) failed. Error code: 0x%08x"), Result);
...
void FPS4ShareScreenshot::Shutdown()
{
	if (bIsInitialized)
	{
		int32 Result = sceSysmoduleUnloadModule(SCE_SYSMODULE_SCREEN_SHOT);
  		if (Result != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSysmoduleUnloadModule(SCE_SYSMODULE_SCREEN_SHOT) failed. Error code: 0x%08x"), Result);
  		}
