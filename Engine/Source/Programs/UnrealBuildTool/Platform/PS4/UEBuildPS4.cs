...
namespace UnrealBuildTool
{
	...
	class PS4Platform : UEBuildPlatform
	{
		...
		public override void ValidateTarget(TargetRules Target)
		{
			...
			{
				Log.TraceInformation("Building for PGO generation. Output will be to /data/<gamename>/build/ps4/default.profraw!");
			}
			else if (GetLTOEnabled(new ReadOnlyTargetRules(Target)))
			{
				Log.TraceInformation("Using LTO, linking will be extremely (45-90m) slow!");
				...
				if (string.IsNullOrEmpty(Target.PS4Platform.ProfileGuidedOptimizationInput) == false)
				{
					...
					Log.TraceInformation("Using {0} PGO with input {1}", PGOType, PGOInput);
				}
			}
		}
		...
		public override void ModifyModuleRulesForActivePlatform(string ModuleName, ModuleRules Rules, ReadOnlyTargetRules Target)
		{
			...
			{
				string FreeType2Path = "ThirdParty/FreeType2/FreeType2-2.6/";
				Rules.PublicLibraryPaths.Add(FreeType2Path + "Lib/PS4");
				Rules.PublicAdditionalLibraries.Add("freetype26");
			}
		...
		public override void SetUpEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment CompileEnvironment, LinkEnvironment LinkEnvironment)
		{
			...
			CompileEnvironment.IncludePaths.SystemIncludePaths.Add(BaseSdkDir);
			CompileEnvironment.IncludePaths.SystemIncludePaths.Add(DirectoryReference.Combine(BaseSdkDir, "host_tools/lib/clang/3.1/include"));
			CompileEnvironment.IncludePaths.SystemIncludePaths.Add(DirectoryReference.Combine(BaseSdkDir, "target/include"));
			CompileEnvironment.IncludePaths.SystemIncludePaths.Add(DirectoryReference.Combine(BaseSdkDir, "target/include_common"));
			...
			bool bMorpheusPluginEnabled = Plugins.IsPluginEnabledForProject(MorpheusPlugin, Project, UnrealTargetPlatform.PS4, Target.Configuration, Target.Type);
	...
	class PS4PlatformSDK : UEBuildPlatformSDK
	{
		...
		public const string ExpectedSDKVersion = "5.508.031";

		// SDK version expected from a default install from SDK_Manager
		public const string ExpectedManuallyInstalledSDKVersion = "5.500";
		...
		public const uint MinimumSystemSoftwareVersion = 0x05508011u;
			...
			if (InstalledVersion != INVALID_SDK)
			{
				...
				else
				{
					...
					if (InstalledVersion > SDKVersionStringToUint(ExpectedSDKVersion))
					{
						if( !bAutoSDKIsValid )
						{
							Log.TraceInformation("*** Found installed PS4 SDK version {0} but require {1}, this may cause unknown issues ***", SDKVersionToString(InstalledVersion), ExpectedSDKVersion);
						}
						return SDKStatus.Valid;
					}
					else
					{
						if (!bAutoSDKIsValid)
						{
							Log.TraceInformation("*** Found installed PS4 SDK version {0} but require {1} ***", SDKVersionToString(InstalledVersion), ExpectedSDKVersion);
						}
					}
				}
			}
