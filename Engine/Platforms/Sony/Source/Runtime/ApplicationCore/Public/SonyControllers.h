...
////////////////////////////////////////////////////////////////////
//
// This code is shared between Sony runtime (SonyInputInterface) and a
...
#define USE_CONSOLE_FEATURES (!PLATFORM_DESKTOP)
...
struct FSonyLightColorProperty : public FInputDeviceProperty
{
	FSonyLightColorProperty(bool bInEnable, FColor InColor)
		: FInputDeviceProperty(PropertyName())
		, bEnable(bInEnable)
		, Color(InColor)
	{}

	static FName PropertyName() { return FName("SonyLightColor"); }

	bool bEnable;
	FColor Color;
}; 

class FSonyControllers
{
public:
...
#if USE_CONSOLE_FEATURES
		FCoreDelegates::OnUserLoginChangedEvent.AddRaw(this, &FSonyControllers::UserLoginEventHandler);
#endif
	}

	virtual ~FSonyControllers()
	{
	}

	bool IsMouseAttached() const
	{
		return bMouseAttached;
	}
	...
	{		
		...
		if (ControllerState.bIsConnected || bWasConnected)
		{
			...
			if (!bWasConnected && ControllerState.bIsConnected)
			{
				...
				{
					// All is well
					ControllerState.bIsRemotePlay = (ControllerState.ControllerInfo.connectionType != SCE_PAD_CONNECTION_TYPE_LOCAL);
			...
			for (int NewTouchIndex = 0; NewTouchIndex < ControllerState.PadData.touchData.touchNum; NewTouchIndex++)
			{
				...
				for (int32 OldTouchIndex = 0; OldTouchIndex < SCE_PAD_MAX_TOUCH_NUM && !bFoundOldTouch; OldTouchIndex++)
				{
					...
					if (ControllerState.TouchMapping[OldTouchIndex].Z == Touch.id)
					{
						...
						{
							// found one, update it
							MessageHandler->OnTouchMoved(FVector2D(Touch.x, Touch.y), 1.0f, OldTouchIndex, ControllerState.ControllerId);
						}
...
#undef POSITIVE_DEADZONE
		...
		extern int32 GSonyEnableMouse;
		if (GSonyEnableMouse && ControllerState.MouseHandle >= 0 && UserIndex == UserBoundToCursor)
		{
			...
			int32 NumData = sceMouseRead( ControllerState.MouseHandle, MouseData, UE_ARRAY_COUNT( MouseData ) );
			if( NumData > 0 )
			{
				bool bAttached = false;
				for( int32 MouseDataIndex = 0; MouseDataIndex < NumData; MouseDataIndex++ )
				{
					bAttached |= MouseData[MouseDataIndex].connected;
					if( MouseData[MouseDataIndex].connected == true && ( ( MouseData[MouseDataIndex].buttons & SCE_MOUSE_BUTTON_INTERCEPTED ) == 0 ) )
					{
						if( Cursor.IsValid() )
						{
							extern float GSonyMouseSensitivity;

							// Update cursor position
							FVector2D CursorPos = Cursor->GetPosition();
							CursorPos.X += MouseData[MouseDataIndex].xAxis * GSonyMouseSensitivity;
							CursorPos.Y += MouseData[MouseDataIndex].yAxis * GSonyMouseSensitivity;
						...
						LastMouseOffset.X += MouseData[MouseDataIndex].xAxis;
						LastMouseOffset.Y += MouseData[MouseDataIndex].yAxis;
						...
						for( int32 ButtonIndex = 0; ButtonIndex < UE_ARRAY_COUNT( MouseButtonMappings ); ButtonIndex++ )
						{
							...
							if( ( MouseData[MouseDataIndex].buttons & SceButton ) != ( ControllerState.MouseButtonState & SceButton ) )
							{
								...
								{
									extern float GSonyMouseDoubleClickDelay;
									extern float GSonyMouseDoubleClickArea;
									const float LastPressDelay = CurrentTime - LastMousePressTime[UE4MouseButton];
									const bool bHasDoubleClick = (GSonyMouseDoubleClickDelay > 0.0f && LastPressDelay < GSonyMouseDoubleClickDelay && LastMouseOffset.SizeSquared() <= FMath::Square(GSonyMouseDoubleClickArea));
									if( bHasDoubleClick )
									{
										MessageHandler->OnMouseDoubleClick( nullptr, UE4MouseButton );
									}
									else
									{
									...
									LastMousePressTime[UE4MouseButton] = CurrentTime;
	...
	{
		// Don't bother looking if this isn't a channel the Sony supports
	...
	{
		if (Property->Name == FSonyLightColorProperty::PropertyName())
		{
			// The channel is valid, so if the controller is open, set its value
			for (int32 UserIndex = 0; UserIndex < SCE_USER_SERVICE_MAX_LOGIN_USERS; ++UserIndex)
			{
				...
				if (ControllerState.ControllerId == ControllerId)
				{
					const FSonyLightColorProperty* LightColor = static_cast<const FSonyLightColorProperty*>(Property);
					if (LightColor->bEnable)
					{
						ScePadLightBarParam LightBarParam;
						LightBarParam.r = LightColor->Color.R;
						LightBarParam.g = LightColor->Color.G;
						LightBarParam.b = LightColor->Color.B;
						scePadSetLightBar(ControllerState.Handle, &LightBarParam);
					}
					else
					{
						scePadResetLightBar(ControllerState.Handle);
					}
					break;
				}
			}
	...
	int32 ConnectStateToUser(int32 UserID, int32 UserIndex)
	{
		...
		for (int32 TouchIndex = 0; TouchIndex < UE_ARRAY_COUNT(ControllerState.TouchMapping); TouchIndex++)
		{
		...
		extern int32 GSonyEnableMouse;
		if (GSonyEnableMouse && UserIndex == 0)
		{
...
protected:
	...
	void UpdateVibeMotors(FControllerState &State)
	{
		...
		if (!FMath::IsNearlyEqual(LargeValue, State.LastLargeValue) || !FMath::IsNearlyEqual(SmallValue, State.LastSmallValue))
		{
			...
			State.LastLargeValue = LargeValue;
			State.LastSmallValue = SmallValue;
		}
	}
	...
	/** true when a mouse is attached to the console (and mouse support is enabled) */
	bool bMouseAttached;
	...
	/** Timestamps for last known mouse press events */
	float LastMousePressTime[EMouseButtons::Invalid];

	/** Relative mouse movement distance since last mouse down (for double-click dead zone) */
