	...
	, ForegroundMaxDistance(300.0f)
{
	...
	for (unsigned int i = 0; i < NumRTs; ++i)
	{
		...
		BackgroundRenderTargets[i]->ClearColor = FLinearColor::Transparent;
		...
		ForegroundRenderTargets[i]->ClearColor = FLinearColor::Transparent;
...
void AOculusMR_CastingCameraActor::Tick(float DeltaTime)
{
	...
	if (GetCaptureComponent2D()->IsVisible())
	{
		...
		void** BackgroundTexture = (void**)BackgroundRenderTargets[EncodeIndex]->Resource->TextureRHI->GetNativeResource();
		void** ForegroundTexture = (void**)ForegroundRenderTargets[EncodeIndex]->Resource->TextureRHI->GetNativeResource();
		int NumChannels = 2;
