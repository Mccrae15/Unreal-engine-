...
UInteractiveToolsContext* UFractureTransformGizmoSettings::GetToolsContext()
{
	if (AttachedCutter)
	{
		return AttachedCutter->GetToolsContext();
	}
	return nullptr;
}
...
FBox UFractureToolVoronoiCutterBase::GetVoronoiBounds(const FFractureToolContext& FractureContext, const TArray<FVector>& Sites) const
{
	...
	VoronoiBounds = VoronoiBounds.ExpandBy(CutterSettings->Amplitude + CutterSettings->Grout + KINDA_SMALL_NUMBER);
