...
void* FGnmComputeCommandListContext::AllocateFromTempFrameBuffer(uint32 Size, uint32 Alignment)
{
...
void FGnmComputeCommandListContext::SetTexture(FRHITexture* NewTextureRHI, uint32 TextureIndex)
{
	check(IsInRenderingThread() || IsInRHIThread());
	if (NewTextureRHI)
	{
...
void FGnmComputeCommandListContext::SetSRV(FRHIShaderResourceView* SRVRHI, uint32 TextureIndex)
{
	...
	if (SRV)
	{
		...
		else
		{
			...
			{
				void* BufferData = SRV->SourceMultiBufferResource->GetCurrentBuffer();
#if PS4_USE_NEW_MULTIBUFFER == 0
				if (BufferData == nullptr)
				{
					UE_LOG(LogRHI, Warning, TEXT("Volatile multibuffer trying to use data before any data has been provided. Adding memory to avoid crash."));
					BufferData = SRV->SourceMultiBufferResource->GetCurrentBuffer(true, true);
					FMemory::Memset(BufferData, 0, SRV->SourceMultiBufferResource->GetBufferSize());
				}
#endif
...
void FGnmComputeCommandListContext::RHIWaitComputeFence(FRHIComputeFence* InFenceRHI)
{
...
void FGnmComputeCommandListContext::RHISetComputeShader(FRHIComputeShader* ComputeShaderRHI)
{
...
void FGnmComputeCommandListContext::RHIDispatchIndirectComputeShader(FRHIVertexBuffer* ArgumentBufferRHI, uint32 ArgumentOffset)
{
#if 0
	if (IsImmediate())
	{
...
void FGnmComputeCommandListContext::RHITransitionResources(EResourceTransitionAccess TransitionType, EResourceTransitionPipeline TransitionPipeline, FRHIUnorderedAccessView** InUAVs, int32 NumUAVs, FRHIComputeFence* WriteComputeFenceRHI)
{
...
void FGnmComputeCommandListContext::RHISetShaderTexture(FRHIComputeShader* ComputeShaderRHI, uint32 TextureIndex, FRHITexture* NewTextureRHI)
{	
	SetTexture(NewTextureRHI, TextureIndex);
}

void FGnmComputeCommandListContext::RHISetShaderSampler(FRHIComputeShader* ComputeShader, uint32 SamplerIndex, FRHISamplerState* NewStateRHI)
{
	FGnmSamplerState* NewState = ResourceCast(NewStateRHI);	
	ComputeContext.setSamplers(SamplerIndex, 1, &NewState->Sampler);
}

void FGnmComputeCommandListContext::RHISetUAVParameter(FRHIComputeShader* ComputeShader, uint32 UAVIndex, FRHIUnorderedAccessView* UAVRHI)
{
...
void FGnmComputeCommandListContext::RHISetUAVParameter(FRHIComputeShader* ComputeShader, uint32 UAVIndex, FRHIUnorderedAccessView* UAVRHI, uint32 InitialCount)
{
	FGnmUnorderedAccessView* UAV = ResourceCast(UAVRHI);
	check(UAVIndex < FGnmContextCommon::MaxBoundUAVs);
...
void FGnmComputeCommandListContext::RHISetShaderResourceViewParameter(FRHIComputeShader* ComputeShader, uint32 SamplerIndex, FRHIShaderResourceView* SRVRHI)
{
	SetSRVForStage(SRVRHI, SamplerIndex, Gnm::kShaderStageCs);
}

void FGnmComputeCommandListContext::RHISetShaderUniformBuffer(FRHIComputeShader* ComputeShader, uint32 BufferIndex, FRHIUniformBuffer* BufferRHI)
{
...
void FGnmComputeCommandListContext::RHISetShaderParameter(FRHIComputeShader* ComputeShader, uint32 BufferIndex, uint32 BaseIndex, uint32 NumBytes, const void* NewValue)
{
