...
void FPS4SharePlay::EnableSharing(bool Enable)
{
	if (bIsInitialized)
	{
		...
		if (bIsEnabled)
		{
			...
			if (Result != SCE_OK)
			{
				UE_LOG(LogSony, Error, TEXT("sceSharePlaySetProhibition failed. Error code: 0x%08x"), Result);
			}
		}
		else
		{
	...
	{
		UE_LOG(LogSony, Error, TEXT("FPS4SharePlay not initialized"));
	}
}
...
bool FPS4SharePlay::Initialize()
{
	if (!bIsInitialized)
	{
		...
		int32 Result = sceSysmoduleLoadModule(SCE_SYSMODULE_SHARE_PLAY);
  		if (Result != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_SHARE_PLAY) failed. Error code: 0x%08x"), Result);
		...
		if (Result != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSharePlayInitialize failed. Error code: 0x%08x"), Result);
...
void FPS4SharePlay::Shutdown()
{
	if (bIsInitialized)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSharePlayTerminate failed. Error code: 0x%08x"), Result);
		}

		Result = sceSysmoduleUnloadModule(SCE_SYSMODULE_SHARE_PLAY);
  		if (Result != SCE_OK)
		{
			UE_LOG(LogSony, Error, TEXT("sceSysmoduleUnloadModule(SCE_SYSMODULE_SHARE_PLAY) failed. Error code: 0x%08x"), Result);
  		}
