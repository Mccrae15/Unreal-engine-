...
{
	CommitGraphicsPipelineState(RHICmdList, *this, DrawRenderState, View->GetFeatureLevel(), View->bVRProjectEnabled || PolicyContext.bIsSinglePassStereo);
	...
	)
{
	...
	if (Mesh.bUseAsOccluder || !DrawingContext.bRespectUseAsOccluderFlag || DrawingContext.DepthDrawingMode == DDM_AllOpaque)
	{
			...
			)
		{
			...
			DrawingPolicy.SetSharedState(RHICmdList, &View, FPositionOnlyDepthDrawingPolicy::ContextDataType(bIsInstancedStereo, bIsSinglePassStereo, bIsInstancedStereoEmulated, bNeedsSinglePassStereoBias), DrawRenderStateLocal);
		...
		else if (!IsTranslucentBlendMode(BlendMode) || Material->IsTranslucencyWritingCustomDepth())
		{
			...
			if(bDraw)
			{
				...
				FDrawingPolicyRenderState DrawRenderStateLocal(&RHICmdList, DrawRenderState);
				DrawingPolicy.SetSharedState(RHICmdList, &View, FDepthDrawingPolicy::ContextDataType(bIsInstancedStereo, bIsSinglePassStereo, bIsInstancedStereoEmulated, bNeedsSinglePassStereoBias), DrawRenderStateLocal);
...
bool FDeferredShadingSceneRenderer::RenderPrePassHMD(FRHICommandListImmediate& RHICmdList)
{
	...
	if (IsLMSEnabled)
	{
		...
		RHICmdList.ClearDepthStencilTexture(SceneContext.GetSceneDepthSurface(), EClearDepthStencil::DepthStencil, ClearDepth, ClearStencil, FIntRect());
	}
