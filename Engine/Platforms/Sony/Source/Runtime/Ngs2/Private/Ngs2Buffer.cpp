...
FAutoConsoleVariableRef CVarUseResourceTracking(
	TEXT("au.ngs2.UseResourceTracking"),
...
void FNgs2SoundBuffer::InitWaveformInfo(USoundWave* InSoundWave)
{
	...
	Format.numChannels = InSoundWave ? InSoundWave->NumChannels : 1;
	if (InSoundWave && InSoundWave->bProcedural)
	{
		Format.sampleRate = InSoundWave->GetSampleRateForCurrentPlatform();
	}
	else
	{
...
bool FNgs2SoundBuffer::ReadCompressedData(uint8* Destination, int32 NumFramesToDecode, bool bLooping)
{
	if (!DecompressionState)
	{
...
FNgs2SoundBuffer* FNgs2SoundBuffer::CreateNativeBuffer(FAudioDevice* InAudioDevice, USoundWave* InSoundWave)
{
	...
	if (AT9BulkData)
	{
		...
		AT9BulkData->GetCopy((void**)&DataPtr, UseResourceTrackingCVar);
...
FNgs2SoundBuffer* FNgs2SoundBuffer::CreateStreamingBuffer(FAudioDevice* InAudioDevice, USoundWave* InSoundWave)
{
	...
	{
		InSoundWave->SetSampleRate(QualityInfo.SampleRate);
