				...
				if( NumData > 0 )
				{
					...
					for( int32 MouseDataIndex = 0; MouseDataIndex < NumData; MouseDataIndex++ )
					{
						...
						if( MouseData[MouseDataIndex].connected == true && ( ( MouseData[MouseDataIndex].buttons & SCE_MOUSE_BUTTON_INTERCEPTED ) == 0 ) )
						{
							if( Cursor.IsValid() )
							{
								extern float GPS4MouseSensitivity;

								// Update cursor position
								FVector2D CursorPos = Cursor->GetPosition();
								CursorPos.X += MouseData[MouseDataIndex].xAxis * GPS4MouseSensitivity;
								CursorPos.Y += MouseData[MouseDataIndex].yAxis * GPS4MouseSensitivity;
							...
							for( int32 ButtonIndex = 0; ButtonIndex < ARRAY_COUNT( MouseButtonMappings ); ButtonIndex++ )
							{
								...
								if( ( MouseData[MouseDataIndex].buttons & SceButton ) != ( ControllerState.MouseButtonState & SceButton ) )
								{
									...
									{
										extern float GPS4MouseDoubleClickDelay;
										extern float GPS4MouseDoubleClickArea;
										const float LastPressDelay = CurrentTime - LastMousePressTime[UE4MouseButton];
										const bool bHasDoubleClick = (GPS4MouseDoubleClickDelay > 0.0f && LastPressDelay < GPS4MouseDoubleClickDelay && LastMouseOffset.SizeSquared() <= FMath::Square(GPS4MouseDoubleClickArea) );
										if( bHasDoubleClick )
										{
