...
{
	return FString::Printf(TEXT("FOnlineAsyncTaskPS4EnumerateTitleFiles bWasSuccessful:%d"), WasSuccessful());
}
...
void FOnlineAsyncTaskPS4EnumerateTitleFiles::StartFetchingStatusData()
{
	...
	if (Result < 0)
	{
		UE_LOG_ONLINE_TITLEFILE(Error, TEXT("sceUserServiceGetInitialUser() failed when enumerating title files. Error code: 0x%08x"), Result);
		return;
	}
...
void FOnlineAsyncTaskPS4EnumerateTitleFiles::Tick()
{
	...
	if (CurrentPhase == PHASE_POLLING)
	{
		...
		if (Result == 0)
		{
			...
			else
			{
				...
				UE_LOG_ONLINE_TITLEFILE(Error, TEXT("sceNpTssGetDataAsync returned an error. Error code: 0x%08x"), OperationResult);
				bWasSuccessful = false;
				bIsComplete = true;
				return;
			}
		...
		{
			// There was an error.
			UE_LOG_ONLINE_TITLEFILE(Error, TEXT("sceNpTusPollAsync failed. Error code: 0x%08x"), Result);
	...
	if (CurrentPhase == PHASE_NEED_QUERY)
	{
		...
		if (RequestId >=0)
		{
			...
			if (Result != SCE_OK)
			{
				UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceNpTusDeleteRequest() failed. Error code: 0x%08x"), Result);
			}
		...
		if (Result < 0)
		{
			UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceNpTusCreateRequest() failed. Error code: 0x%08x"), Result);
			bWasSuccessful = false;
			bIsComplete = true;
...
{
	return FString::Printf(TEXT("FOnlineAsyncTaskPS4ReadFile bWasSuccessful:%d"), WasSuccessful());
}
...
void FOnlineAsyncTaskPS4ReadFile::RequestFileData(const FString& InFileName)
{
	...
	if (Result < 0)
	{
		UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceUserServiceGetInitialUser() failed. Error code: 0x%08x"), Result);
		return;
	}
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceNpTssGetDataAsync() failed. Error code: 0x%08x"), Result);
		bWasSuccessful = false;
		bIsComplete = true;
		return;  // isComplete(true), wasSuccessful(false)
	}
...
void FOnlineAsyncTaskPS4ReadFile::Tick()
{
	...
	if (Result == 0)
	{
		...
		if (OperationResult == SCE_OK)
		{
			...
			if (CurrentPhase == PHASE_QUERY)
			{
				...
				if (RequestId >= 0)
				{
					...
					if (Result != SCE_OK)
					{
						UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceNpTusDeleteRequest() failed in phase %d. Error code: 0x%08x"), CurrentPhase, Result);
					}
				...
				if (Result < 0)
				{
					UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceNpTusCreateRequest() failed in phase %d. Error code: 0x%08x"), CurrentPhase, Result);
					bWasSuccessful = false;
					bIsComplete = true;
					return;  // isComplete(true), wasSuccessful(false)
				}
				...
				if (Result != SCE_OK)
				{
					UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceNpTssGetDataAsync() failed in phase %d. Error code: 0x%08x"), CurrentPhase, Result);
					bWasSuccessful = false;
					bIsComplete = true;
					return;  // isComplete(true), wasSuccessful(false)
				}
		...
		{
			// There was an error. Stop enumerating and return a failure for everything
			UE_LOG_ONLINE_TITLEFILE(Error, TEXT("sceNpTusGetMultiSlotDataStatusAAsync returned an error in phase %d. Error code: 0x%08x"), CurrentPhase, OperationResult);
			bWasSuccessful = false;
			bIsComplete = true;
			return;
		}
	...
	{
		// There was an error.
		UE_LOG_ONLINE_TITLEFILE(Error, TEXT("sceNpTusPollAsync failed in phase %d. Error code: 0x%08x"), CurrentPhase, Result);
...
bool FOnlineTitleFilePS4::Init()
{
	...
	if ( Result != SCE_OK )
	{
		UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_NP_TUS) failed. Error code: 0x%08x"), Result);
		return false;
	}
...
void FOnlineTitleFilePS4::Shutdown()
{
	...
	if (sceSysmoduleIsLoaded(SCE_SYSMODULE_NP_TUS))
	{
		...
		if (Result != SCE_OK )
		{
			UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("sceSysmoduleUnloadModule(SCE_SYSMODULE_NP_TUS) failed. Error code: 0x%08x"), Result);
		}
	}
}
...
bool FOnlineTitleFilePS4::GetFileContents(const FString& FileName, TArray<uint8>& FileContents)
{
	if (FileSet.Contains(FileName))
	{
		...
		if (File.bDoesExist)
		{
			...
			{
				UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("TitleFile: GetFileContents() failed. File %s is configured and enumeratoed but has not been loaded"), *FileName);
			}
		}
		else
		{
			UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("TitleFile: GetFileContents() failed. File %s is configured but has not been enumerated"), *FileName);
		}
	}
	else
	{
		UE_LOG_ONLINE_TITLEFILE(Warning, TEXT("TitleFile:: GetFileContents() failed. File %s is not configured. Edit PS4Engine.ini to configure"), *FileName);
	}
