	...
	namespace Gnmx
	{
		...
		*/
		...
		/** @brief
			...
			@param[out] csvsShaderInfo	Receives the information for the CS-VS shader. This pointer must not be <c>NULL</c>.
			@param[out] csShaderInfo		Receives the information for the CS shader. This pointer must not be <c>NULL</c>.
			@param[in] data				A pointer to the contents of an <c>.sb</c> file in memory. This pointer must not be <c>NULL</c>.
		*/
