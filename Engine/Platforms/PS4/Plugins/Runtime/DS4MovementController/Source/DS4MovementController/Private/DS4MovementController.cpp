...
FDS4MovementController::~FDS4MovementController()
{
	...
	for (auto& UserState : UserStates)
	{
		...
		FSonyApplication::GetSonyApplication()->DisconnectControllerStateFromUser(UserState.UserID, UserState.ControllerId);
	}
}
...
{
	if (ControllerIndex < 0 || ControllerIndex >= UE_ARRAY_COUNT(UserStates))
	{
		return false;
	}
...
void FDS4MovementController::ConnectStateToUser(int32 UserID, int32 UserIndex)
{
	...
	UserState.Tracker = IPS4Tracker::INVALID_TRACKER_HANDLE;
	UserState.Pad = FSonyApplication::GetSonyApplication()->ConnectControllerStateToUser(UserID, UserIndex);
}
...
void FDS4MovementController::DisconnectStateFromUser(int32 UserID, int32 UserIndex)
{
	...
	FSonyApplication::GetSonyApplication()->DisconnectControllerStateFromUser(UserID, UserIndex);
	UserState.Pad = 0;
