...
bool FOnlineFriendsPS4::SendInvite(int32 LocalUserNum, const FUniqueNetId& FriendId, const FString& ListName, const FOnSendInviteComplete& Delegate /*= FOnSendInviteComplete()*/)
{
	...
	if (!LocalUserId.IsValid())
	{
		...
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineFriendsPS4::SendInvite - Local user not signed in"));
	...
	if (IsFriend(LocalUserNum, FriendId, ListName))
	{
		PS4Subsystem->ExecuteNextTick([LocalUserNum, PS4PersonToFriend, ListName, Delegate]()
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineFriendsPS4::SendInvite - %s is already a friend"), *PS4PersonToFriend->ToDebugString());
	...
	if (ReturnCode < 0)
	{
		...
		PS4Subsystem->ExecuteNextTick([LocalUserNum, PS4PersonToFriend, ListName, Delegate]()
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineFriendsPS4::SendInvite - Failed to init dialog"));
	...
	if (ReturnCode < 0)
	{
		...
		PS4Subsystem->ExecuteNextTick([LocalUserNum, PS4PersonToFriend, ListName, Delegate]()
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineFriendsPS4::SendInvite - Failed to open dialog"));
			Delegate.ExecuteIfBound(LocalUserNum, false, *PS4PersonToFriend, ListName, TEXT("Failed to open Send Friend Invite dialog"));
		});
		return false;
	}
...
bool FOnlineFriendsPS4::DeleteFriend(int32 LocalUserNum, const FUniqueNetId& FriendId, const FString& ListName)
{
	...
	if (!IsFriend(LocalUserNum, FriendId, ListName))
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineFriendsPS4::SendInvite - %s is already not a friend"), *PS4PersonToFriend->ToDebugString());
		return false;
	}
	...
	{
		// FAILURE
		UE_LOG_ONLINE(Error, TEXT("FOnlineFriendsPS4::SendInvite - Failed to open dialog; sceNpProfileDialogInitialize error = 0x%08x"), ReturnCode);
		sceNpProfileDialogTerminate();
		return false;
	}
	...
	{
		// FAILURE
		UE_LOG_ONLINE(Error, TEXT("FOnlineFriendsPS4::SendInvite - Failed to open dialog; sceNpProfileDialogOpenA error = 0x%08x"), ReturnCode);
		sceNpProfileDialogTerminate();
		return false;
	}
