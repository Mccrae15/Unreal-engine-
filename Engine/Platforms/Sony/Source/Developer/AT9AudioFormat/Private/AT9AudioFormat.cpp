...
public:
	...
	void RunConverter(const TCHAR* Params, int32* OutReturnCode, FString* OutStdOut, FString* OutStdErr) const
	{		
		static const TCHAR* ToolSubPath = TEXT("host_tools/bin/at9tool.exe");
		static const TCHAR* EnviromentVar = TEXT("SCE_ORBIS_SDK_DIR");
		
		// Default, try the environment variable first. AutoSDK should handle this normally on a build machine and locally it will be filled in
		FString ExecutablePath = FPlatformMisc::GetEnvironmentVariable(EnviromentVar);
		FString ToolPath = ExecutablePath / ToolSubPath;	
		if (ExecutablePath.IsEmpty() || !FPaths::FileExists(*ToolPath) || !FPlatformProcess::ExecProcess(*ToolPath, Params, OutReturnCode, OutStdOut, OutStdErr))
		{						
			// Fallback to using the $PATH (i.e. no prefix and let the OS find the .exe)
			UE_LOG(LogHAL, Log, TEXT("Failed to run '%s' (error code %i) using path in SDK EnviromentVariable '%s' which expands to '%s'. Falling back to see if its in $PATH..."),
				ToolSubPath,
				*OutReturnCode,
				EnviromentVar,
				*ExecutablePath);

			// Clean off any prefix (i.e. sub-paths) and see if its in our $PATH
			ToolPath = FPaths::GetCleanFilename(ToolSubPath);
			FPlatformProcess::ExecProcess(*ToolPath, Params, OutReturnCode, OutStdOut, OutStdErr);
		}		
	}
	...
	virtual bool Cook(FName AudioFormat, const TArray<uint8>& SrcBuffer, FSoundQualityInfo& QualityInfo, TArray<uint8>& CompressedDataStore) const
	{
		...
		if (SourceFileWriter)
		{
			...
			if (WriteWaveFile(*SourceFileWriter, PaddedBuffer.GetData(), FMath::Max(QualityInfo.SampleDataSize, MinBytes), &Format))
			{
				...
				// look first in the path, then check in SDK dir
				...
				FString StdOut;
				FString StdErr;
				RunConverter(*Commandline, &ReturnCode, &StdOut, &StdErr);

				if (ReturnCode != 0)
				{
					UE_LOG(LogHAL, Warning, TEXT("Failed to encode AT9 audio, err code = %d:\nCommand:at9tool %s\nStdOut: %s\nStdErr: %s"), ReturnCode, *Commandline, *StdOut, *StdErr);
					bSucceeded = false;
				}
				else
				{
...
*/
	...
	virtual bool SplitDataForStreaming(const TArray<uint8>& SrcBuffer, TArray<TArray<uint8>>& OutBuffers, const int32 InitialChunkMaxSize, const int32 MaxChunkSize) const override
	{
		...
		if (InitialChunkMaxSize != Header.DataStartOffset)
		{
			const int32 InitialChunkNumFrames = InitialChunkMaxSize / FrameSize;
			const uint32 InitialBytesPerChunk = InitialChunkNumFrames * FrameSize;

			// The first block will be the bytes per chunk plus the header information
			ChunkByteSize = InitialBytesPerChunk + Header.DataStartOffset;
		}
		else
		{
			// If the initial chunk size if equal to our header size, don't account for the header size.
			ChunkByteSize = InitialChunkMaxSize;
		}
