...
void FGnmUniformBuffer::Update(FRHICommandListImmediate* RHICmdList, const void* Contents, EUniformBufferValidation Validation)
{
	...
	if (ConstantBufferSize > 0)
	{
		...
		if ((Usage == UniformBuffer_SingleDraw || Usage == UniformBuffer_SingleFrame) && FGnmManager::SafeToAllocateFromTempRingBuffer())
		{
			...
			BufferPtr = GGnmManager.AllocateFromTempRingBuffer(ConstantBufferSize);
#if ENABLE_BUFFER_INTEGRITY_CHECK
	...
	if (NumResources)
	{
		...
		for (int32 i = 0; i < NumResources; ++i)
		{
			...
			{
				checkf(Resource, TEXT("Invalid resource entry creating uniform buffer, %s.Resources[%u], ResourceType 0x%x."), *Layout.GetDebugName().ToString(), i, (uint8)Layout.Resources[i].MemberType);
			}
#endif
			if (ResourceTableRef)
			{
...
void FGnmDynamicRHI::RHIUpdateUniformBuffer(FUniformBufferRHIParamRef UniformBufferRHI, const void* Contents)
{
