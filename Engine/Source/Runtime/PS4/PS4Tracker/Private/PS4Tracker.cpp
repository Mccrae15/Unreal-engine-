...
public:
	...
	virtual void SetPredictionTiming(int32 FlipToDisplayLatency, int32 RenderFrameTime, bool bIs60Render120ScanoutMode, int32 CameraProcessStartTime, int32 UpdateMotionSensorDataStartTime) override;
...
private:
	...
	int32 UpdateMotionSensorDataStartTime = -1;
	SceCameraFrameData CameraFrameData;
	uint64_t LastCameraCaptureTime = 0;
...
void FPS4Tracker::Initialize()
{
	...
	OnionTrackingRam = FMemBlock::Allocate(QueryMemoryResult.directMemoryOnionSize, QueryMemoryResult.directMemoryOnionAlignment, EGnmMemType::GnmMem_CPU, GET_STATID(STAT_Onion_TrackingRam));
...
//=============================================================================
void FPS4Tracker::SetPredictionTiming(int32 InFlipToDisplayLatency, int32 InRenderFrameTime, bool InbIs60Render120ScanoutMode, int32 InCameraProcessStartTime, int32 InUpdateMotionSensorDataStartTime)
{
	FScopeLock ScopeLock(&CritSect);
	...
	CameraProcessStartTime			= InCameraProcessStartTime;
	UpdateMotionSensorDataStartTime = InUpdateMotionSensorDataStartTime;
}
...
//=============================================================================
// The camera runs at 60fps, so calls to SubmitComputeWithCamera faster than that will fail and fall back
// to UpdateMotionSensorData.  Doing that fail and fall back takes about twice as much cpu time as just calling UpdatMotionSensorData,
// but still less than a successful SubmitComputeWithCamera.
// There are three output modes: 60 or less, 60/120, 90, and 120.
//
// To do all that the current scheme is that MorpheusHMD tells us when a new frame happens, we then follow parameters to decide what updates
// to do when.
void FPS4Tracker::CameraProcess()
{
	while (1)
	{
		...
		sceVrTrackerGetTime(&FrameStartTime);

		sceKernelUsleep(CameraProcessStartTime);
		...
		if (UpdateMotionSensorDataStartTime > 0)
		{
			uint64_t PostComputeTime = 0;
			sceVrTrackerGetTime(&PostComputeTime);
			check(PostComputeTime >= FrameStartTime);
			uint64_t CurrentFrameTime = PostComputeTime - FrameStartTime;
			if (CurrentFrameTime < UpdateMotionSensorDataStartTime)
			{
				sceKernelUsleep(UpdateMotionSensorDataStartTime - CurrentFrameTime);
			...
			{
				UE_LOG(LogPS4Tracker, Warning, TEXT("FPS4Tracker::CameraProcess() is set to run UpdateMotionSensorData at %ius, but frametime is already %i.  Skipping it so we have the best chance of catching up."), UpdateMotionSensorDataStartTime, CurrentFrameTime);
			}
		}

		// See if we should wait some, and try to sync to frame... or if we should just go.
		// If we stray from 60fps tracking will break (hmd will move oddly, ie diagonally when your head goes straight forward/back  - can induce this by stop in debugger, rotate hmd, continue from debugger)
		// So if we have frame time left we will try to wait for just a little longer than a 60fps frame for the next frame to start.
		// If we are way off our frame start we will creep back to on, if our frames are runnign too slow though we will go ahead and call very close to 60fps.
		uint64_t PostWorkTime = 0;
		sceVrTrackerGetTime(&PostWorkTime);
		if (ensure(PostWorkTime >= FrameStartTime))
		{
			uint64_t CurrentFrameTime = PostWorkTime - FrameStartTime;
			if (CurrentFrameTime < RenderFrameTime)
			{
				uint64_t RemainingFrameTime = RenderFrameTime - CurrentFrameTime;
				if (RemainingFrameTime > 500)
				{
					uint64_t MsToWait = (RemainingFrameTime + 500) / 1000;
					NewFrameEvent->Wait(MsToWait);
				}
				else
				{
					UE_LOG(LogPS4Tracker, Warning, TEXT("FPS4Tracker::CameraProcess() Just going            %i."), CurrentFrameTime);
				}
			}
		}
	};
