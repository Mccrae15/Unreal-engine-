...
void FAVPlayerMovieStreamer::CheckForNextVideoFrame()
{
	...
	if (FrameAvailable)
	{
		...
		if( LumaRHI.IsValid() && ChromaRHI.IsValid() )
		{
			...
			FRHITexture* BoundTextureTarget = BoundSlateTextureTarget->GetRHIRef();

			FRHIRenderPassInfo RPInfo(BoundTextureTarget, ERenderTargetActions::Load_Store);
			RHICmdList.BeginRenderPass(RPInfo, TEXT("PS4MovieStream"));
			{
				...
				PixelShader->SetParameters( RHICmdList, LumaRHI->GetTexture2D(), ChromaRHI->GetTexture2D(), MediaShaders::YuvToSrgbPs4, MediaShaders::YUVOffset8bits, true );
			...
			RHICmdList.CopyToResolveTarget(BoundTextureTarget, BoundTextureTarget, FResolveParams());
