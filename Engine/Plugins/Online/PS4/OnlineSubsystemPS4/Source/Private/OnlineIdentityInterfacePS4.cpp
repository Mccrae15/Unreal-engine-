	...
	void DoWork()
	{
		...
		if (Privilege == EUserPrivileges::CanPlayOnline)
		{
			...
			if (CheckNpResult < 0 && 
				PrivilegeFailures != 0)
			{
				UE_LOG_ONLINE(Warning, TEXT("sceNpCheckNpAvailabilityA Failed : 0x%x, UserID: 0x%x"), CheckNpResult, SceUserId);
			...
			if (PS4Subsystem->IsPSNPlusRequired())
			{
				...
				if (CheckPSPlusResult < 0 &&
					PrivilegeFailures != 0)
				{
					UE_LOG_ONLINE(Warning, TEXT("sceNpCheckPlus Failed : 0x%x, UserID: 0x%x"), CheckPSPlusResult, SceUserId);
					PrivilegeFailures |= (uint32)IOnlineIdentity::EPrivilegeResults::GenericFailure;
				}
			}
		}
		...
		if (Privilege == EUserPrivileges::CanCommunicateOnline || Privilege == EUserPrivileges::CanUseUserGeneratedContent)
		{
			...
			{
				UE_LOG_ONLINE(Warning, TEXT("sceNpGetParentalControlInfoA Failed : 0x%x, UserID: 0x%x"), CheckParentalRet, SceUserId);
				PrivilegeFailures |= (uint32)IOnlineIdentity::EPrivilegeResults::GenericFailure;
			}
			else
			{
...
bool FOnlineIdentityPS4::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	...
	TriggerOnLoginCompleteDelegates(LocalUserNum, false, FUniqueNetIdPS4::GetInvalidUser(), TEXT("com.epicgames.identity.notloggedin"));
	return false;
}
