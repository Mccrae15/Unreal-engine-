...
class FSonySaveGameSystem : public ISaveGameSystem
{
public:
	FSonySaveGameSystem();
	virtual ~FSonySaveGameSystem();
...
protected:
	virtual const TCHAR* GetSaveFileName() = 0;
	virtual int32 Mount(const SceSaveDataMountMode Mode, SceUserServiceUserId UserId, SceSaveDataDirName& DirName, SceSaveDataBlocks Blocks, SceSaveDataMountResult& MountResult) = 0;
	virtual int32 Unmount(SceSaveDataMountPoint* MountPoint, bool bCommit) = 0;
	virtual SceSaveDataDialogSystemMessageType GetNoSpaceMessageType() = 0;
