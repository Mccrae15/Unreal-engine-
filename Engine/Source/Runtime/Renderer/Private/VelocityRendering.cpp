...
{
	CommitGraphicsPipelineState(RHICmdList, *this, DrawRenderState, SceneView->GetFeatureLevel(), SceneView->bVRProjectEnabled || PolicyContext.bIsSinglePassStereo);
	...
	)
{
	...
	if(BlendMode == BLEND_Opaque || BlendMode == BLEND_Masked)
	{
		...
		{			
			...
			DrawingPolicy.SetSharedState(RHICmdList, &View, FVelocityDrawingPolicy::ContextDataType(bIsInstancedStereo, bIsSinglePassStereo), DrawRenderStateLocal);
