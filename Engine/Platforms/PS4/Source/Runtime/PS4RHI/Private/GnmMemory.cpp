...
void FMemBlock::Dump()
{
	...
	for (int32 Type = 0; Type < (int32)EGnmMemType::GnmMem_MaxNumTypes; Type++)
	{
		...
		for (TMap<FName, uint64>::TIterator It(TaggedMemorySizes[Type]); It; ++It)
		{
			...
			UE_LOG(LogSony, Log, TEXT("[%s] Tag %s is using %.2fMB"), MemoryDescriptions[Type], *It.Key().ToString(), (double)It.Value() / (1024.0 * 1024.0));
			Total += It.Value();
		}

		UE_LOG(LogSony, Log, TEXT("[%s] --TOTAL-- %.2fMB [%.2fMB in pending deletes]\n"), MemoryDescriptions[Type],
