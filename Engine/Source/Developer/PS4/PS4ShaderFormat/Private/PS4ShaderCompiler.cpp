...
static void CompileUsingInternal(bool bSecondPass, FCompilerInfo& CompilerInfo, FString& PreprocessedShaderSource, FString& EntryPointName, FShaderCompilerOutput& Output)
{
	...
	CompilerInfo.WaveOptions.suppressedWarningsCount = ARRAY_COUNT(SuppressedWarnings);
	...
	if (RunPsslc.Results)
	{
		...
		for (int32 MessageIndex = 0; MessageIndex < RunPsslc.Results->diagnosticCount; MessageIndex++)
		{
			...
			if (Message.level == Wave::Psslc::kDiagnosticLevelWarning || Message.level == Wave::Psslc::kDiagnosticLevelError)
			{
				...
				else // Message.level == Wave::Psslc::kDiagnosticLevelWarning
				{
					if (Message.code == 20087)
					{
						check(CompilerInfo.WaveOptions.locale != Wave::Psslc::kLocaleEnglish || FString(ANSI_TO_TCHAR(Message.message)).Contains(TEXT("unreferenced formal parameter")));
						continue;
					}
					else if (Message.code == 20088)
					{
						check(CompilerInfo.WaveOptions.locale != Wave::Psslc::kLocaleEnglish || FString(ANSI_TO_TCHAR(Message.message)).Contains(TEXT("unreferenced local variable")));
						continue;
					}
				}
