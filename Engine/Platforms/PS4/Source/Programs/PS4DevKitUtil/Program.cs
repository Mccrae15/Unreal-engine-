...
namespace PS4DevKitTool
{
	...
	class Program
	{
		...
		{ 
			...
			{
				throw new Exception(string.Format("Failed to add target {0}. {1}", Hostname, Ex.Message));
			}
		...
		static int RemoveDevice(ORTMAPI TM, List<string> ArgList, bool removeAll)
		{
			...
			else
			{
				...
				if (Target != null)
				{
					...
					{
						throw new Exception(string.Format("Failed to delete target {0}. {1}", Hostname, Ex.Message));
					}
				}
				else
				{
		...
		static int SetSetting(ORTMAPI TM, string[] args)
		{
			string TargetString = ParseParamValue(args, "Target");
			ITarget Target = null;
			...
			string SettingName = ParseParamValue(args, "setting");
			string SettingValue = ParseParamValue(args, "value");

			// if we set the settings then reboot so they take effect
			if (PS4DevKitUtil.SetSettings.SetTargetSetting(Target, SettingName, SettingValue) )
			{
				Target.Reboot();
			}
			return 0;
			...
			using (FTargetConnection Connection = new FTargetConnection(Target))
			{
				try
				{
					...
					if (TargetHandler.Result != FLaunchTargetHandler.eProcessResult.Success)
					{
						...
						Console.WriteLine("Waiting 20 secs for devkit to finish writing dump...", ReturnCode, TargetHandler.ResultString);

						Thread.Sleep(20 * 1000);

						Console.WriteLine("Attempting to postmortem most recent crash on {0}", Target.Name);

						if (PS4DevKitUtil.Postmortem.TryPostmortemOnDevkit(Console.Out, TM, Target, 120) == false)
						{
		...
		static int SendInput( ORTMAPI TM, string[] args )
		{
			string TargetString = ParseParamValue( args, "Target" );
			ITarget Target = null;
			...
			if (ArgsStartIndex != -1)
			{
				...
				if( ArgsEndIndex != -1 )
				{
					InputString = CommandLine.Substring( ArgsStartIndex, ArgsEndIndex - ArgsStartIndex - 1 );
				}
				else
				{
					// If not everything after -args= goes to stdin
					InputString = CommandLine.Substring( ArgsStartIndex );
				}
			}
			...
			if (Target.PowerStatus == ePowerStatus.POWER_STATUS_ON)
			{
				try
				{
					using (FTargetConnection Connection = new FTargetConnection( Target ))
					{
						...
						foreach (IProcessInfo ProcessInfo in ProcessInfos)
						{
							const eProcessAttr CheckFlags = (eProcessAttr.PROCESS_ATTR_APP_STATE_MASK | eProcessAttr.PROCESS_ATTR_SYSTEM | eProcessAttr.PROCESS_ATTR_SUSPENDED);
							const eProcessAttr DesiredFlags = (eProcessAttr.PROCESS_ATTR_APP_STATE_RUNNING);

							if ((ProcessInfo.Attributes & CheckFlags) == DesiredFlags)
							{
								ProcessInfo.ProcessInterface.SendConsoleInput( eConsoleOutputPort.STREAM_STDIO, InputString );
								Console.WriteLine( "Sent input line to {0} on {1}", ProcessInfo.Name, TargetString );
								bHasSent = true;
							}
						}
					}
			...
			if (!bHasSent)
			{
				Console.Error.WriteLine( "Cannot find running process on devkit {0}", TargetString );
				return -1;
			}
		...
		static int ShowHelp()
		{
			...
			string[] Functions = new string[] {"list", "detail", "add", "remove", "removeall", "snapshot","launch",
				"connect", "disconnect", "poweron", "poweroff", "reboot","suspend","monitor", "postmortem", "sendinput", "setsetting" };
			...
			Options["sendinput"].Add( "sendinput <string>: Send input to the current foreground process on the specified target" );
			Options["setsetting"].Add("setsetting -target=<name> -setting=<settingname> -value=<newsettingvalue>: Sets a setting on the devkit, will not set the setting if it's already set");
		...
		static int Main(string[] args)
		{
			...
			try
			{
				if (ArgList.Count > 0)
				{
					...
					else if (String.Compare( ArgList[0], "sendinput", StringComparison.InvariantCultureIgnoreCase ) == 0)
					{
						ReturnCode = SendInput( TM, args );
					}
					else if (String.Compare(ArgList[0], "setsetting", StringComparison.InvariantCultureIgnoreCase) == 0)
					{
						ReturnCode = SetSetting(TM, args);
					}
					else
					{
						throw new ApplicationException("No command specified. Valid commands are list, detail, snapshot, launch, monitor, portmortem, sendinput. Use help for help");
					}
				}
				else
				{
			...
			{
				Console.WriteLine("PS4DevKitUtil {0} {1}", Ex.Message, Ex.StackTrace);
				ReturnCode = -1;
			}
