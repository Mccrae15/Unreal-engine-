...
const int32 DefaultSampleRate = 48000; // docs say to always use 48000
...
{	
	...
	PortID = sceAudioOutOpen(UserIndex, OutputDestination, 0, GrainSize, DefaultSampleRate, ChannelsParam);
...
FAudioPlatformSettings FNgs2Device::GetPlatformSettings() const
{
...
FNgs2Pathway* FNgs2Device::GetPathway(ENgs2Pathway::Type Pathway, int32 UserIndex)
{
	...
	if (Pathways[PathwayIndex] == NULL)
	{
		switch (Pathway)
		{
			...
			case ENgs2Pathway::Controller:
				{
					// SonyApplication has the master mapping from UserIndex -> UserID.  You can't get it from the SceUserServiceLoginUserIdList
					// because it gets remapped behind the scenes as users log in/out.
					verify(UserIndex < SCE_USER_SERVICE_MAX_LOGIN_USERS);
					const FSonyApplication* SonyApplication = FSonyApplication::GetSonyApplication();
					int32 UserID = SonyApplication->GetUserID(UserIndex);
