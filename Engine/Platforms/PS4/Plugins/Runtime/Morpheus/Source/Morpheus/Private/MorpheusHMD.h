...
public:	
...
#if PLATFORM_PS4
	class FMorpheusBridge : public FXRRenderBridge
	{
	public:
		...
		// Implementation of FXRRenderBridge
	...
	static const MORPHEUS_API FName MorpheusSystemName;
...
public:
	...
	void MORPHEUS_API Show2DVRSplashScreen(class UTexture* Texture, FVector2D Scale, FVector2D Offset);
	void MORPHEUS_API Show2DVRSplashScreen(FTexture2DRHIRef Texture, FVector2D Scale, FVector2D Offset);
	void MORPHEUS_API Hide2DVRSplashScreen();

	/** Morpheus sceHmdReprojectionSetOutputMinColor */
	void MORPHEUS_API HMDReprojectionSetOutputMinColor(FLinearColor MinColor);
	...
	virtual void CreateHMDPostProcessPass_RenderThread(class FRDGBuilder& GraphBuilder, const class FViewInfo& View, const struct FHMDDistortionInputs& Inputs, struct FScreenPassTexture& OutPass) const override;
#endif
...
private:
	...
	/** If nothing is bound to OnVRHeadsetRecenter this delegate is fired and we recenter, to comply with trc4181 by default.  */
	FDelegateHandle OnVRHeadsetRecenterDefaultDelegateHandle;
	void OnVRHeadsetRecenterDefaultDelegateReceived();
