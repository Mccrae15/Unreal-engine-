	...
	: public FGenericPlatformMemory
{
	enum EMemoryCounterRegion
	{
		...
		/** PS4 specific vr tracker buffer pool. Memory in this pool is both physically and virtually contiguous. */
		MCR_OnionDirect,
	...
	static FORCEINLINE void* Memcpy(void* RESTRICT Dest, const void* RESTRICT Src, SIZE_T Count)
	{
		if(Count <= 64)
		{
			...
			if (Count >= 16)
			{
				while (Count >= 32)
				{
					FGenericPlatformMemory::WriteUnaligned<uint64>(DestAddr +  0, FGenericPlatformMemory::ReadUnaligned<uint64>(SrcAddr + 0));
					FGenericPlatformMemory::WriteUnaligned<uint64>(DestAddr +  8, FGenericPlatformMemory::ReadUnaligned<uint64>(SrcAddr + 8));
					FGenericPlatformMemory::WriteUnaligned<uint64>(DestAddr + 16, FGenericPlatformMemory::ReadUnaligned<uint64>(SrcAddr + 16));
					FGenericPlatformMemory::WriteUnaligned<uint64>(DestAddr + 24, FGenericPlatformMemory::ReadUnaligned<uint64>(SrcAddr + 24));
					Count -= 32;
					DestAddr += 32;
					SrcAddr += 32;
				}
			...
			while (Count >= 4)
			{
				FGenericPlatformMemory::WriteUnaligned<uint32>(DestAddr, FGenericPlatformMemory::ReadUnaligned<uint32>(SrcAddr));
				Count -= 4;
				DestAddr += 4;
				SrcAddr += 4;
			}
