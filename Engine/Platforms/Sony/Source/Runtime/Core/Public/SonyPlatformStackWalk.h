...
#ifndef SONY_ENABLE_ASLR_STACKWALK
#define SONY_ENABLE_ASLR_STACKWALK 0
#endif

struct CORE_API FSonyPlatformStackWalk : public FGenericPlatformStackWalk
{
	/**
	...
	static void LoadSymbolInfo(const FString& RootDirectory, const FString& SandboxName);

	/** Info about each symbol (Address, Size, NameOffset) */
	struct FSonySymbolInfo
	{
		uint32	Address;
		uint32	Size;
		uint32	NameOffset;
	...
	static TArray<FSonySymbolInfo> SymbolInfo;
	...
	struct FSonySegmentInfo
	{
		uintptr_t	Address;
		size_t		Size;
	};

	static TArray<FSonySegmentInfo> SelfSegments;
#endif
};


typedef FSonyPlatformStackWalk FPlatformStackWalk;
