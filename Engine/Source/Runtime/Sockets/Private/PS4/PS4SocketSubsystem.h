...
public:
	...
	FPS4Socket(SOCKET InSocket, ESocketType InSocketType, const FString& InSocketDescription, ESocketProtocolFamily InProtocolType, const bool bInIsP2P, ISocketSubsystem * InSubsystem)
	...
	{
		in_addr BSDAddrData;
		const FInternetAddrPS4& PS4Addr	= static_cast<const FInternetAddrPS4&>(Addr);
		PS4Addr.GetIp(BSDAddrData);

		SceNetSockaddrIn P2PAddr = { 0 };

		P2PAddr.sin_len			= sizeof( P2PAddr );
		// PS4 doesn't have official support for ipv6
		P2PAddr.sin_family		= AF_INET;

		P2PAddr.sin_addr.s_addr	= BSDAddrData.s_addr;
		P2PAddr.sin_port		= PS4Addr.GetPlatformPortNetworkOrder();
		P2PAddr.sin_vport		= htons(PS4Addr.GetRawPort());
		return P2PAddr;
	}
	...
	void ConvertP2PToBSD( const SceNetSockaddrIn & Source, FInternetAddr & Dest )
	{
		...
		sockaddr_storage NewAddrData;
		FMemory::Memzero(NewAddrData);
		sockaddr_in& NewAddrDataIPv4 = (sockaddr_in&)NewAddrData;

		NewAddrDataIPv4.sin_len = sizeof(sockaddr_in);
		NewAddrDataIPv4.sin_family = Source.sin_family;
		NewAddrDataIPv4.sin_addr.s_addr = Source.sin_addr.s_addr;
		NewAddrDataIPv4.sin_port		= Source.sin_vport;

		DestPR.Set(NewAddrData);
