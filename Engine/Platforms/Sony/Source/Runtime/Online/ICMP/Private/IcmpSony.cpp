...
namespace IcmpSony
{
	// 56 bytes is the default size in the Sony sample
...
FIcmpEchoResult IcmpEchoImpl(ISocketSubsystem* SocketSub, const FString& TargetAddress, float Timeout)
{
	...
	static const SIZE_T PacketSize = IcmpHeaderSize + IcmpSony::IcmpPayloadSize;
	...
	if (sceNetInetPton(SCE_NET_AF_INET, TCHAR_TO_ANSI(*ResolvedAddress), &DstAddress.sin_addr) >= 0)
	{
		...
		FMemory::Memcpy(Payload, IcmpSony::IcmpPayload, IcmpSony::IcmpPayloadSize);
		...
		{
			FScopeLock PingLock(&IcmpSony::gPingCS);
