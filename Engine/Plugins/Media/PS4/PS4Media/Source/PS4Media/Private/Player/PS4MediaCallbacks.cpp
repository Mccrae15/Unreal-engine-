...
FPS4MediaCallbacks::FPS4MediaCallbacks(const TSharedRef<FMediaSamples, ESPMode::ThreadSafe>& InSamples)
	...
	, bVideoDecoderPausedForLoop(false)
{ }
...
void FPS4MediaCallbacks::TickVideo(float Rate, FTimespan Time)
{
	...
	while (true)
	{
		...
		if (LastVideoSampleTime != FTimespan::MinValue())
		{
			FTimespan SampleTime = FTimespan(FrameInfo.timeStamp * ETimespan::TicksPerMillisecond);
			if ((Rate > 0.0f) && (SampleTime < LastVideoSampleTime))
			{
				bVideoDecoderPausedForLoop = true;
			}
			else if ((Rate < 0.0f) && (SampleTime > LastVideoSampleTime))
			{
				bVideoDecoderPausedForLoop = true;
			}
		}
