...
public:
	...
	uint64 SubmitSubmissionTimestampsForFrame(uint32 FrameNumber, const TArray<uint64*>& BeginSubmissionTimestamps, const TArray<uint64*>& EndSubmissionTimestamps );
...
public:	
	...
	const FGnmAuxBuffer& GetSocialScreenAuxBuffer() const;
	 ...
	 */
	static Gnm::DataFormat GetDataFormat(EPixelFormat Format, EGnmSrgbMode SrgbMode=EGnmSrgbMode::Default);
	...
	*/
	...
	void* AllocateFromTempRingBuffer(uint32 Size, uint32 Alignment, ETempRingBufferAllocateMode Mode = ETempRingBufferAllocateMode::NoFailure);
...
private:
	...
	*/
	...
	volatile uint64_t* ReprojectionLabels[2];
...
#if HAS_MORPHEUS
	FGnmAuxBuffer AuxBuffers[NUM_AUX_RENDER_BUFFERS];
...
#endif
	void FreePendingMemBlocks(bool bImmediate);
