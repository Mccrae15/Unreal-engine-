...
public:
		...
		, NumPlayersSuccessfullyLookedUp(0)
	{
		...
		{
			if (FNpWebApiThrottle::GetRateLimitStatus(ENpApiGroup::UserProfile) == EOnlinePSNRateLimitStatus::RateLimited)
			{
				if (!ErrorStr.IsEmpty())
				{
					ErrorStr += TEXT("; ");
				}
				ErrorStr += FString::Printf(TEXT("rate limit exceeded for UserProfile"));
				UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo: Attempted request for ENpApiGroup::UserProfile, but was RateLimited"));
				continue;
			}
			...
			if (Ret < SCE_TOOLKIT_NP_V2_SUCCESS)
			{
				...
				UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo: %d/%d: getNpProfiles failed: 0x%x"), RequestIndex, NumRequests, Ret);

				if (Ret == SCE_TOOLKIT_NP_V2_ERROR_INCORRECT_ARGUMENTS)
				{
					// Unexpected, print the values we set
					UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo: Incorrect arguments: begin"));
					UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo:  userId=%d"), Request.userId);
					UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo:  serviceLabel=%d"), Request.serviceLabel);
					UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo:  async=%d"), Request.async);
					UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo:  numValidAccountIds=%d"), Request.numValidAccountIds);
					for (int32 Index = 0; Index < Request.numValidAccountIds; ++Index)
					{
						UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo:  accountIds[%d]=%s"), Index, *PS4AccountIdToString(Request.accountIds[Index]));
					}
					UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo: Incorrect arguments: end"));
				}
			}
		}
	}
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4QueryUserInfo bWasSuccessful: %d RequestedUserIds.Num: %d NumPlayersSuccessfullyLookedUp: %d ErrorStr: %s"), WasSuccessful(), RequestedUserIds.Num(), NumPlayersSuccessfullyLookedUp, *ErrorStr);
	}
	...
	virtual void Finalize() override
	{
		...
		for (int32 ResponseIndex = 0, AbsUserIndex = 0; ResponseIndex < Responses.Num(); ++ResponseIndex, AbsUserIndex += MaxUsersPerRequest)
		{
			...
			if (ServerError)
			{
				PrintNPToolkitServerError(FString::Printf(TEXT("FOnlineAsyncTaskPS4QueryUserInfo: %d/%d"), ResponseIndex, Responses.Num()), *ServerError);
				...
				if (ServerError->httpStatusCode == EHttpResponseCodes::TooManyRequests)
				{
					...
					FNpWebApiThrottle::SetNextWebRequestTime(ENpApiGroup::UserProfile, nextTime);
				}
 			}
			...
			if (ProfilesList->numNpProfiles != UsersThisResponse)
			{
				...
				UE_LOG_ONLINE_USER(Warning, TEXT("FOnlineAsyncTaskPS4QueryUserInfo: Request %d/%d: Requested %i UserIds, got %i"), UsersThisResponse, ProfilesList->numNpProfiles);
			}
...
bool FOnlineUserPS4::QueryUserInfo(int32 LocalUserNum, const TArray<TSharedRef<const FUniqueNetId>>& UserIds)
{
	...
	if (!bStarted)
	{
		UE_LOG_ONLINE_USER(Warning, TEXT("QueryUserInfo request failed. %s"), *ErrorStr);
...
bool FOnlineUserPS4::QueryUserIdMapping(const FUniqueNetId& UserId, const FString& DisplayName, const FOnQueryUserMappingComplete& CompletionDelegate)
{
	if (DisplayName.IsEmpty())
	{
		...
		UE_LOG_ONLINE_USER(Warning, TEXT("QueryUserIdMapping request failed. %s"), *ErrorStr);
		CompletionDelegate.ExecuteIfBound(false, UserId, DisplayName, *FUniqueNetIdPS4::EmptyId(), ErrorStr);
	...
	else
	{
		...
		ResolveDelegate.AddLambda([=](FOnlineIdMap const& InResolvedIds, bool bInWasSuccessful, FString const& InErrorString)
		{
			...
			TSharedRef<const FUniqueNetId> FoundUserId = FUniqueNetIdPS4::EmptyId();
