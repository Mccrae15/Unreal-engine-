...
void UPermissionsUserWidget::NativeConstruct()
{
	...
	HasFineLocationPermission = HasPermission(TEXT("android.permission.ACCESS_FINE_LOCATION"));
	HasReadContactsPermission = HasPermission(TEXT("android.permission.READ_CONTACTS"));
...
void UPermissionsUserWidget::RequestPermission(const FString& PermissionStr)
{
	...
	if (!UAndroidPermissionFunctionLibrary::CheckPermission(PermissionStr))
	{
		...
		if (UAndroidPermissionCallbackProxy* Callback = UAndroidPermissionFunctionLibrary::AcquirePermissions(PermsToEnable))
		{
			...
			Callback->OnPermissionsGrantedDelegate.BindLambda([this, PermissionStr](const TArray<FString>& Permissions, const TArray<bool>& GrantResults)
			{
				...
				else if (TEXT("android.permission.ACCESS_FINE_LOCATION") == PermissionStr)
				{
					HasFineLocationPermission = HasBeenGranted;
				}
				else if (TEXT("android.permission.READ_CONTACTS") == PermissionStr)
				{
					HasReadContactsPermission = HasBeenGranted;
				}
...
void UPermissionsUserWidget::RequestFineLocationPermission()
{
	RequestPermission(TEXT("android.permission.ACCESS_FINE_LOCATION"));
}

void UPermissionsUserWidget::RequestReadContactsPermission()
{
	RequestPermission(TEXT("android.permission.READ_CONTACTS"));
}
