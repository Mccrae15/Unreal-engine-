...
public:
	...
	void* Allocate(int32 Size, int32 Alignment)
	{
		...
		uint8* AllocMem = Align(CurrentPtr, Alignment);
		int32 AlignSize = int32(AllocMem - CurrentPtr);

		if (CurrentPtr==nullptr || Remaining < (Size + AlignSize))
		{
			...
			AllocMem = Align(CurrentPtr, Alignment);
			AlignSize = int32(AllocMem - CurrentPtr);
			check((Size + AlignSize) <= TBlockSizeBytes);
		}
	...
	void* AllocateChunk()
	{
		return Allocate(TBlockSizeBytes, 16);
	}
