...
FString CreateWebApiSessionForTesting(FOnlineSubsystemPS4* PS4Subsystem)
{
	...
	if (HttpStatusCode == 200)
	{
		...
		UE_LOG_ONLINE_EXTERNALUI(Log, TEXT("Create session response body:\n%s"), UTF8_TO_TCHAR(ResponseData.GetData()));
...
// FOnlineAsyncTaskPS4TrackProfileUIStatus

void FOnlineAsyncTaskPS4TrackProfileUIStatus::Tick()
{
...
void FOnlineAsyncTaskPS4TrackProfileUIStatus::Finalize()
{
	sceNpProfileDialogTerminate();
}

void FOnlineAsyncTaskPS4TrackProfileUIStatus::TriggerDelegates()
{
	...
	IOnlineExternalUIPtr ExternalUIPtr = Subsystem->GetExternalUIInterface();
	if (ExternalUIPtr.IsValid())
	{
		ExternalUIPtr->TriggerOnExternalUIChangeDelegates(false);
	}
}
...
public:
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4UpdateWebBrowserDialog bWasSuccessful: %d"), WasSuccessful());
	}
...
public:
	...
	FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4UpdateInviteUI bWasSuccessful: %d"), WasSuccessful());
	}
...
static int32 GetSonyStoreIconPosition()
{
	...
	if (bStorePositionSet)
	{
		...
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("Unexpected PlayStation Store Icon position of %s, defaulting to Left"), *StoreIconString);
	}
	...
	, StoreIconPosition(GetSonyStoreIconPosition())
{
	...
	if (ReturnCode < 0)
	{
		UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceCommonDialogInitialize error = 0x%08x"), ReturnCode);
	}
}
...
bool FOnlineExternalUIPS4::ShowLoginUI(const int ControllerIndex, bool bShowOnlineOnly, bool bShowSkipButton, const FOnLoginUIClosedDelegate& Delegate)
{
	...
	if (!UserId.IsValid())
	{
		FOnlineError Error(EOnlineErrorResult::InvalidUser);
		Delegate.ExecuteIfBound(UserId, ControllerIndex, Error);
		return false;
	}
	...
	if (ReturnCode < SCE_OK)
	{
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceErrorDialogInitialize() failed with error code 0x%x"), ReturnCode);

		FOnlineError Error(ReturnCode);
		Delegate.ExecuteIfBound(UserId, ControllerIndex, Error);
		return false;
	}
	...
	TSharedPtr<const FUniqueNetIdPS4> UserIdPS4 = FUniqueNetIdPS4::Cast(UserId);
	if (!UserIdPS4->IsValid())
	{
		FOnlineError Error(EOnlineErrorResult::InvalidUser);
		Delegate.ExecuteIfBound(UserId, ControllerIndex, Error);
		return false;
	...
	Param.userId = UserIdPS4->GetUserId();
	Param.errorCode = SCE_NP_ERROR_SIGNED_OUT;

	ReturnCode = sceErrorDialogOpen(&Param);
	if (ReturnCode < SCE_OK)
	{
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceUserServiceGetInitialUser() failed with error code 0x%x"), ReturnCode);
...
bool FOnlineExternalUIPS4::ShowFriendsUI(int32 LocalUserNum)
{
	...
	UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("ShowFriendsUI not supported on PS4"));
	return false;
}
...
bool FOnlineExternalUIPS4::ShowInviteUI(int32 LocalUserNum, FName SessionName)
{
	...
	{
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("ShowInviteUI failed because there is no session to invite to."));
		return false;
	}
#endif
	...
	if (ReturnCode < 0)
	{
		UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceInvitationDialogInitialize error = 0x%08x"), ReturnCode);
	...
	else
	{
		...
		{
			UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("ShowInviteUI failed. No local user id for local user %d"), LocalUserNum);
			bShowedInviteUI = false;
		}
		else
		{
			...
			if (ReturnCode < 0)
			{
				UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceInvitationDialogOpenA error = 0x%08x"), ReturnCode);
				bShowedInviteUI = false;
			}
			else
			{
...
{
	UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("ShowAchievementsUI not supported on PS4"));
	return false;
}

bool FOnlineExternalUIPS4::ShowLeaderboardUI( const FString& LeaderboardName )
{
	UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("ShowLeaderboardUI not supported on PS4"));
	return false;
}
...
bool FOnlineExternalUIPS4::ShowWebURL(const FString& Url, const FShowWebUrlParams& ShowParams, const FOnShowWebUrlClosedDelegate& Delegate)
{
	...
	UE_LOG_ONLINE_EXTERNALUI(Log, TEXT("ShowWebURL: %s"), *Url);

	if (Url.Len() > SCE_WEB_BROWSER_DIALOG_URL_SIZE_EXTENDED)
	{
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("ShowWebURL: Url length is too long length=%d url=%s"), Url.Len(), *Url);
	}
	...
	if (ReturnCode < 0)
	{
		UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceWebBrowserDialogInitialize error = 0x%08x"), ReturnCode);
	}
	else
	{
		...
		if (ReturnCode < 0)
		{
			UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceUserServiceGetInitialUser error = 0x%08x"), ReturnCode);
			bShowedWebURL = false;
		}
		else
		{
			if (ShowParams.AllowedDomains.Num() > 0)
			{
				...
				{
					UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("ShowWebURL Passing in a greater number of AllowedDomains(%d) than supported(%d)"), ShowParams.AllowedDomains.Num(), SCE_WEB_BROWSER_DIALOG_DOMAIN_COUNT);
				}
			...
			if (ReturnCode < 0)
			{
				UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceWebBrowserDialogOpen error = 0x%08x"), ReturnCode);
				bShowedWebURL = false;
			}
			else
			{
...
bool FOnlineExternalUIPS4::CloseWebURL()
{
	...
	if (Ret != SCE_OK)
	{
		UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceWebBrowserDialogClose error = 0x%08x"), Ret);
		return false;
	}
	return true;
}
...
bool FOnlineExternalUIPS4::ShowProfileUI(const FUniqueNetId& Requestor, const FUniqueNetId& Requestee, const FOnProfileUIClosedDelegate& Delegate)
{
	...
	if (ReturnCode < 0)
	{
		UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceNpProfileDialogInitialize error = 0x%08x"), ReturnCode);
	...
	else
	{
		...
		if (ReturnCode < 0)
		{
			UE_LOG_ONLINE_EXTERNALUI(Error, TEXT("sceNpProfileDialogOpenA error = 0x%08x"), ReturnCode);
			bShowedProfileUI = false;
		}
		else
		{
	...
	if (bShowedProfileUI)
	{
		...
		FOnlineAsyncTaskPS4TrackProfileUIStatus* NewShowProfileUITask = new FOnlineAsyncTaskPS4TrackProfileUIStatus(*PS4Subsystem, Delegate);
		PS4Subsystem->GetAsyncTaskManager()->AddToInQueue(NewShowProfileUITask);
	}
	else
	{
		// Clean up
...
bool FOnlineExternalUIPS4::ShowAccountUpgradeUI(const FUniqueNetId& UniqueId)
{
	...
	if (LocalPlatformId != SCE_USER_SERVICE_USER_ID_INVALID)
	{
		...
		if (Ret != SCE_OK)
		{
			UE_LOG_ONLINE_EXTERNALUI(Log, TEXT("sceNpCommerceDialogInitialize failed with error: 0x%x"), Ret);
			return false;
		}
		...
		if (Ret != SCE_OK)
		{
			UE_LOG_ONLINE_EXTERNALUI(Log, TEXT("sceNpCommerceDialogOpen failed with error: 0x%x"), Ret);
			return false;
		}
...
public:
	...
	void DoWork()
	{
		...
		if (Ret != SCE_OK)
		{
			UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceMsgDialogInitialize failed with error: 0x%x"), Ret);
			return;
		}
		...
		case EPlatformMessageType::UGCRestricted:
			...
			break;
		default:
			UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("Unknown SceMsgDialogParam message type"));
			return;
		}		
		...
		if (Ret != SCE_OK)
		{
			UE_LOG_ONLINE_EXTERNALUI(Log, TEXT("sceMsgDialogOpen failed with error: 0x%x"), Ret);
			return;
		}
...
void FOnlineExternalUIPS4::Tick(float DeltaTime)
{
	...
	if (SignInDialogStatus == SCE_ERROR_DIALOG_STATUS_FINISHED)
	{
		...
		OnLoginUIClosedDelegate.ExecuteIfBound(UserId, LoginUIControllerIndex, FOnlineError::Success());
...
public:
	...
	FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4StoreDialog bWasSuccessful: %d"), WasSuccessful());
	}
	...
	virtual void Tick() override
	{
		...
		if (Status == SCE_COMMON_DIALOG_STATUS_FINISHED || Status == SCE_COMMON_DIALOG_STATUS_NONE)
		{
			...
			if (Ret < 0)
			{
				UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceNpCommerceDialogGetResult failed with error: 0x%x"), Ret);
			}
			else
			{
...
bool FOnlineExternalUIPS4::ShowStoreUI(int32 LocalUserNum, const FShowStoreParams& ShowParams, const FOnShowStoreUIClosedDelegate& Delegate)
{
	...
	if (Ret != SCE_OK)
	{
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceNpCommerceDialogInitialize failed with error: 0x%x"), Ret);
	}
	else
	{
		...
		if (Ret != SCE_OK)
		{
			UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceNpCommerceDialogOpen failed with error: 0x%x"), Ret);
		}
		else
		{
			bStarted = true;
...
public:
	...
	FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4SendMessageDialog bWasSuccessful: %d"), WasSuccessful());
	}
	...
	virtual void Tick() override
	{
		...
		if (Status == SCE_COMMON_DIALOG_STATUS_FINISHED || Status == SCE_COMMON_DIALOG_STATUS_NONE)
		{
			...
			if (Ret < 0)
			{
				UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceGameCustomDataDialogGetResultA failed with error: 0x%x"), Ret);
			}
			else
			{
...
bool FOnlineExternalUIPS4::ShowSendMessageUI(int32 LocalUserNum, const FShowSendMessageParams& ShowParams, const FOnShowSendMessageUIClosedDelegate& Delegate)
{
	...
	if (Ret != SCE_OK)
	{
		UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceGameCustomDataDialogInitialize failed with error: 0x%x"), Ret);
	}
	else
	{
		...
		if (Ret != SCE_OK)
		{
			UE_LOG_ONLINE_EXTERNALUI(Warning, TEXT("sceGameCustomDataDialogOpenA failed with error: 0x%x"), Ret);
		}
		else
		{
			bStarted = true;
