...
public class PS4Platform : Platform
{
	...
	private static void RecurseStagingDirectory(string LocalDirectory, string PS4Directory, int Depth, List<ChunkLanguageEntry> ChunkLanguageEntries, PerTitlePackageParameters TitleParams, StringBuilder FilesContents, StringBuilder DirectoriesContents, UnrealTargetConfiguration TargetConfiguration, bool bPlayGoEmulation, bool bCompressPakFiles, string TargetExecutable, bool bGeneratingPatch, bool bGeneratingRemaster, out int NumChunks)
	{
		NumChunks = 1;
		...
		foreach (string FullDir in Dirs)
		{
			...
			int RecurseChunks;
			RecurseStagingDirectory(LocalDirectory + Dir + "\\", PS4Directory + Dir + "/", Depth + 1, ChunkLanguageEntries, TitleParams, FilesContents, DirectoriesContents, TargetConfiguration, bPlayGoEmulation, bCompressPakFiles, TargetExecutable, bGeneratingPatch, bGeneratingRemaster, out RecurseChunks);
	...
	private static void BuildChunkContents(StringBuilder ChunkContents, List<ChunkLanguageEntry> ChunkLanguageEntries, int NumChunks, bool bPlaygoEmulation, ProjectParams Params, DeploymentContext SC, bool bForceDualLayer, out String[] OutChunkLayers)
	{
		...
		for (int i = 0; i < NumChunks; ++i)
		{
			...
			if (MatchinglanguageEntries.Count() == 0 || !String.Equals(MatchinglanguageEntries.First().Label, "EMPTY", StringComparison.OrdinalIgnoreCase))
			{
    ...
    public override void Package(ProjectParams Params, DeploymentContext SC, int WorkingCL)
	{
		...
		Parallel.ForEach(TitleParameters, (TitleParams) =>
		{
			...
			Parallel.For(0, SC.StageTargetConfigurations.Count, TargetConfigurationIdx =>
			{
				...
				RecurseStagingDirectory(Path.Combine(Params.BaseStageDirectory, "PS4") + "\\", "", 0, ChunkLanguageEntries, TitleParams, FilesContents, DirectoriesContents, TargetConfiguration, bPlayGoEmulation, true, TargetExecutable, Params.GeneratePatch, Params.GenerateRemaster, out NumChunks);
				...
				if (Params.IsGeneratingPatch || Params.IsGeneratingRemaster)
				{
					...
					{
						LogInformation("Generating patch against {0}", LatestPatch);
	...
	public override void PostStagingFileCopy(ProjectParams Params, DeploymentContext SC)
	{
		...
		//PlayGo handling
		{
			...
			if (SC.PlatformUsesChunkManifests && !Params.Distribution)
			{
				...
				List<ChunkLanguageEntry> ChunkLanguageEntries = GetChunkLanguageEntries(Params, SC);
				RecurseStagingDirectory(Path.Combine(Params.BaseStageDirectory, "PS4") + "\\", "", 0, ChunkLanguageEntries, PackageParameters, FilesContents, DirectoriesContents, SC.StageTargetConfigurations[0], bPlayGoEmulation, false, TargetExecutable, Params.GeneratePatch, Params.GenerateRemaster, out NumChunks);
