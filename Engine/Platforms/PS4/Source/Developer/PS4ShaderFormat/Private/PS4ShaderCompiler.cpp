...
#ifndef USING_PS4_SHADER_PERF_ANALYSIS
#define USING_PS4_SHADER_PERF_ANALYSIS 0
#endif
...
static bool bShaderSDBCreate = false;
static bool bShaderSDBIgnoreFileHash = false;
 ...
 */
static void AddResource(FShaderCompilerOutput& Output, const FString& ResourceName, uint16 BufferIndex, uint16 ResourceIndex, uint16 ElementSize, EShaderParameterType ParameterType)
{
	// is it an array?
	if (ResourceName.Contains(TEXT("[")))
	{
		...
		Output.ParameterMap.AddParameterAllocation(	*Tokens[0], BufferIndex, BaseIndex, ArrayTotalSize, ParameterType );
...
static FString GetSystemSemanticName(const char* PSSLEnumName)
{
	...
	EnumName.RightChopInline(10, false);
...
static size_t ShaderAnalysisTraceLogCallBack(void* arg, const void* buffer, size_t sizeInBytes)
{
	UE_LOG(LogShaders, Log, TEXT("%s"), ANSI_TO_TCHAR(buffer));

	return sizeInBytes;
}

static size_t ShaderAnalysisWarningLogCallBack(void* arg, const void* buffer, size_t sizeInBytes)
{
	UE_LOG(LogShaders, Warning, TEXT("%s"), ANSI_TO_TCHAR(buffer));

	return sizeInBytes;
}

static size_t ShaderAnalysisErrorLogCallBack(void* arg, const void* buffer, size_t sizeInBytes)
{
	UE_LOG(LogShaders, Error, TEXT("%s"), ANSI_TO_TCHAR(buffer));
...
static void ProcessShaderInformation(FCompilerInfo& CompilerInfo, bool bSecondPass, const TArray<uint8>& ShaderContents, FString& PreprocessedSource, FString& EntryPoint, FShaderCompilerOutput& Output)
{
...
#if USING_PS4_SHADER_PERF_ANALYSIS
	{
		Shader::Perf::ShaderParams ShaderAnalysisParams;
		Shader::Perf::initializeDefaultShaderParams(&ShaderAnalysisParams);
		ShaderAnalysisParams.traceLogWriter.callback = ShaderAnalysisTraceLogCallBack;
		ShaderAnalysisParams.warningLogWriter.callback = ShaderAnalysisWarningLogCallBack;
		ShaderAnalysisParams.errorLogWriter.callback = ShaderAnalysisErrorLogCallBack;
	...
	for (uint16 BufferIndex = 0; BufferIndex < Program.m_numBuffers; BufferIndex++)
	{
		...
		if (Buffer.m_internalType == Shader::Binary::kInternalBufferTypeCbuffer)
		{
			...
			if (Buffer.m_resourceIndex == 0 && FCStringAnsi::Strcmp((ANSICHAR*)Buffer.getName(), "__GLOBAL_CB__") == 0)
			{
				...
				for( uint32 ElementIndex = Buffer.m_elementOffset; ElementIndex < Buffer.m_elementOffset + Buffer.m_numElements; ElementIndex++ )
				{
					...
					Output.ParameterMap.AddParameterAllocation( ANSI_TO_TCHAR( CurrentElement.getName() ), Buffer.m_resourceIndex, ( uint16 )CurrentElement.m_byteOffset, CurrentElement.m_size, EShaderParameterType::LooseData );
				}
			}
			else
			{
				Output.ParameterMap.AddParameterAllocation( ANSI_TO_TCHAR( Buffer.getName() ), Buffer.m_resourceIndex, 0, Buffer.m_strideSize, EShaderParameterType::UniformBuffer );
			}
		...
		{
			AddResource(Output, ANSI_TO_TCHAR(Buffer.getName()), 0, (uint16)Buffer.m_resourceIndex, 1, EShaderParameterType::SRV);
		}
	}
	...
	for (uint16 SamplerIndex = 0; SamplerIndex < Program.m_numSamplerStates; SamplerIndex++)
	{
		...
		AddResource(Output, ANSI_TO_TCHAR(Sampler.getName()), 0, (uint16)Sampler.m_resourceIndex, 1, EShaderParameterType::Sampler);
	}
...
static FString GetCommandLineExtraParameters(const FCompilerInfo& CompilerInfo)
{
	...
	if (bMixedModeShaderDebugInfo)
	{
		ExtraSwitches += FString(TEXT(" -Od"));
	}
...
static FString CreateShaderCompileCommandLine(const FCompilerInfo& CompilerInfo, const FString& SourceFile, const FString& OutputFile, const FString& EntryPointName, const FString& Profile) 
{
	...
	CmdLine += FString::Printf(TEXT("\n\"%%SCE_ORBIS_SDK_DIR%%\\host_tools\\bin\\orbis-shaderperf.exe\" -i \"%s\" --disasm \"%sPerf.asm\" --disasm-reg-pressure > %sPerfReport.txt"), *OutputFile, *OutputFileNoExt, *OutputFileNoExt);
...
static inline FString GetTargetOutputModeString( EPixelFormat PixelFormat )
{
	...
	switch( PixelFormat )
		...
		TARGETPIXELFORMATCASE(PF_R32G32B32A32_UINT, FMT_32_ABGR)
		TARGETPIXELFORMATCASE(PF_R32_FLOAT, FMT_32_R)
		TARGETPIXELFORMATCASE(PF_R32_UINT, FMT_32_R)
		TARGETPIXELFORMATCASE(PF_G16, FMT_UNORM16_ABGR)
		TARGETPIXELFORMATCASE(PF_G16R16, FMT_UNORM16_ABGR)
		TARGETPIXELFORMATCASE(PF_G32R32F, FMT_32_GR)
		TARGETPIXELFORMATCASE(PF_R32G32_UINT, FMT_32_GR)
		TARGETPIXELFORMATCASE(PF_A16B16G16R16, FMT_UNORM16_ABGR)
		TARGETPIXELFORMATCASE(PF_R16_UINT, FMT_UINT16_ABGR)
		TARGETPIXELFORMATCASE(PF_R16G16_UINT, FMT_UINT16_ABGR)
		TARGETPIXELFORMATCASE(PF_R16G16B16A16_UINT, FMT_UINT16_ABGR)
	default:
		// PF_B8G8R8A8
		// PF_FloatRGB
		// PF_FloatRGBA
...
struct FRunPsslc
{
	...
	Wave::Psslc::SourceFile SingleSourceFile;

	FRunPsslc(FCompilerInfo& CompilerInfo, const FString& InPreprocessedSource, const char* Filename)
	{
		auto Source = StringCast<ANSICHAR>(*InPreprocessedSource);

		SingleSourceFile.fileName = Filename;
		SingleSourceFile.text = Source.Get();
		SingleSourceFile.size = Source.Length();

		// set up callback functions
		Wave::Psslc::initializeCallbackList(&Callbacks, Wave::Psslc::kCallbackDefaultsTrivial);
		Callbacks.openFile = (Wave::Psslc::CallbackOpenFile)CallbackOpenFileWave;
		CompilerInfo.WaveOptions.userData = this;
	...
	{
		FRunPsslc& This = *(FRunPsslc*)userData;
...
static void CompileUsingInternal(bool bSecondPass, FCompilerInfo& CompilerInfo, FString& PreprocessedShaderSource, FString& EntryPointName, FShaderCompilerOutput& Output)
{
	...
	auto EntryName = StringCast<ANSICHAR>(*EntryPointName);
	auto FileName = StringCast<ANSICHAR>(*CompilerInfo.Input.VirtualSourceFilePath);

	CompilerInfo.WaveOptions.entryFunctionName = EntryName.Get();
	CompilerInfo.WaveOptions.packingParameter = (GPS4PackParameters != 0) ? 1 : 0;

	static const uint32 SuppressedWarnings[] = 
	{ 
		20087,  // "unreferenced formal parameter"
		20088	// "unreferenced local variable"
	};
	CompilerInfo.WaveOptions.suppressedWarningsCount = UE_ARRAY_COUNT(SuppressedWarnings);
	CompilerInfo.WaveOptions.suppressedWarnings = SuppressedWarnings;
	...
	// compile it!
	FRunPsslc RunPsslc(CompilerInfo, PreprocessedShaderSource, FileName.Get());

	if (RunPsslc.Results)
	{
		Output.bSucceeded = true;

		FShaderCompilerError CurrentError = {};
		auto FlushError = [&]()
		{
			if (!CurrentError.StrippedErrorMessage.IsEmpty())
			{
				Output.Errors.Add(CurrentError);
				CurrentError = {};
			}
		};
		...
		for (int32 MessageIndex = 0; MessageIndex < RunPsslc.Results->diagnosticCount; MessageIndex++)
		{
			...
			{
				// Found a new warning/error. Start over...
				FlushError();
				...
				if (Message.location)
				{
					CurrentError.ErrorVirtualFilePath = ANSI_TO_TCHAR(Message.location->file->fileName);
					CurrentError.ErrorLineString = FString::Printf(TEXT("%d:%d"), Message.location->lineNumber, Message.location->columnNumber);
				}
				else
				{
					CurrentError.ErrorVirtualFilePath = TEXT("[Unknown File]");
					CurrentError.ErrorLineString = TEXT("[Unknown Line Number]");
				}

				CurrentError.StrippedErrorMessage = FString::Printf(TEXT("(%s, code:%d) - %s"),
					(Message.level == Wave::Psslc::kDiagnosticLevelError) ? TEXT("error") : TEXT("warning"),
					Message.code,
					ANSI_TO_TCHAR(Message.message));
			}
			// If we've already started an error/warning string, consume any additional info lines.
			else if (!CurrentError.StrippedErrorMessage.IsEmpty())
			{
				if (Message.location)
				{
					CurrentError.StrippedErrorMessage += FString::Printf(TEXT("\n%s: %s:%d:%d"),
						UTF8_TO_TCHAR(Message.message),
						UTF8_TO_TCHAR(Message.location->file->fileName),
						Message.location->lineNumber,
						Message.location->columnNumber);
				}
				else
				{
					CurrentError.StrippedErrorMessage += FString::Printf(TEXT("\n%s"), UTF8_TO_TCHAR(Message.message));
				}
			}
		}
	...
	if (Output.bSucceeded)
	{
		...
		// serialize the platform debug data
		if (bShaderSDBCreate)
		{
			FPS4ShaderDebugData DebugData;
			FSDB* SDB = new (DebugData.SDBs) FSDB();
			SDB->Hash = FSDB::ExtensionToHash(RunPsslc.Results->sdbExt);
			SDB->Contents.AddUninitialized(RunPsslc.Results->sdbDataSize);
			FMemory::Memcpy(SDB->Contents.GetData(), RunPsslc.Results->sdbData, RunPsslc.Results->sdbDataSize);
...
bool PreprocessShaderPS4( FString& PreprocessedShaderSource, const FShaderCompilerInput& Input, FShaderCompilerOutput& Output, const FShaderCompilerDefinitions& AdditionalDefines, FString& OutFinalEntryPoint, FShaderParameterParser& ShaderParameterParser)
{
...
void CompilePSSLShader(const FShaderCompilerInput& Input, FShaderCompilerOutput& Output, const FString& WorkingDirectory)
{
...

	{
		...
		{
			if (!Input.Environment.CompilerFlags.Contains(CFLAG_ForceOptimization))
			{
	...
	}
	{
		...
		switch (CVar ? CVar->GetInt() : 0)
		{
		case 1:
			// Enable SDBs. Ignore file hashes (force the value to 0).
			bShaderSDBCreate = true;
			bShaderSDBIgnoreFileHash = true;
			break;

		case 2:
			// Enable SDBs. Keep the full hash.
			bShaderSDBCreate = true;
			bShaderSDBIgnoreFileHash = false;
			break;

		default:
			// Disable SDBs.
			bShaderSDBCreate = false;
			bShaderSDBIgnoreFileHash = false;
			break;
		}
	}
	...
	*/
	...
	FShaderParameterParser ShaderParameterParserNeo;

	if ( PreprocessShaderPS4( PreprocessedShaderSource, Input, Output, AdditionalDefines, EntryPointName, ShaderParameterParser) == false )
	{
		return;
	}
	...
	if( bIsPlatformSpecific )
	{
		...
		if ( PreprocessShaderPS4( PreprocessedShaderSourceNeo, Input, Output, AdditionalDefines, NeoEntryPointName, ShaderParameterParserNeo) == false )
		{
			return;
		}
	...
	CompilerInfo.WaveOptions.sdbCache = bShaderSDBCreate ? 1 : 0;
	CompilerInfo.WaveOptions.cacheGenSourceHash = bShaderSDBIgnoreFileHash ? 0 : 1;
	CompilerInfo.WaveOptions.cacheUserSourceHash = 0;
	...
	if (bIsPlatformSpecific)
	{
		...
		ShaderParameterParser.ValidateShaderParameterTypes(CompilerInfo.Input, Output);
		ShaderParameterParserNeo.ValidateShaderParameterTypes(CompilerInfo.Input, OutputNeo);
		...
		// If we have SDB data, copy the neo SDB into the base output
		if (Output.PlatformDebugData.Num())
		{
			check(OutputNeo.PlatformDebugData.Num());

			// Load the existing data
			FMemoryReader BaseAr(Output.PlatformDebugData);
			FMemoryReader NeoAr(OutputNeo.PlatformDebugData);

			FPS4ShaderDebugData BaseDebugData, NeoDebugData;
			BaseAr << BaseDebugData;
			NeoAr << NeoDebugData;

			// Add the neo SDB to the base output
			BaseDebugData.SDBs.Append(NeoDebugData.SDBs);

			// Re-serialize
			Output.PlatformDebugData.Reset();
			FMemoryWriter Writer(Output.PlatformDebugData);
			Writer << BaseDebugData;
		}
	}
	else
	{
	...
	if (CompilerInfo.Input.DumpDebugInfoPath != TEXT("") && IFileManager::Get().DirectoryExists(*CompilerInfo.Input.DumpDebugInfoPath))
	{
		...
		// Output OpenSCUI.bat.
		{
			...
			FString BatchFileContents = FString::Printf(
				TEXT("\"%%SCE_ORBIS_SDK_DIR%%\\host_tools\\bin\\orbis-scui.exe\" -i %s --entry %s --profile %s --compiler-type pssl --compile-on-start --compiler-args \"-enable-dx10-clamp %s\"\n"),
				*Filename, *EntryPointName, *CompilerInfo.Profile, *ExtraSwitches);
			FFileHelper::SaveStringToFile(BatchFileContents, *(CompilerInfo.Input.DumpDebugInfoPath / FString::Printf(TEXT("%sOpenSCUI.bat"), FileNamePrefix)));
		}
