...
void FOnlinePresencePS4::SetPresence(const FUniqueNetId& User, const FOnlineUserPresenceStatus& Status, const FOnPresenceTaskCompleteDelegate& Delegate)
{
	if(!PS4Subsystem)
	{
		UE_LOG_ONLINE_PRESENCE(Warning, TEXT("FOnlinePresencePS4::SetPresence - No PS4Subsystem, returning false."));
	...
	{
		UE_LOG_ONLINE_PRESENCE(Warning, TEXT("FOnlinePresencePS4::SetPresence - Call rate limit has been exceeded, returning false."));
		Delegate.ExecuteIfBound(User, false);
		return;
	}
	...
	{
		UE_LOG_ONLINE_PRESENCE(Warning, TEXT("FOnlinePresencePS4::SetPresence - Empty presence string, returning false."));
		Delegate.ExecuteIfBound(User, false);
		return;
	}
	...
	WebApiTask->GetTask().SetRequest(ENpApiGroup::UserProfile, BaseURL, SCE_NP_WEBAPI_HTTP_METHOD_PUT);
...
void FOnlinePresencePS4::QueryPresence(const FUniqueNetId& User, const FOnPresenceTaskCompleteDelegate& Delegate)
{
	if (!PS4Subsystem)
	{
		UE_LOG_ONLINE_PRESENCE(Warning, TEXT("FOnlinePresencePS4::QueryPresence - No PS4Subsystem, returning false."));
		Delegate.ExecuteIfBound(User, false);
		return;
	}
	...
	{
		UE_LOG_ONLINE_PRESENCE(Warning, TEXT("FOnlinePresencePS4::QueryPresence - Call rate limit has been exceeded, returning false."));
		Delegate.ExecuteIfBound(User, false);
		return;
	}
	...
	BackgroundWebApiTask * WebApiTask = new BackgroundWebApiTask(LocalUserWebApiContext);
	WebApiTask->GetTask().SetRequest(ENpApiGroup::UserProfile, BaseURL, SCE_NP_WEBAPI_HTTP_METHOD_GET);
...
void FOnlinePresencePS4::CleanTaskList(TArray<FPresenceTaskBackgroundWrapper>& CurrentTasks, double CallRateLimitDuration)
{
	...
	CurrentTasks.RemoveAllSwap([&CallRateLimitDuration, &CurrentTime](const FPresenceTaskBackgroundWrapper& Task)
	{
		...
		// Delete this task
		UE_LOG_ONLINE_PRESENCE(VeryVerbose, TEXT("Removing Presence Task from call-rate list"));
		return true;
	});
}
