...
bool FPS4SaveGameSystem::DeleteGame(bool bAttemptToUseUI, const TCHAR* Name, const int32 UserIndex)
{
	int32 Result;
	...
	{
		// Can't mount because we don't have user, so say that the save game doesn't exist
		UE_LOG(LogPS4SaveGame, Warning, TEXT("Failing DeleteGame because we don't have a user for mounting save data"));
		return false;
	}
	SetCurrentUser(Id);
	...
	if (SavedGameCount <= 0)
	{
		UE_LOG(LogPS4SaveGame, Log, TEXT("DeleteGame() called with no saved games."));
		return false;
	}
	...
	// Start a potential dialog scope for this entire function call
	{
		...
		if (bAttemptToUseUI)
		{
			...
			if (!DisplayDialog())
			{
				UE_LOG(LogPS4SaveGame, Warning, TEXT("DisplayDialog() failed in DeleteGame(). See previous log entries for reasons."));
				return false;
			}
			...
			if (SaveDialogResults.result == SCE_COMMON_DIALOG_RESULT_OK)
			{
				if (SaveDialogResults.dirName == NULL || SaveDialogResults.dirName->data == NULL || SaveDialogResults.dirName->data[0] == '\0')
				{
					UE_LOG(LogPS4SaveGame, Warning, TEXT("Invalid directory name returned from sceSaveDataDialogGetResult() in DeleteGame()"));
					return false;
				}
