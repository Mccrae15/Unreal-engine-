			...
			for (SceSystemServiceEvent& SystemEvent : SystemServiceEvents)
			{
				...
				case SCE_SYSTEM_SERVICE_EVENT_RESET_VR_POSITION:
					...
					else
					{
						...
						FPS4Application::GetPS4Application()->OnVRHeadsetRecenterDefaultDelegate.Broadcast();
					}
					break;
				case SCE_SYSTEM_SERVICE_EVENT_PLAY_TOGETHER_HOST_A:
				{
					const SceNpPlayTogetherHostEventParamA& Param = reinterpret_cast<SceNpPlayTogetherHostEventParamA&>(SystemEvent.data.param);
					...
					{
						FPS4Application::GetPS4Application()->CachedPlayTogetherHostEventParam = Param;
					}
					break;
				}
				case SCE_SYSTEM_SERVICE_EVENT_JOIN_EVENT:
				{
					const SceNpEventJoinEventParam& Param = reinterpret_cast<const SceNpEventJoinEventParam&>(SystemEvent.data.param);
					UE_LOG(LogPS4, Log, TEXT("JoinEvent event received."));

					if (FPS4Application::GetPS4Application()->OnTournamentJoinEventDelegate.IsBound())
					{
						FPS4Application::GetPS4Application()->OnTournamentJoinEventDelegate.Broadcast(Param);
					}
					else
					{
						FPS4Application::GetPS4Application()->CachedTournamentJoinEventParam = Param;
					}
					break;
				}
				case SCE_SYSTEM_SERVICE_EVENT_JOIN_MATCH_EVENT:
				{
					const SceNpEventJoinMatchEventParam& Param = reinterpret_cast<const SceNpEventJoinMatchEventParam&>(SystemEvent.data.param);
					UE_LOG(LogPS4, Log, TEXT("JoinMatchEvent event received."));

					if (FPS4Application::GetPS4Application()->OnTournamentJoinMatchEventDelegate.IsBound())
					{
						FPS4Application::GetPS4Application()->OnTournamentJoinMatchEventDelegate.Broadcast(Param);
					}
					else
					{
						FPS4Application::GetPS4Application()->CachedTournamentJoinMatchEventParam = Param;
					}
					break;
				}
				case SCE_SYSTEM_SERVICE_EVENT_JOIN_TEAM_ON_TEAM_MATCH_EVENT:
				{
					const SceNpEventJoinTeamOnTeamMatchEventParam& Param = reinterpret_cast<const SceNpEventJoinTeamOnTeamMatchEventParam&>(SystemEvent.data.param);
					UE_LOG(LogPS4, Log, TEXT("JoinMatchEvent event received."));

					if (FPS4Application::GetPS4Application()->OnTournamentJoinTeamMatchEventDelegate.IsBound())
					{
						FPS4Application::GetPS4Application()->OnTournamentJoinTeamMatchEventDelegate.Broadcast(Param);
					}
					else
					{
						FPS4Application::GetPS4Application()->CachedTournamentJoinTeamOnTeamMatchEventParam = Param;
					}
					break;
				}
				}
...
bool FPS4Application::IsMouseAttached() const
{
...
{
	FPS4PlatformCrashContext(ECrashContextType InType, const TCHAR* InErrorMessage)
...
struct FPS4CoreDumpHandler
{
	...
	static int32 HandleCoreDump(void* Common)
	{
		...
		// Stop the hitch heartbeat thread so that it doesn't interfere with crash reporting
		FGameThreadHitchHeartBeat* HitchHeartBeat = FGameThreadHitchHeartBeat::GetNoInit();
		if (HitchHeartBeat)
		{
			HitchHeartBeat->Stop();
		}
		...
		FPlatformCrashContext CrashContext(ECrashContextType::Crash, TEXT("Core dump"));
