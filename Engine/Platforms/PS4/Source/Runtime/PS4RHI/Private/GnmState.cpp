...
static Gnm::BlendFunc TranslateBlendOp(EBlendOperation BlendOp)
{
	switch(BlendOp)
	{
		...
		case BO_ReverseSubtract:	return Gnm::kBlendFuncReverseSubtract;
...
static Gnm::BlendMultiplier TranslateBlendFactor(EBlendFactor BlendFactor)
{
	switch(BlendFactor)
	{
		...
		case BF_Source1Color:			return Gnm::kBlendMultiplierSrc1Color;
		case BF_InverseSource1Color:	return Gnm::kBlendMultiplierInverseSrc1Color;
		case BF_Source1Alpha:			return Gnm::kBlendMultiplierSrc1Alpha;
		case BF_InverseSource1Alpha:	return Gnm::kBlendMultiplierInverseSrc1Alpha;
...
FGnmSamplerState::FGnmSamplerState(const FSamplerStateInitializerRHI& Initializer)
{
	...
	constexpr float MaxLodU4_8 = float(0xFFF) / 256.0f; // upper limit of range [0.0..16.0) in unsigned 4.8
	Sampler.setLodRange(sce::Gnmx::convertF32ToU4_8(FMath::Clamp(Initializer.MinMipLevel, 0.0f, MaxLodU4_8)), sce::Gnmx::convertF32ToU4_8(FMath::Clamp(Initializer.MaxMipLevel, 0.0f, MaxLodU4_8)));
	
	Sampler.setLodBias(sce::Gnmx::convertF32ToS6_8(Initializer.MipBias), sce::Gnmx::convertF32ToS2_4(0.f));
