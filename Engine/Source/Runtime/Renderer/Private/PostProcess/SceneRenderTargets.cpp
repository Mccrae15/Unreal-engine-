...
void FSceneRenderTargets::AllocateCommonDepthTargets(FRHICommandList& RHICmdList)
{
	...
	if (!SceneDepthZ || GFastVRamConfig.bDirty)
	{
		...
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(BufferSize, bRequireMultiview ? PF_D24 : PF_DepthStencil, DefaultDepthClear, TexCreate_None, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead | DepthUAVFlag, false));
		Desc.NumSamples = GetNumSceneColorMSAASamples(CurrentFeatureLevel);
