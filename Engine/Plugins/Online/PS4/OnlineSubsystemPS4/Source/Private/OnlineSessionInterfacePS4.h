...
public:
	...
	*/
	FOnlineAsyncTaskPS4SessionPutChangeableSessionData(FOnlineSubsystemPS4 * InPS4Subsystem, int32 InUserWebApiContext, const FUniqueNetIdString* InSessionId, const FString& InData)
		: PS4Subsystem(InPS4Subsystem), UserWebApiContext(InUserWebApiContext), SessionId(InSessionId->ToString(), InSessionId->GetType()), Data(InData)
	{
		check(SessionId.GetType() == PS4_SUBSYSTEM);
	}
