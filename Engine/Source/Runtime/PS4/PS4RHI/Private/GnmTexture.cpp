...
{	
	...
	uint32 DataSize = SourcePitch == 0 ?
		UpdateRegion.Width * UpdateRegion.Height * BytesPerTexel :
		SourcePitch * UpdateRegion.Height;
	...
	checkf(SourcePitch % BytesPerTexel == 0,
		TEXT("Padding needs to be in multiple of texels. SourcePitch=%d, BytesPerTexel=%d"),
		SourcePitch,
		BytesPerTexel);
	...
	{
		SourcePitch == 0 ? UpdateRegion.Width * DestDataFormat.getNumComponents() : (SourcePitch / BytesPerTexel) * DestDataFormat.getNumComponents(),
	...
	int32 NumSrcElements = SourcePitch == 0 ?
		...
		(SourcePitch / BytesPerTexel) * UpdateRegion.Height;
