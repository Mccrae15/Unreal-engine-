...
FPS4MediaPlayer::FPS4MediaPlayer(IMediaEventSink& InEventSink)
	...
	, FirstAudioSampleTime(FTimespan::MinValue())
	, HasSetTimeToFirstAudioSample(false)
{ }
...
bool FPS4MediaPlayer::Open(const FString& Url, const IMediaOptions* Options, const FMediaPlayerOptions* PlayerOptions)
{
	Close();
...
void FPS4MediaPlayer::TickFetch(FTimespan DeltaTime, FTimespan /*Timecode*/)
{
	...
	if (Tracks->GetTrackSelectionChanged())
	{
		...
		InitializePlayer(Callbacks->GetArchive(), CurrentUrl, false, nullptr);
	}
	...
	ENQUEUE_RENDER_COMMAND(PS4MediaPlayerTickVideo)(
		[Params = TickVideoParams](FRHICommandListImmediate& RHICmdList)
		{
...
void FPS4MediaPlayer::TickInput(FTimespan DeltaTime, FTimespan /*Timecode*/)
{
	...
	for (const auto& Event : OutEvents)
	{
		...
		case EMediaEvent::MediaOpened:
			...
			UE_LOG(LogPS4Media, Verbose, TEXT("FPS4MediaPlayer::TickInput() EMediaEvent::MediaOpened, Clear FirstAudioSample, CurrentTime: %s, AvPlayerTime: %s"), *CurrentTime.ToString(), *AvPlayerTime.ToString());
			FirstAudioSampleTime = FTimespan::MinValue();
			HasSetTimeToFirstAudioSample = false;
			break;
		...
		case EMediaEvent::PlaybackEndReached:
			...
			{
				// Wait until next frame to update CurrentTime to match audio sample time
				bLooped = true;
			}
			break;

		case EMediaEvent::PlaybackResumed:
			AvPlayerTime = FTimespan(sceAvPlayerCurrentTime(PlayerHandle) * ETimespan::TicksPerMillisecond);

			UE_LOG(LogPS4Media, Verbose, TEXT("FPS4MediaPlayer::TickInput() EMediaEvent::PlaybackResumed, CurrentTime: %s, AvPlayerTime: %s"),
				*CurrentTime.ToString(), *AvPlayerTime.ToString());

			// AvPlayer decoder position can be 3+ seconds ahead of CurrentTime after fast forward
			// Change CurrentTime to match the decoder otherwise no video/audio will play for several seconds (with desynced or stuttering audio afterwards)
			if (AvPlayerTime > CurrentTime)
			{
				CurrentTime = AvPlayerTime;
			}
		...
		case EMediaEvent::SeekCompleted:
			...
			UE_LOG(LogPS4Media, Verbose, TEXT("FPS4MediaPlayer, TickInput EMediaEvent::SeekCompleted, CurrentTime: %s set to AvPlayerTime"), *CurrentTime.ToString());
			break;
	...
	{
		if (ShouldLoop || (CurrentTime < CurrentDuration))
		{
			CurrentTime += DeltaTime * CurrentRate;
		}
	}

	if (!HasSetTimeToFirstAudioSample)
	{
		const FTimespan FirstSampleTime = FirstAudioSampleTime.Load();
		if (FirstSampleTime != FTimespan::MinValue())
		{
			CurrentTime = FirstSampleTime;
			HasSetTimeToFirstAudioSample = true;
			UE_LOG(LogPS4Media, Verbose, TEXT("FPS4MediaPlayer::TickInput() CurrentTime: %s set to first decoded audio sample"), *CurrentTime.ToString());
		}
	}
	...
	if ((CurrentTime >= CurrentDuration) || (CurrentTime < FTimespan::Zero()))
	{
		if (ShouldLoop)
		{
			// Keep decoding samples until we receive EMediaEvent::PlaybackEndReached
		}
		else
		{
	...
	if (bLooped)
	{
		...
		// Set CurrentTime to next audio sample time played
		FirstAudioSampleTime = FTimespan::MinValue();
		HasSetTimeToFirstAudioSample = false;
	}
}
...
bool FPS4MediaPlayer::InitializePlayer(const TSharedPtr<FArchive, ESPMode::ThreadSafe>& Archive, const FString& Url, bool Precache, const FMediaPlayerOptions* PlayerOptions)
{
	// get settings
	...
	const bool CurrentShouldLoop = ShouldLoop;

	if (PlayerOptions)
	{
		const FMediaPlayerTrackOptions& TrackOptions = PlayerOptions->Tracks;
		Tracks->SetDesiredTrack(EMediaTrackType::Audio, TrackOptions.Audio);
		Tracks->SetDesiredTrack(EMediaTrackType::Caption, TrackOptions.Caption);
		Tracks->SetDesiredTrack(EMediaTrackType::Video, TrackOptions.Video);
	}

	Async(Execution, [Archive, CallbacksPtr = TWeakPtr<FPS4MediaCallbacks, ESPMode::ThreadSafe>(Callbacks), NumVideoBuffers, Precache, Url, VideoBufferSizeMB, CurrentShouldLoop]()
	{
		TSharedPtr<FPS4MediaCallbacks, ESPMode::ThreadSafe> PinnedCallbacks = CallbacksPtr.Pin();

		if (PinnedCallbacks.IsValid())
		{
			PinnedCallbacks->Initialize(Archive, Url, Precache, NumVideoBuffers, VideoBufferSizeMB, CurrentShouldLoop);
		}
	});
...
bool FPS4MediaPlayer::FlushOnSeekStarted() const
{
	return true;
}

bool FPS4MediaPlayer::FlushOnSeekCompleted() const
{
	return false;
}
...
bool FPS4MediaPlayer::SetLooping(bool Looping)
{
	...
	const SceAvPlayerHandle PlayerHandle = GetCurrentHandle();
	if (PlayerHandle && sceAvPlayerIsActive(PlayerHandle))
	{
		sceAvPlayerSetLooping(PlayerHandle, Looping);
	}
...
{
	if (Samples->FetchAudio(TimeRange, OutSample))
	{
		// Use first audio sample to set player's CurrentTime
		// Required because after Play() it can take over one second for the first sample to be decoded
		if (FirstAudioSampleTime.Load() == FTimespan::MinValue() && OutSample.IsValid())
		{
			FirstAudioSampleTime = OutSample->GetTime();
		}
