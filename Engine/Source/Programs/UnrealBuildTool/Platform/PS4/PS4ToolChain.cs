	...
	class PS4ToolChain : UEToolChain
	{
		...
		public override CPPOutput CompileCPPFiles(CppCompileEnvironment CompileEnvironment, List<FileItem> InputFiles, DirectoryReference OutputDir, string ModuleName, ActionGraph ActionGraph)
		{
			...
			foreach (FileItem SourceFile in InputFiles)
			{
				...
				if (CompileEnvironment.PrecompiledHeaderAction == PrecompiledHeaderAction.Create)
				{
					string PCHExtension = UEBuildPlatform.GetBuildPlatform(UnrealTargetPlatform.PS4).GetBinaryExtension(UEBuildBinaryType.PrecompiledHeader);
				...
				else
				{
					...
					string ObjectFileExtension = UEBuildPlatform.GetBuildPlatform(UnrealTargetPlatform.PS4).GetBinaryExtension(UEBuildBinaryType.Object);
