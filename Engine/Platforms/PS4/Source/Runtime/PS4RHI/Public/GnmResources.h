...
#error Disabling the new PS4 memory system is no longer supported
#endif
...
public:
	...
	/** Memory address handle for allocation of shader debug resource **/
	sce::Gnm::ResourceHandle DebugResourceHandle;
#endif
...
public:
	/** Constructor */
	FGnmMultiBufferResource(uint32 Size, uint32 Usage, FRHIResourceCreateInfo& CreateInfo, TStatId StatGPU, TStatId StatCPU);
	 ...
	 */
	void* GetCurrentBuffer(bool bTopOfPipe);

	/**
	 * Prepare a CPU accessible buffer for uploading to GPU memory
	 */
	void* Lock(FRHICommandListImmediate& RHICmdList, EResourceLockMode InLockMode, uint32 Size, uint32 Offset);
	...
	/**
	 * Currently implemented as bitwise swap. If adding new member variables,
	 * please review whether they can be swapped in a bitwise fashion.
	 */
	void Swap(FGnmMultiBufferResource& Other);
...
private:
	...
	FMemBlock CurrentBufferTopOfPipe;
	FMemBlock CurrentBufferBottomOfPipe;

	bool bLocked;
	bool bFirstLock;
	EResourceLockMode LockedMode;

	void SwitchBuffers(const FMemBlock& NewBuffer);
};
...
protected:
	...
	FMemBlock CopyToResolveTargetLabel;
	void* DefragAddress;
...
public:
	...
	{
		check(Surface.GetCMaskMem().GetPointer());

		OutSize = sizeof(uint32);
		if (OutData)
		{
			*static_cast<uint32*>(OutData) = FMath::DivideAndRoundUp(GetSizeX(), 8u);
		}
	}
};
...
public:
	...
	// Constructor, just calls base and Surface constructor
	FGnmTexture2DArray(EPixelFormat Format, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, uint32 Flags, FResourceBulkDataInterface* BulkData, const FClearValueBinding& InClearValue)
...
public:
	...
	/** Constructor */
	FGnmIndexBuffer(uint32 InStride, uint32 InSize, uint32 InUsage, FRHIResourceCreateInfo& CreateInfo);

	/** Bitwise swap */
	void Swap(FGnmIndexBuffer& Other);
};
...
public:
	...
	/** Constructor */
	FGnmVertexBuffer(uint32 InSize, uint32 InUsage, FRHIResourceCreateInfo& CreateInfo);

	/** Bitwise swap */
	void Swap(FGnmVertexBuffer& Other);
};
...
public:
	...
	// Constructor
	FGnmUniformBuffer(const void* Contents, const FRHIUniformBufferLayout& InLayout, EUniformBufferUsage Usage, EUniformBufferValidation InValidation = EUniformBufferValidation::None);
	...
	bool IsConstantBuffer() const { return ConstantBufferSize > 0; }

	void Update(FRHICommandListImmediate* RHICmdList, const void* Contents, EUniformBufferValidation Validation);

private:
	// for storage (multi use) buffers, this is the permanent storage that needs to be freed on destruction
	FMemBlock UniformBufferMemoryRT;
	FMemBlock UniformBufferMemoryRHIT;
...
{
public:
	FGnmUnorderedAccessView();
	FGnmUnorderedAccessView(FRHIStructuredBuffer* StructuredBufferRHI, bool bUseUAVCounter, bool bAppendBuffer);
	FGnmUnorderedAccessView(FRHITexture* TextureRHI, uint32 MipLevel = 0);
	FGnmUnorderedAccessView(FRHITexture* TextureRHI, uint32 MipLevel, uint8 Format);
	FGnmUnorderedAccessView(FRHIVertexBuffer* VertexBufferRHI, uint8 Format);
	FGnmUnorderedAccessView(FRHIIndexBuffer* IndexBufferRHI, uint8 Format);
	...
	*/
	...
	TRefCountPtr<FGnmIndexBuffer> SourceIndexBuffer;
	TRefCountPtr<FRHITexture> SourceTexture;

	// number of components (1-4) in this buffer. zero if unknown
	uint32 NumComponents;

	// The Gnm buffer descriptor, initialized from structured buffer, vertex buffer
	Gnm::Buffer Buffer;
	// The Gnm buffer descriptor, initialized from texture
	Gnm::Texture Texture;
...
public:
	...
	*/
	...
	// The MultiBuffer RHI resource so that we make sure it stays alive (can be null)
	TRefCountPtr<FRHIResource> SourceMultiBufferRHI;

	uint32 BufferOffset = 0;

	// The MultiBuffer resource pointer, refers to the buffer in SourceMultiBufferRHI
	FGnmMultiBufferResource* SourceMultiBufferResource = nullptr;
...
class FGnmGPUFence : public FRHIGPUFence
{
public:
	FGnmGPUFence(FName InName)
		: FRHIGPUFence(InName)
		, LabelLoc(nullptr)
		, ExpectedValue(0)
		, FenceIssued(false)
	...
	void WriteInternal(FGnmComputeCommandListContext& CommandContext);
	void WriteInternal(FGnmCommandListContext& CommandContext);
...
/*
	Gnm implementation of staging buffer class
*/
class FGnmStagingBuffer final : public FRHIStagingBuffer
{
	friend class FGnmCommandListContext;
public:
	FGnmStagingBuffer()
		: FRHIStagingBuffer()
	{}

	virtual ~FGnmStagingBuffer() final override;

	// Locks the shadow of VertexBuffer for read without stalling.
	virtual void *Lock(uint32 Offset, uint32 NumBytes) final override;

	// Unlocks the shadow. This is an error if it was not locked previously.
	virtual void Unlock() final override;
private:
	FMemBlock ShadowBuffer;
};
...
template<>
struct TGnmResourceTraits<FRHIGPUFence>
{
	typedef FGnmGPUFence TConcreteType;
};
template<>
struct TGnmResourceTraits<FRHIStagingBuffer>
{
	typedef FGnmStagingBuffer TConcreteType;
};
