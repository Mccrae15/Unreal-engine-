...
#if PS4_HDR_SCOPE_SUPPORT
#include <libhdrscopes.h>
#endif
...
#if PS4_HDR_SCOPE_SUPPORT
	Ret = sceSysmoduleLoadModule(SCE_SYSMODULE_HDR_SCOPES);
	checkf(Ret == SCE_OK, TEXT("sceSysmoduleLoadModule failed: 0x%x"), Ret);

	SceHdrScopesRequiredMemorySizeAlign RequiredSizesAndAlignment;
	Ret = sceHdrScopesGetRequiredMemorySizeAlign(&RequiredSizesAndAlignment);
	checkf(Ret == SCE_OK, TEXT("sceHdrScopesGetRequiredMemorySizeAlign failed: 0x%x"), Ret);

	SceHdrScopesAttr InitAttr;
	InitAttr.garlicMemoryAddress = FMemBlock::Allocate(RequiredSizesAndAlignment.garlicSize, RequiredSizesAndAlignment.garlicAlign, EGnmMemType::GnmMem_FrameBuffer, GET_STATID(STAT_Garlic_HDRScopes)).GetPointer();
	InitAttr.onionMemoryAddress = FMemBlock::Allocate(RequiredSizesAndAlignment.onionSize, RequiredSizesAndAlignment.onionAlign, EGnmMemType::GnmMem_OnionDirect, GET_STATID(STAT_Onion_HDRScopes)).GetPointer();

	Ret = sceHdrScopesInit(&InitAttr);
	checkf(Ret == SCE_OK, TEXT("sceHdrScopesInit failed: 0x%x"), Ret);
#endif
...
void FGnmManager::CreateSocialScreenBackBuffers()
{
	...
	{
		FRHIResourceCreateInfo CreateInfo;
		AuxRenderBuffers[BufferIndex] = (FGnmTexture2D*)(FRHITexture2D*)RHICreateTexture2D(AuxSizeX, AuxSizeY, PF_B8G8R8A8, 1, 1, TexCreate_RenderTargetable, CreateInfo);
		...
		AuxVideoOutBuffers[BufferIndex] = (FGnmTexture2D*)(FRHITexture2D*)RHICreateTexture2D(AuxSizeX, AuxVideoOutHeight, PF_L8, 1, 1, TexCreate_RenderTargetable | TexCreate_NoTiling | TexCreate_Presentable | TexCreate_UAV, CreateInfo);
...
{
#if PS4_HDR_SCOPE_SUPPORT
	if (bPresent && !bUpdatingCubemap)
	{
		sceHdrScopesAnalyze(&ImmediateContext->GetContext().m_dcb, ImmediateContext->GetCurrentRenderTarget(0));
	}
#endif
...
void FGnmManager::FreePendingMemBlocks(bool bImmediate)
{
	...
	bImmediate = (bImmediate || RHICmdList.Bypass());
	RHICmdList.EnqueueLambda
	(
		bImmediate,
		[this, CapturedRTValue, bImmediate](FRHICommandList&)
		{
