...
#if PLATFORM_ANDROID
	// Set clear color to transparent when MRC is enabled so that we can have transparent backgrounds
	ExecuteOnRenderThread_DoNotWait([](FRHICommandListImmediate& RHICmdList)
	{
		FSceneRenderTargets::Get(RHICmdList).SetDefaultColorClear(FClearValueBinding::Transparent);
	});
#endif
}
