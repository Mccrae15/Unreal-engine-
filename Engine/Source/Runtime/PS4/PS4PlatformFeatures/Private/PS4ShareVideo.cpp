...
bool FPS4ShareVideo::OpenRecording()
{
	...
	UE_LOG(LogPS4, Log, TEXT("PS4ShareVideo::NewRecording: opening new recording with filename %s"), *CurrentRecordingFileName);
...
bool FPS4ShareVideo::NewRecording(const TCHAR* DestinationFileName, FVideoRecordingParameters InParameters)
{
	...
	{
		UE_LOG(LogPS4, Log, TEXT("PS4ShareVideo::NewRecording: length greater than platform maximum of %ds."), GetMaximumRecordingSeconds());
		return false;
	}
...
void FPS4ShareVideo::PS4VideoExportDelegate(bool bSaveRecording, bool bAutoContinue, FText Title, FText Comment)
{
	...
	if (CloseResult == SCE_OK)
	{
		...
		if (bAutoContinue)
		{
			...
			if (OpenRecording())
			{
				...
				else
				{
					...
					UE_LOG(LogPS4, Log, TEXT("PS4ShareVideo::PS4VideoExportDelegate: sceVideoRecordingStart failed. Error code: 0x%08x"), StartResult);
				}
			}
		}
		...
		if (bSaveRecording && Parameters.bExportToLibrary)
		{
			...
			UE_LOG(LogPS4, Log, TEXT("PS4ShareVideo::PS4VideoExportDelegate: finished exporting %s."), *OutputFilePath);
		}
