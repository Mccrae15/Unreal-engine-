...
DEFINE_LOG_CATEGORY_STATIC(LogSonySaveGame, Log, All);
		...
		else
		{
			...
			switch (Result)
			{
				case SCE_COMMON_DIALOG_ERROR_NOT_SYSTEM_INITIALIZED:
					UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDialogInitialize failed in SaveGame() because the common dialog system is not initialized."));
					break;

				case SCE_COMMON_DIALOG_ERROR_BUSY:
					UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDialogInitialize failed in SaveGame() because another common dialog is running."));
					break;

				case SCE_COMMON_DIALOG_ERROR_OUT_OF_MEMORY:
					UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDialogInitialize failed in SaveGame() due to lack of memory."));
					break;

				case SCE_COMMON_DIALOG_ERROR_UNEXPECTED_FATAL:
					// Fall through to default case
				default:
					UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDialogInitialize failed in SaveGame() for an unknown reason. Error code: 0x%x"), Result);
					break;
			}
 ...
 */
static FString GetSaveGamePath(SceSaveDataMountPoint* MountPoint, const TCHAR* SaveFileName)
{
	// NOTE: The mount point is a char * (UTF-8), while all of the string handling in the engine is TCHAR.
	// return FString::Printf(TEXT("%hs/%s"), MountPoint->data, SaveFileName);  // this works
	// return FString::Printf(TEXT("%S/%s"), MountPoint->data, SaveFileName);  // this does not work. %S is not recognized and is treated as literal
	return FString::Printf(TEXT("%s/%s"), UTF8_TO_TCHAR(MountPoint->data), SaveFileName);
}

bool FSonySaveGameSystem::PlatformHasNativeUI()
{
	// Only has native UI if the appropriate module succesfully loads, otherwise treat as if there was no native UI
	return bHasNativeUI;
}

ISaveGameSystem::ESaveExistsResult FSonySaveGameSystem::DoesSaveGameExistWithResult(const TCHAR* Name, const int32 UserIndex)
{
	...
	if (!IsUserIdValid(Id))
	{
		...
		UE_LOG(LogSonySaveGame, Warning, TEXT("Returning that save game doesn't exist because we don't have a user for mounting save data"));
	...
	int32 result = Mount(SCE_SAVE_DATA_MOUNT_MODE_RDONLY, CurrentUser, MountDirName, SCE_SAVE_DATA_BLOCKS_MIN2, MountResult);
	if(result < SCE_OK)
	{
		...
		{
			UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataMount failed in DoesSaveGameExist(). Error code 0x%x"), result);
	...
	// what the Sony reports as saved data and this function, i.e. the system menus will
	...
	FString SaveGamePath = GetSaveGamePath(MountPoint, GetSaveFileName());
	...
	if (!FPaths::FileExists(SaveGamePath))
	{
		...
		UE_LOG(LogSonySaveGame, Error, TEXT("Sony save data dir exists but does not have save file (%s)"), GetSaveFileName());
	...
	// Unmount
	result = Unmount(MountPoint, false);
	if (result < 0 )
	{
		UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataUmount failed in DoesSaveGameExist(). Error code 0x%x"), result);
...
int FSonySaveGameSystem::FindSaveSlot()
{
	// Design Notes:
...
bool FSonySaveGameSystem::SelectOrCreateSavedGame(SceSaveDataParam &MetaData)
{
	...
	while (true)
	{
		...
		if (!DisplayDialog())
		{
			UE_LOG(LogSonySaveGame, Warning, TEXT("DisplayDialog() failed in SaveGame(). See previous log entries for reasons."));
			return false;
		}
		...
		if (SaveDialogResults.result == SCE_COMMON_DIALOG_RESULT_OK)
		{
			...
			else
			{
				...
				else
				{
					...
					UE_LOG(LogSonySaveGame, Warning, TEXT("Unknown result for 'overwrite' message: 0x%x. See prior log entries for reason(s)"), SaveDialogResults.result);
					return false;
				}
			}
...
void FSonySaveGameSystem::DisplaySaveError(int32 ErrorCode)
{
	DisplayError(ErrorCode, SCE_SAVE_DATA_DIALOG_TYPE_SAVE);
}

void FSonySaveGameSystem::DisplayLoadError(int32 ErrorCode)
{
	DisplayError(ErrorCode, SCE_SAVE_DATA_DIALOG_TYPE_LOAD);
}

void FSonySaveGameSystem::DisplayError(int32 ErrorCode, SceSaveDataDialogType DialogType)
{
	// Fill out the basic information for a save game dialog
	FillBasicSaveGameDialogData();
	...
	if (!DisplayDialog())
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("DisplayDialog() failed in DisplayError(). See previous log entries for reasons."));
	}
}

bool FSonySaveGameSystem::DisplayDeleteConfirm()
{
	return DisplayConfirm(SCE_SAVE_DATA_DIALOG_TYPE_DELETE);
}

bool FSonySaveGameSystem::DisplaySystemMessageDialog (SceSaveDataDialogType DialogType, SceSaveDataDialogSystemMessageType MessageType)
{
	// Fill out the basic information for a save game dialog
	FillBasicSaveGameDialogData();
	...
	if (!DisplayDialog())
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("DisplayDialog() failed in DisplayConfirm(). See previous log entries for reasons."));
		return false;
	}
...
bool FSonySaveGameSystem::DisplayConfirm(SceSaveDataDialogType DialogType)
{
	return DisplaySystemMessageDialog(DialogType, SCE_SAVE_DATA_DIALOG_SYSMSG_TYPE_CONFIRM);
}

void FSonySaveGameSystem::DisplayCorruptDialog(SceSaveDataDialogType DialogType)
{
...
bool FSonySaveGameSystem::DisplayDeleteDialog()
{
	...
	while (true)
	{
		...
		if (!DisplayDialog())
		{
			UE_LOG(LogSonySaveGame, Warning, TEXT("DisplayDialog() failed in DisplayDeleteDialog(). See previous log entries for reasons."));
			return false;
		}
		...
		if (SaveDialogResults.result == SCE_COMMON_DIALOG_RESULT_OK)
		{
			...
			if (DisplayDeleteConfirm())
			{
				...
				else
				{
					...
					UE_LOG(LogSonySaveGame, Warning, TEXT("DEBUG: DeleteSavedGame() failed in DisplayDeleteDialog(). See prior log entries for more information."));
					return false;
				}
			}
			else
...
//
bool FSonySaveGameSystem::SaveGame(bool bAttemptToUseUI, const TCHAR* Name, const int32 UserIndex, const TArray<uint8>& Data)
{
	// If the data is > 1GB, fail. The limit is per TRC[R4100].
	if (Data.Num() > 1024 * 1024 * 1024)
	{
		UE_LOG(LogSonySaveGame, Error, TEXT("Failing SaveGame becaause save data is bigger than the TRC[R4100] limit."));
		return false;
	}
	...
	{
		// Can't mount because we don't have user, so say that the save game doesn't exist
		UE_LOG(LogSonySaveGame, Warning, TEXT("Failing SaveGame because we don't have a user for mounting save data"));
		return false;
	}
	SetCurrentUser(Id);
	...
	if (GetSaveGameParameter(Name, EGameDelegates_SaveGame::Icon, IconFilename, sizeof(IconFilename)))
	{
		...
		{
			UE_LOG(LogSonySaveGame, Error, TEXT("Error reading icon file \"%s\", or icon file is not valid."), UTF8_TO_TCHAR(IconFilename));
		}
	}
	...
	// Start a potential dialog scope for this entire function call
	{
		...
		while (true)
		{
			...
			if (GetSaveGameParameter(Name, EGameDelegates_SaveGame::MaxSize, SaveGameSizeStr, sizeof(SaveGameSizeStr)))
			{
			...
			Result = Mount(SCE_SAVE_DATA_MOUNT_MODE_CREATE2, CurrentUser, MountDirName, BlockCount, MountResult);
			...
			if (Result == SCE_SAVE_DATA_ERROR_BROKEN)
			{
				...
				{
					// Failed to delete, so log and give up
					UE_LOG(LogSonySaveGame, Warning, TEXT("DEBUG: DeleteSavedGame() failed in SaveGame(). See prior log entries for more information."));
					return false;
				}
			}
			...
			if (/*Result == SCE_SAVE_DATA_ERROR_NO_SPACE ||*/ Result == SCE_SAVE_DATA_ERROR_NO_SPACE_FS)
			{
				...
				// so based on the API here our only recourse is to fail the save. The user will then have to go out to the Sony
				...
				FMemory::Memzero(&SaveDialogMsgParam, sizeof(SaveDialogMsgParam));
				SaveDialogMsgParam.sysMsgType = GetNoSpaceMessageType();
				...
				else
				{
					...
					UE_LOG(LogSonySaveGame, Warning, TEXT("Fetching results for 'out of storage space' message failed. Error code: 0x%x"), Result);
					return false;
				}
			}
		...
		if (Result < SCE_OK)
		{
			...
			UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataMount failed in SaveGame(). Error code 0x%x"), Result);
		...
		if (!bWasGameSaved)
		{
			...
			UE_LOG(LogSonySaveGame, Error, TEXT("Failed to write save data file %s"), *SaveGamePath);
		...
		else 
		{
			...
			if (IconFileData.Num())
			{
				...
				if (Result != SCE_OK)
				{
					UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataSaveIcon failed in SaveGame(). Error code 0x%x"), Result);
			...
			if (GetSaveGameParameter(Name, EGameDelegates_SaveGame::Title, MetaData.title, sizeof(MetaData.title)))
			{
				...
				if (Result != SCE_OK)
				{
					UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataSetParam failed in SaveGame() for 'Title'. Error Code 0x%x"), Result);
				}
			}
			...
			if (GetSaveGameParameter(Name, EGameDelegates_SaveGame::SubTitle, MetaData.subTitle, sizeof(MetaData.subTitle)))
			{
				...
				if (Result != SCE_OK)
				{
					UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataSetParam failed in SaveGame() for 'SubTitle'. Error Code 0x%x"), Result);
				}
			}
			...
			if (GetSaveGameParameter(Name, EGameDelegates_SaveGame::Detail, MetaData.detail, sizeof(MetaData.detail)))
			{
				...
				if (Result != SCE_OK)
				{
					UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataSetParam failed in SaveGame() for 'Detail'. Error Code 0x%x"), Result);
				}
			}
		}

		// Unmount the directory and commit
		Result = Unmount(MountPoint, true);
		if (Result < 0 )
		{
			UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataUmount failed in SaveGame(). Error code 0x%x"), Result);
...
bool FSonySaveGameSystem::LoadGame(bool bAttemptToUseUI, const TCHAR* Name, const int32 UserIndex, TArray<uint8>& Data)
{
	...
	{
		// Can't mount because we don't have user, so say that the save game doesn't exist
		UE_LOG(LogSonySaveGame, Warning, TEXT("Failing LoadGame because we don't have a user for mounting save data"));
		return false;
	}
	SetCurrentUser(Id);
	...
	if (SavedGameCount <= 0)
	{
		UE_LOG(LogSonySaveGame, Log, TEXT("LoadGame() called with no saved games."));
		return false;
	}
	...
	// Start a potential dialog scope for this entire function call
	{
		...
		if (bAttemptToUseUI)
		{
			...
			if (!DisplayDialog())
			{
				UE_LOG(LogSonySaveGame, Warning, TEXT("DisplayDialog() failed in LoadGame(). See previous log entries for reasons."));
				return false;
			}
			...
			if (SaveDialogResults.result == SCE_COMMON_DIALOG_RESULT_OK)
			{
				if (SaveDialogResults.dirName == NULL || SaveDialogResults.dirName->data == NULL || SaveDialogResults.dirName->data[0] == '\0')
				{
					UE_LOG(LogSonySaveGame, Warning, TEXT("Invalid directory name returned from sceSaveDataDialogGetResult() in LoadGame()"));
					return false;
				}
		...
		SceSaveDataMountResult MountResult;
		Result = Mount(SCE_SAVE_DATA_MOUNT_MODE_RDONLY, CurrentUser, MountDirName, SCE_SAVE_DATA_BLOCKS_MIN2, MountResult);
		...
		if (Result == SCE_SAVE_DATA_ERROR_NOT_FOUND)
		{
			...
			UE_LOG(LogSonySaveGame, Log, TEXT("sceSaveDataMount failed in LoadGame() because save file '%s' does not exist."), Name);
			return false;
		}
		...
		if (Result == SCE_SAVE_DATA_ERROR_BROKEN)
		{
			...
			UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataMount failed in LoadGame() due to corrupted save data"));
			return false;
		}
		...
		if (Result != SCE_OK)
		{
			UE_LOG(LogSonySaveGame, Log, TEXT("sceSaveDataMount failed in LoadGame(). Error code: 0x%x"), Result);
		...
		if (Result < 0 )
		{
			UE_LOG(LogSonySaveGame, Error, TEXT("sceSaveDataUmount failed in LoadGame(). Error code 0x%x"), Result);
			DisplayLoadError(Result);
...
bool FSonySaveGameSystem::DeleteGame(bool bAttemptToUseUI, const TCHAR* Name, const int32 UserIndex)
{
	int32 Result;
	...
	{
		// Can't mount because we don't have user, so say that the save game doesn't exist
		UE_LOG(LogSonySaveGame, Warning, TEXT("Failing DeleteGame because we don't have a user for mounting save data"));
		return false;
	}
	SetCurrentUser(Id);
	...
	if (SavedGameCount <= 0)
	{
		UE_LOG(LogSonySaveGame, Log, TEXT("DeleteGame() called with no saved games."));
		return false;
	}
	...
	// Start a potential dialog scope for this entire function call
	{
		...
		if (bAttemptToUseUI)
		{
			...
			if (!DisplayDialog())
			{
				UE_LOG(LogSonySaveGame, Warning, TEXT("DisplayDialog() failed in DeleteGame(). See previous log entries for reasons."));
				return false;
			}
			...
			if (SaveDialogResults.result == SCE_COMMON_DIALOG_RESULT_OK)
			{
				if (SaveDialogResults.dirName == NULL || SaveDialogResults.dirName->data == NULL || SaveDialogResults.dirName->data[0] == '\0')
				{
					UE_LOG(LogSonySaveGame, Warning, TEXT("Invalid directory name returned from sceSaveDataDialogGetResult() in DeleteGame()"));
					return false;
				}
...
FSonySaveGameSystem::FSonySaveGameSystem() :
	bIsInitialized(false),
	bHasNativeUI(false),
	SavedGameCount(0)
{
...
bool FSonySaveGameSystem::Initialize()
{
	// Initialize the save data library
	int32 Result = sceSaveDataInitialize3(nullptr);
	if (Result < SCE_OK)
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataInitialize() failed. Error code 0x%x"), Result);
	}
	else
	{
		bIsInitialized = true;
	...
	if (Result == SCE_OK)
	{
		...
		if ( Result < SCE_OK )
		{
			if (Result == SCE_COMMON_DIALOG_ERROR_ALREADY_SYSTEM_INITIALIZED)
			{
				...
				UE_LOG(LogSonySaveGame, Log, TEXT("sceCommonDialogInitialize() called with common dialog already initialized"));
			}
			else
			{
				// Should not be possible according to the documentations
				UE_LOG(LogSonySaveGame, Warning, TEXT("sceCommonDialogInitialize() failed. Error code 0x%x"), Result);
			}
		}
	}
	else
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("sceSysmoduleLoadModule() failed. Error code 0x%x"), Result);
...
void FSonySaveGameSystem::Shutdown()
{
	int32 Result;
	if (bHasNativeUI)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG(LogSonySaveGame, Warning, TEXT("sceSysmoduleUnloadModule() failed. Error code 0x%x"), Result);
		}
		bHasNativeUI = false;
	}
	...
	if (bIsInitialized)
	{
		...
		if (Result < 0 )
		{
			UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataTerminate() failed. Error code 0x%x"), Result);
		}
...
void FSonySaveGameSystem::SetCurrentUser(const SceUserServiceUserId User)
{
...
SceUserServiceUserId FSonySaveGameSystem::GetSaveGameUserId(const int32 UserIndex)
{
	// This function implements the policy for getting the id of the user to be used for saving a game.
	// Currently the only valid policy is to ask the SonyApplication based on the UserIndex	
	return FSonyApplication::GetSonyApplication()->GetUserID(UserIndex);
}

void FSonySaveGameSystem::SetMountDirName(const TCHAR *DirName)
{
...
void FSonySaveGameSystem::SetMountDirName(const char *DirName)
{
...
void FSonySaveGameSystem::FindSavedGames()
{
	...
	searchResults.dirNamesNum = UE_ARRAY_COUNT(SavedGameDirs);

	int32 result = sceSaveDataDirNameSearch(&cond, &searchResults);
	if (result != SCE_OK)
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDirNameSearch() failed in FindSavedGames(). Error code: 0x%x"), result);
...
void FSonySaveGameSystem::FillBasicSaveGameDialogData()
{
	...
	// NOTE: This implements a policy of any number of saved games, limited only by the Sony hardware and system software
...
bool FSonySaveGameSystem::DisplayDialog()
{
	//
	...
	if (result != SCE_OK)
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDialogOpen() failed in DisplayDialog(). Error code: 0x%x"), result);
		return false;
	}
	...
	if (result != SCE_OK)
	{
		UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDialogGetResult() failed in DisplayDialog(). Error code: 0x%x"), result);
		return false;
	}
...
bool FSonySaveGameSystem::GetSaveGameParameter (const TCHAR* SaveName, EGameDelegates_SaveGame Key, char *KeyValue, int KeyValueSize)
{
	FString Value;
...
bool FSonySaveGameSystem::GetSaveGameParameter (const char * SaveName, EGameDelegates_SaveGame Key, char * KeyValue, int KeyValueSize)
{
...
bool FSonySaveGameSystem::DeleteSavedGame()
{
	...
	UE_LOG(LogSonySaveGame, Warning, TEXT("sceSaveDataDelete failed in DeleteSavedGame(). Error code 0x%x. MountDirName='%s'"), result, UTF8_TO_TCHAR(MountDirName.data));
	return false;
}

int32 FSonySaveGameSystem::SaveArrayToSceFile(const TArray<uint8>& Array, const TCHAR *Filename)
{
	int32 Result;
	...
	SceKernelAioResult AioResult;
	SceKernelAioRWRequest Request;
	Request.offset = 0;
	Request.buf = const_cast<uint8*>(Data);
	Request.nbyte = DataLength;
	Request.result = &AioResult;

	Request.fd = sceKernelOpen(Utf8Path, SCE_KERNEL_O_RDWR | SCE_KERNEL_O_TRUNC | SCE_KERNEL_O_CREAT, SCE_KERNEL_S_IRWU);
	if (Request.fd < SCE_OK)
	{
		return Request.fd;
	}

	SceKernelAioSubmitId Id;
	verify(sceKernelAioSubmitWriteCommands(&Request, 1, SCE_KERNEL_AIO_PRIORITY_MID, &Id) == SCE_OK);

	int32 State = 0;
	verify(sceKernelAioWaitRequest(Id, &State, nullptr) == SCE_OK);
	verify(State == SCE_KERNEL_AIO_STATE_COMPLETED);
	verify(AioResult.state == SCE_KERNEL_AIO_STATE_COMPLETED);

	verify(sceKernelAioDeleteRequest(Id, &Result) == SCE_OK);
	verify(Result == SCE_OK);

	sceKernelClose(Request.fd);
	return AioResult.returnValue >= SCE_OK ? SCE_OK : AioResult.returnValue;
}

int32 FSonySaveGameSystem::LoadSceFileToArray(TArray<uint8>& Array, const TCHAR *Filename)
{
	int32 Result;
	...
	SceKernelStat Stat;
	Result = sceKernelStat(Utf8Path, &Stat);
	if (Result < SCE_OK)
	{
		return Result;
	}

	const int64 FileSize = Stat.st_size;
	...
	Request.offset = 0;
	Request.buf = const_cast<uint8*>(Data);
	Request.nbyte = FileSize;
	Request.result = &AioResult;

	Request.fd = sceKernelOpen(Utf8Path, SCE_KERNEL_O_RDONLY, SCE_KERNEL_S_INONE);
	if (Request.fd < 0)
	{
		return Request.fd;
	}
	...
	SceKernelAioSubmitId Id;
	verify(sceKernelAioSubmitReadCommands(&Request, 1, SCE_KERNEL_AIO_PRIORITY_MID, &Id) == SCE_OK);
