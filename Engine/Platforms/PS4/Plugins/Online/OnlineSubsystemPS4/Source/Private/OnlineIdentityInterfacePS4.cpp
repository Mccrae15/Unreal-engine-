...
public:
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncGetAuthCode bWasSuccessful: %d"), WasSuccessful());
	}
	...
	virtual void Finalize() override
	{
		...
		if (Token.Data.Len() != 6)
		{
			UE_LOG_ONLINE_IDENTITY(Warning, TEXT("PS4 Auth Token expected to be 6 characters, found %d in string %s"), Token.Data.Len(), *Token.Data);
		}
		else
		{
			for (int32 Index = 0; Index < Token.Data.Len(); ++Index)
			{
				...
				{
					UE_LOG_ONLINE_IDENTITY(Warning, TEXT("Encountered non alpha-numeric character in string %s."), *Token.Data);
					break;
				}
			}
		}
	...
	void DoWork()
	{
		...
		if (Privilege == EUserPrivileges::CanPlayOnline)
		{
			...
			if (CheckNpResult < 0 && 
				PrivilegeFailures != 0)
			{
				UE_LOG_ONLINE_IDENTITY(Warning, TEXT("sceNpCheckNpAvailabilityA Failed : 0x%x, UserID: 0x%x"), CheckNpResult, SceUserId);
			...
			if (PS4Subsystem->IsPSNPlusRequired())
			{
				...
				if (CheckPSPlusResult < 0 &&
					PrivilegeFailures != 0)
				{
					UE_LOG_ONLINE_IDENTITY(Warning, TEXT("sceNpCheckPlus Failed : 0x%x, UserID: 0x%x"), CheckPSPlusResult, SceUserId);
					PrivilegeFailures |= (uint32)IOnlineIdentity::EPrivilegeResults::GenericFailure;
				}
			}
		}
		...
		if (Privilege == EUserPrivileges::CanCommunicateOnline || Privilege == EUserPrivileges::CanUseUserGeneratedContent)
		{
			...
			{
				UE_LOG_ONLINE_IDENTITY(Warning, TEXT("sceNpGetParentalControlInfoA Failed : 0x%x, UserID: 0x%x"), CheckParentalRet, SceUserId);
				PrivilegeFailures |= (uint32)IOnlineIdentity::EPrivilegeResults::GenericFailure;
			}
			else
			{
...
void FUserOnlineAccountPS4::UpdateNames()
{
	...
	else
	{
		...
		{
			UE_LOG_ONLINE_IDENTITY(Error, TEXT("sceUserServiceGetUserName Failed : 0x%x, UserID: 0x%x"), Result, SceUserId);
			Username = TEXT("");
		}
		else
		{
...
SceUserServiceUserId FOnlineIdentityPS4::GetSceUserId(int32 LocalUserIndex)
{
	...
	return FSonyApplication::GetSonyApplication()->GetUserID(LocalUserIndex);
}
...
FOnlineIdentityPS4::FOnlineIdentityPS4(FOnlineSubsystemPS4* InSubsystem)
...
{
	UE_LOG_ONLINE_IDENTITY(Log, TEXT("PS4 Online User Service Event - %s %s, UserIndex: %d, UserID: %d"), 
	...
	bool bForceAnnounceLoginStatusChanged = false;
	ELoginStatus::Type PreviousLoginStatus = Account->LoginStatus;
	if (bLoggingIn)
	{
		bForceAnnounceLoginStatusChanged = !Account->bAnnouncedLogin;
	...
	if (bForceAnnounceLoginStatusChanged || Account->LoginStatus != PreviousLoginStatus)
	{
		if (UserIndex != INDEX_NONE)
		{
			Account->bAnnouncedLogin = true;
...
bool FOnlineIdentityPS4::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	...
	UE_LOG_ONLINE_IDENTITY(Log, TEXT("FOnlineIdentityPS4::Login, UserIndex: %d, UserID: %d"),
		LocalUserNum,
		UserId);
	...
	if (Account.IsValid())
	{
		...
		else
		{
			...
			UE_LOG_ONLINE_IDENTITY(Warning, TEXT("No NpClientId specified. User will not receive an auth token."));
	...
	TriggerOnLoginCompleteDelegates(LocalUserNum, false, *FUniqueNetIdPS4::EmptyId(), TEXT("com.epicgames.identity.notloggedin"));
	return false;
}
...
bool FOnlineIdentityPS4::Logout(int32 LocalUserNum)
{
	...
	UE_LOG_ONLINE_IDENTITY(Log, TEXT("FOnlineIdentityPS4::Logout, UserIndex: %d, UserID: %d"),
		LocalUserNum,
		UserId);
...
{
	UE_LOG_ONLINE_IDENTITY(Warning, TEXT("FOnlineIdentityPS4::CreateUniquePlayerId(uint8*, int32) is not implemented."));
	return nullptr;
}
...
{
	UE_LOG_ONLINE_IDENTITY(Display, TEXT("FOnlineIdentityPS4::RevokeAuthToken not implemented"));
