...
#endif
{
	...
	EGnmSrgbMode SrgbMode = (!!(Flags & TexCreate_SRGB))
		? EGnmSrgbMode::Enable
		: EGnmSrgbMode::Default;
	...
	{
		CopyToResolveTargetLabel = FMemBlock::Allocate(8, 8, EGnmMemType::GnmMem_CPU, GET_STATID(STAT_Onion_Label));
		*reinterpret_cast<uint64*>(CopyToResolveTargetLabel.GetPointer()) = (uint64)ECopyToResolveState::Valid;
		FrameSubmitted = 0;
	...
	else if (Flags & TexCreate_CPUReadback)
	{
		GnmMemType = EGnmMemType::GnmMem_CPU;
	}
	...
	if (bRenderTarget)
	{
		...
		RenderTargetSpec.m_pitch = Texture->getPitch();	// we need to get the pitch from the texture itself to make sure we get the computed pitch itself, not what we passed in
...
bool FGnmSurface::TryAllocateDefraggable(uint32 AllocationSize, uint32 Alignment, TStatId StatId)
{
	...
	if (bDefraggable)
	{
		...
		{
			UE_LOG(LogSony, Warning, TEXT("Tried to load texture into defrag pool but not enough space.  Falling back to normal allocator."));
...
#endif
	FMemBlock::Free(CopyToResolveTargetLabel);
...
void* FGnmSurface::Lock(uint32 MipIndex, uint32 ArrayIndex, EResourceLockMode LockMode, uint32& DestStride)
{
	...
	const EPixelFormat PixelFormat = RHITexture->GetFormat();
	...
	const uint32 NumBlocksX = (TilingParams.m_linearWidth + BlockSizeX - 1) / BlockSizeX;
	const uint32 NumBlocksY = (TilingParams.m_linearHeight + BlockSizeY - 1) / BlockSizeY;
	...
	{	
		...
		uint64 OutSize = NumBlocksY * DestStride;
	...
	else if (TilingParams.m_tileMode == Gnm::kTileModeDisplay_LinearAligned)
	{
		...
		DestStride = SurfInfoOut.m_pitch * BlockBytes;
		
		int32 TiledHeight = SurfInfoOut.m_height;
		ensureMsgf(DestStride * TiledHeight == SurfInfoOut.m_surfaceSize, TEXT("DestStride: %i, TiledHeight: %i for Mip: %i of Texture: %i x %i, doesn't match OutSize: %i"), DestStride, TiledHeight, MipIndex, Texture->getWidth(), Texture->getHeight(), SurfInfoOut.m_surfaceSize);
	}
	else
	{
...
{
	if (CopyToResolveTargetLabel.GetPointer() != nullptr)
	{
		// Set the label to the "pending" state until the copy is complete by the GPU
		*reinterpret_cast<uint64*>(CopyToResolveTargetLabel.GetPointer()) = (uint64)ECopyToResolveState::Pending;
...
void FGnmSurface::EndCopyToResolveTarget(FGnmCommandListContext& GnmCommandContext)
{
	if (CopyToResolveTargetLabel.GetPointer() != nullptr)
	{
		...
		GnmCommandContext.GetContext().writeDataInline(CopyToResolveTargetLabel.GetPointer(), &ValidState, 2, Gnm::kWriteDataConfirmDisable);
	}
}
...
void FGnmSurface::BlockUntilCopyToResolveTargetComplete()
{
	if (CopyToResolveTargetLabel.GetPointer() != nullptr)
	{
		...
		volatile uint64* Label = reinterpret_cast<volatile uint64*>(CopyToResolveTargetLabel.GetPointer());
		while (*Label != (uint64)ECopyToResolveState::Valid)
		{
			scePthreadYield();
		}
	}
...
void FGnmSurface::ConvertColorToCMASKBits(sce::Gnm::DataFormat DataFormat, const FLinearColor& Color, uint32* Bits)
{
	...
	switch( RenderTargetFormat )
	{
		...
		case sce::Gnm::kRenderTargetFormat32:
		{
			FMemory::Memcpy(&Bits[0], &XVal, sizeof(uint32));
			Bits[1] = 0;
			break;
		}
		case sce::Gnm::kRenderTargetFormat32_32:
		{
			FMemory::Memcpy(&Bits[0], &XVal, sizeof(uint32));
			FMemory::Memcpy(&Bits[1], &YVal, sizeof(uint32));
			break;
		}
		...
		case sce::Gnm::kRenderTargetFormat32_32_32_32:
		{
			...
			checkf(false, TEXT("128 bit targets are not supported."));
			break;
		}
		default:
		{
...
uint64 FGnmDynamicRHI::RHICalcTexture2DPlatformSize(uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, uint32 NumSamples, uint32 Flags, const FRHIResourceCreateInfo& CreateInfo, uint32& OutAlign)
{	
	...
	// texture native format
	Gnm::DataFormat PlatformFormat = GGnmManager.GetDataFormat((EPixelFormat)Format, SrgbMode);
...
uint64 FGnmDynamicRHI::RHICalcTexture3DPlatformSize(uint32 SizeX, uint32 SizeY, uint32 SizeZ, uint8 Format, uint32 NumMips, uint32 Flags, const FRHIResourceCreateInfo& CreateInfo, uint32& OutAlign)
{
	// dummy texture to help compute the size.
	Gnm::Texture Texture;
...
uint64 FGnmDynamicRHI::RHICalcTextureCubePlatformSize(uint32 Size, uint8 Format, uint32 NumMips, uint32 Flags, const FRHIResourceCreateInfo& CreateInfo, uint32& OutAlign)
{
	// dummy texture to help compute the size.
	Gnm::Texture Texture;
...
uint32 FGnmDynamicRHI::RHIComputeMemorySize(FRHITexture* TextureRHI)
{
	if(!TextureRHI)
	{
		return 0;
...
void FGnmDynamicRHI::RHICopySharedMips(FRHITexture2D* DestTexture2D, FRHITexture2D* SrcTexture2D)
{
	check(0);
}

FTexture2DArrayRHIRef FGnmDynamicRHI::RHICreateTexture2DArray(uint32 SizeX, uint32 SizeY, uint32 SizeZ, uint8 Format, uint32 NumMips, uint32 NumSamples, uint32 Flags, FRHIResourceCreateInfo& CreateInfo)
{
	return new FGnmTexture2DArray((EPixelFormat)Format, SizeX, SizeY, SizeZ, NumMips, NumSamples, Flags, CreateInfo.BulkData, CreateInfo.ClearValueBinding);
}
...
void FGnmDynamicRHI::RHIGetResourceInfo(FRHITexture* Ref, FRHIResourceInfo& OutInfo)
{
}
...
FRHICOMMAND_MACRO(FRHICommandGnmAsyncReallocateTexture2D)
{
	...
	FORCEINLINE_DEBUGGABLE FRHICommandGnmAsyncReallocateTexture2D(FGnmTexture2D* InOldTexture, FGnmTexture2D* InNewTexture, FRHIComputeFence* InComputeFence, int32 InNewMipCount, int32 InNewSizeX, int32 InNewSizeY, FThreadSafeCounter* InRequestStatus)
...
FTexture2DRHIRef FGnmDynamicRHI::AsyncReallocateTexture2D_RenderThread(class FRHICommandListImmediate& RHICmdList, FRHITexture2D* OldTextureRHI, int32 NewMipCount, int32 NewSizeX, int32 NewSizeY, FThreadSafeCounter* RequestStatus)
{
	...
	ALLOC_COMMAND_CL(RHICmdList, FRHICommandGnmAsyncReallocateTexture2D)(OldTexture, NewTexture, MipCopyFence, NewMipCount, NewSizeX, NewSizeY, RequestStatus);
...
FTexture2DRHIRef FGnmDynamicRHI::RHIAsyncReallocateTexture2D(FRHITexture2D* OldTextureRHI, int32 NewMipCount, int32 NewSizeX, int32 NewSizeY, FThreadSafeCounter* RequestStatus)
{
...
ETextureReallocationStatus FGnmDynamicRHI::RHIFinalizeAsyncReallocateTexture2D(FRHITexture2D* Texture2D, bool bBlockUntilCompleted )
{
	FGnmTexture2D* NewTexture = ResourceCast(Texture2D);	
	return TexRealloc_Succeeded;
}

ETextureReallocationStatus FGnmDynamicRHI::RHICancelAsyncReallocateTexture2D(FRHITexture2D* Texture2D, bool bBlockUntilCompleted )
{
	FGnmTexture2D* NewTexture = ResourceCast(Texture2D);	
	return TexRealloc_Succeeded;
}
...
void* FGnmDynamicRHI::RHILockTexture2D(FRHITexture2D* TextureRHI,uint32 MipIndex,EResourceLockMode LockMode,uint32& DestStride,bool bLockWithinMiptail)
{
	FGnmTexture2D* Texture = ResourceCast(TextureRHI);
	return Texture->Surface.Lock(MipIndex, 0, LockMode, DestStride);
}

void FGnmDynamicRHI::RHIUnlockTexture2D(FRHITexture2D* TextureRHI,uint32 MipIndex,bool bLockWithinMiptail)
{
	FGnmTexture2D* Texture = ResourceCast(TextureRHI);
	Texture->Surface.Unlock(MipIndex, 0);
}

void* FGnmDynamicRHI::RHILockTexture2DArray(FRHITexture2DArray* TextureRHI,uint32 TextureIndex,uint32 MipIndex,EResourceLockMode LockMode,uint32& DestStride,bool bLockWithinMiptail)
{
	FGnmTexture2DArray* Texture = ResourceCast(TextureRHI);
	return Texture->Surface.Lock(MipIndex, TextureIndex, LockMode, DestStride);
}

void FGnmDynamicRHI::RHIUnlockTexture2DArray(FRHITexture2DArray* TextureRHI,uint32 TextureIndex,uint32 MipIndex,bool bLockWithinMiptail)
{
	FGnmTexture2DArray* Texture = ResourceCast(TextureRHI);
...
{	
	...
	default:
		UE_LOG(LogSony, Warning, TEXT("Unsupported texture update format: %i"), OutSrcDataFormat.m_bits.m_surfaceFormat);
		break;
	}
}

template<uint32 NumComponents>
void BindShaderUpdateTexture2D(FGnmCommandListContext& CmdContext, uint32 DstX, uint32 DstY, uint32 Width, uint32 Height, uint32 SrcPitch, const Gnm::Buffer& SourceBuffer, const Gnm::Texture& DestTexture)
{
	TShaderMapRef< TUpdateTexture2DSubresouceCS<NumComponents> > ComputeShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	FRHIComputeShader* ShaderRHI = ComputeShader.GetComputeShader();

	CmdContext.RHISetComputeShader(ShaderRHI);

	const uint32 DestPosSize[4] = { DstX, DstY, Width, Height };
	SetShaderValueOnContext(CmdContext, ShaderRHI, ComputeShader->DestPosSizeParameter, DestPosSize);
	SetShaderValueOnContext(CmdContext, ShaderRHI, ComputeShader->SrcPitchParameter, SrcPitch);

	GnmContextType& GnmContext = CmdContext.GetContext();
	GnmContext.setBuffers(Gnm::kShaderStageCs, ComputeShader->SrcBuffer.GetBaseIndex(), 1, &SourceBuffer);
	GnmContext.setRwTextures(Gnm::kShaderStageCs, ComputeShader->DestTexture.GetBaseIndex(), 1, &DestTexture);
}

void FGnmDynamicRHI::RHIUpdateTexture2D(FRHITexture2D* TextureRHI, uint32 MipIndex, const struct FUpdateTextureRegion2D& UpdateRegion, uint32 SourcePitch, const uint8* SourceData)
{	
	...
	const bool bIsBlockCompressed = DestDataFormat.isBlockCompressedFormat();
	const uint32 BytesPerElement = DestDataFormat.getTotalBytesPerElement();
	uint32 NumComponents = DestDataFormat.getNumComponents();
	if (bIsBlockCompressed)
	{
		if (BytesPerElement == 8u)
		{
			// 64 bit block (BC1, BC4)
			DestDataFormat.m_bits.m_surfaceFormat = Gnm::kSurfaceFormat16_16_16_16;
			NumComponents = 4u;
		}
		else if (BytesPerElement == 16u)
		{
			// 128 bit block (BC2,3,5,6h,7)
			DestDataFormat.m_bits.m_surfaceFormat = Gnm::kSurfaceFormat32_32_32_32;
			NumComponents = 4u;
		}
		else
		{
	...
	const uint32 ElementBlockSize = bIsBlockCompressed ? 4u : 1u;
	const uint32 UpdateWidthInBlocks = UpdateRegion.Width / ElementBlockSize;
	const uint32 UpdateHeightInBlocks = UpdateRegion.Height / ElementBlockSize;
	const uint32 UpdateDestXInBlocks = UpdateRegion.DestX / ElementBlockSize;
	const uint32 UpdateDestYInBlocks = UpdateRegion.DestY / ElementBlockSize;

	// must be block-aligned
	check(UpdateWidthInBlocks * ElementBlockSize == UpdateRegion.Width);
	check(UpdateHeightInBlocks * ElementBlockSize == UpdateRegion.Height);
	check(UpdateDestXInBlocks * ElementBlockSize == UpdateRegion.DestX);
	check(UpdateDestYInBlocks * ElementBlockSize == UpdateRegion.DestY);

	// Allocate some space for GPU to read from (needs to be GPU accessible, unlike SourceData)	
	const uint32 DataSize = SourcePitch == 0 ?
		UpdateWidthInBlocks * UpdateHeightInBlocks * BytesPerElement :
		SourcePitch * UpdateHeightInBlocks;
	void* TempBuffer = GGnmManager.AllocateFromTempRingBuffer(DataSize, DEFAULT_VIDEO_ALIGNMENT);
	FMemory::Memcpy(TempBuffer, SourceData, DataSize);

	checkf(SourcePitch % BytesPerElement == 0,
		TEXT("Padding needs to be in multiple of texels. SourcePitch=%d, BytesPerElement=%d"),
		SourcePitch,
		BytesPerElement);

	const uint32 SrcPitch = SourcePitch == 0 ? UpdateWidthInBlocks : (SourcePitch / BytesPerElement);
	...
	int32 NumSrcElements = SourcePitch == 0 ?
		UpdateWidthInBlocks * UpdateHeightInBlocks :
		(SourcePitch / BytesPerElement) * UpdateHeightInBlocks;
	...
	DestTextureCopy.setWidthMinus1(Texture->GetSizeX() / ElementBlockSize - 1u);
	DestTextureCopy.setHeightMinus1(Texture->GetSizeY() / ElementBlockSize - 1u);
	DestTextureCopy.setPitchMinus1(Texture->Surface.Texture->getPitch() / ElementBlockSize - 1u);
	
	// Setup the compute shader
	switch (NumComponents)
	{
	case 1u: BindShaderUpdateTexture2D<1u>(CmdContext, UpdateDestXInBlocks, UpdateDestYInBlocks, UpdateWidthInBlocks, UpdateHeightInBlocks, SrcPitch, SourceBuffer, DestTextureCopy); break;
	case 2u: BindShaderUpdateTexture2D<2u>(CmdContext, UpdateDestXInBlocks, UpdateDestYInBlocks, UpdateWidthInBlocks, UpdateHeightInBlocks, SrcPitch, SourceBuffer, DestTextureCopy); break;
	case 3u: BindShaderUpdateTexture2D<3u>(CmdContext, UpdateDestXInBlocks, UpdateDestYInBlocks, UpdateWidthInBlocks, UpdateHeightInBlocks, SrcPitch, SourceBuffer, DestTextureCopy); break;
	case 4u: BindShaderUpdateTexture2D<4u>(CmdContext, UpdateDestXInBlocks, UpdateDestYInBlocks, UpdateWidthInBlocks, UpdateHeightInBlocks, SrcPitch, SourceBuffer, DestTextureCopy); break;
	default: check(false);
	}

	CmdContext.RHIDispatchComputeShader((UpdateWidthInBlocks + 7) / 8, (UpdateHeightInBlocks + 7) / 8, 1);
...
void FGnmDynamicRHI::RHIUpdateFromBufferTexture2D(FRHITexture2D* TextureRHI, uint32 MipIndex, const struct FUpdateTextureRegion2D& UpdateRegion, uint32 SourcePitch, FRHIStructuredBuffer* BufferRHI, uint32 BufferOffset)
{
	check(IsInRenderingThread() || IsInRHIThread());
	...
	CmdContext.PushMarker("UpdateTexture2D");
	FGnmTexture2D* Texture = ResourceCast(TextureRHI);
	FGnmStructuredBuffer* Buffer = ResourceCast(BufferRHI);
	const FGnmSurface& Surface = Texture->Surface;
	...
	// Set source buffer to use the same format as the destination
	Gnm::Buffer SourceBuffer = Buffer->Buffer;
	SourceBuffer.setFormat(DestDataFormat);
	SourceBuffer.setNumElements(NumSrcElements);
	SourceBuffer.setResourceMemoryType(Gnm::kResourceMemoryTypeRO);
...
void FGnmDynamicRHI::RHIUpdateTexture3D_Internal(FRHITexture3D* TextureRHI, uint32 MipIndex, const FUpdateTextureRegion3D& UpdateRegion, uint32 SourceRowPitch, uint32 SourceDepthPitch, void* SourceDataGarlic)
{
	FGnmCommandListContext& CmdContext = GGnmManager.GetImmediateContext();
	...
	int32 NumSrcElements = SourceDepthPitch == 0 ?
		...
		(SourceDepthPitch / BytesPerTexel) * UpdateRegion.Depth;
	AdjustTextureUpdateSrcFormat(DestDataFormat, SrcDataFormat, NumSrcElements);
...
void FGnmDynamicRHI::RHIUpdateTexture3D(FRHITexture3D* TextureRHI,uint32 MipIndex,const FUpdateTextureRegion3D& UpdateRegion,uint32 SourceRowPitch,uint32 SourceDepthPitch,const uint8* SourceData)
{
	const FGnmTexture3D* Texture = ResourceCast(TextureRHI);
	const FGnmSurface& Surface = Texture->Surface;
	...
	void* TempBuffer = GGnmManager.AllocateFromTempRingBuffer(DataSize, DEFAULT_VIDEO_ALIGNMENT, FGnmManager::ETempRingBufferAllocateMode::AllowFailure);
...
void* FGnmDynamicRHI::RHILockTextureCubeFace(FRHITextureCube* TextureCubeRHI,uint32 FaceIndex,uint32 ArrayIndex,uint32 MipIndex,EResourceLockMode LockMode,uint32& DestStride,bool bLockWithinMiptail)
{
...
void FGnmDynamicRHI::RHIUnlockTextureCubeFace(FRHITextureCube* TextureCubeRHI,uint32 FaceIndex,uint32 ArrayIndex,uint32 MipIndex,bool bLockWithinMiptail)
{
	FGnmTextureCube* TextureCube = ResourceCast(TextureCubeRHI);
	uint32 GnmFace = GetGnmCubeFace((ECubeFace)FaceIndex);
...
void FGnmDynamicRHI::RHIBindDebugLabelName(FRHITexture* TextureRHI, const TCHAR* Name)
{
...
void FGnmDynamicRHI::RHIVirtualTextureSetFirstMipInMemory(FRHITexture2D* TextureRHI, uint32 FirstMip)
{
	// TODO: Implement for PS4
	UE_LOG(LogSony, Fatal, TEXT("RHIVirtualTextureSetFirstMipInMemory(FRHITexture2D* TextureRHI, uint32 FirstMip) needs to be implemented"));
}

void FGnmDynamicRHI::RHIVirtualTextureSetFirstMipVisible(FRHITexture2D* TextureRHI, uint32 FirstMip)
{
	// TODO: Implement for PS4
	UE_LOG(LogSony, Fatal, TEXT("RHIVirtualTextureSetFirstMipVisible(FRHITexture2D* TextureRHI, uint32 FirstMip) needs to be implemented"));
}
...
void FGnmCommandListContext::RHIUpdateTextureReference(FRHITextureReference* TextureRefRHI, FRHITexture* NewTextureRHI)
{
