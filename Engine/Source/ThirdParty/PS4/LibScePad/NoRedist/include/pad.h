...
/**
 *E
 *  @brief It is a function which gets opened handle by the arguments.
 *
 *  @param [in] userId : The UserID which opened controller port.
 *                        
 *  @param [in] type   : Type of device
 *                       - SCE_PAD_PORT_TYPE_STANDARD
 *                       - SCE_PAD_PORT_TYPE_SPECIAL
...
/**
 * @j 対象となるハンドルが存在しない @ej
 * @e There is no handle. @ee
 */
