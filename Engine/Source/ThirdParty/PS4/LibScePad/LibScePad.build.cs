		...
		if (Target.Platform == UnrealTargetPlatform.Win64)
		{
			...
			if (bFoundPadLib)
			{
				...
				if (Target.Configuration == UnrealTargetConfiguration.Debug && Target.bDebugBuildsActuallyUseDebugCRT)
				{
					...
					PublicAdditionalLibraries.Add(Path.Combine(PadLibLocation, DebugStaticLibSubDir, DebugStaticLib2015Name));
				}
				else
				{
					// Release runtime
					WhitelistRestrictedFolders.Add(Path.GetFullPath(Path.Combine(PadLibLocation, StaticLibSubDir)));
					PublicAdditionalLibraries.Add(Path.Combine(PadLibLocation, StaticLibSubDir, StaticLib2015Name));
				}
			}
		}
	}
