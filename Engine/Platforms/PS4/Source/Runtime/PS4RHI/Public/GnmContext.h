...
class FGnmCommandListContext : public IRHICommandContextPSOFallback
{
private:
...
public:		
	...
	*/
	...
	void SetCurrentRenderTarget(uint32 RTIndex, FRHITexture* RTTexture, bool CMASKRequired, uint32 MipIndex = 0, uint32 SliceIndex = -1);
	...
	FRHITexture* GetCurrentRenderTargetTexture(uint32 RTIndex)
	{
		return CurrentRenderTargetTextures[RTIndex];
	}

	void SetCurrentDepthTarget(FRHITexture* DepthTexture, bool HTILERequired, bool bReadOnly);

	FRHITexture* GetCurrentDepthTarget()
	{
	...
	*/
	void EliminateFastClear(FRHITexture* Target);
	...
	*/
	void DecompressFmaskSurface(FRHITexture* Target);

	/**
	* Push the HTile bits to the depth buffer so it can be read
	*/
	void DecompressDepthTarget(FRHITexture* DepthTexture);

	/**
	* Rebuilds the depth target HTILE meta data.
	*/
	...
	*/
	...
	FORCEINLINE void SetViewportAndGuardBand(float MinX, float MinY, float MinZ, float MaxX, float MaxY, float MaxZ, float ZScale, float ZOffset);

	FORCEINLINE void SetViewport(float MinX, float MinY, float MinZ, float MaxX, float MaxY, float MaxZ);
	
	FORCEINLINE void SetStereoViewport(float LeftMinX, float RightMinX, float LeftMinY, float RightMinY, float MinZ, float LeftMaxX, float RightMaxX, float LeftMaxY, float RightMaxY, float MaxZ);

	/**
	* Helper function to set a full screen viewport and bypass the caching system
	*/
	FORCEINLINE void SetViewportUncached(float MinX, float MinY, float MinZ, float MaxX, float MaxY, float MaxZ, float ZScale, float ZOffset);
	...
	*/
	...
	virtual void RHIResummarizeHTile(FRHITexture2D* DepthTexture) final override;
	...
	*/
	FORCEINLINE void RememberBoundShaderState(FRHIBoundShaderState* BoundShaderStateRHI)
	{
...
private:
	...
	static void ClearUAV(TRHICommandList_RecursiveHazardous<FGnmCommandListContext>& RHICmdList, FGnmUnorderedAccessView* UnorderedAccessView, const void* ClearValues, bool bFloat);
	...
	void SetRenderTargets_Impl(uint32 NumSimultaneousRenderTargets, const FRHIRenderTargetView* NewRenderTargets, const FRHIDepthRenderTargetView* NewDepthStencilTarget, bool bHWColorRequired, bool bHWDepthRequired);
	...
	void ClearColorTargetsByHardware(TRHICommandList_RecursiveHazardous<FGnmCommandListContext>& RHICmdList, int32 NumClearColors, const FLinearColor* ClearColorArray);
	...
	void SetupMSAARenderState(Gnm::NumSamples NumAASamples);
	...
	TSet<FRHITexture*> TargetsNeedingEliminateFastClear;
	
	Gnm::DepthRenderTarget CurrentDepthRenderTarget;
	FRHITexture* CurrentDepthTarget;
	FRHITexture* CurrentRenderTargetTextures[MaxSimultaneousRenderTargets];
	...
	Gnm::RenderOverrideControl CachedRenderOverrideControl;
	...
	Gnm::NumSamples CachedNumSamples;
	...
	FUniformBufferRHIRef BoundUniformBuffers[SF_NumStandardFrequencies][FGnmContextCommon::MAX_UNIFORM_BUFFERS_PER_SHADER_STAGE];
