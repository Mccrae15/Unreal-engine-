...
void FOnlineAsyncTaskPS4EnumerateUserFiles::FetchAccessoryData()
{
	...
	if (Result < 0)
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusCreateRequest() failed. Error code: 0x%08x"), Result);
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusGetMultiSlotDataStatusAAsync() failed. Error code: 0x%08x"), Result);
...
void FOnlineAsyncTaskPS4EnumerateUserFiles::Tick()
{
	...
	if (Result == 0)
	{
			...
			for (int i = 0; i < CloudPtr->SlotCount; ++i)
			{
				...
				if (Status.hasData)
				{
					...
					if (DirEntry->Magic1 == DE_MAGIC_1 && DirEntry->Magic2 == DE_MAGIC_2)
					{
						...
						{
							UE_LOG_ONLINE_CLOUD(Warning, TEXT("UserCloud directory data version is incorrect. Want %d, got %d. Skipping entry"), DE_VERSION, DirEntry->Version);
						}
					}
					else
					{
						UE_LOG_ONLINE_CLOUD(Error, TEXT("Malformed UserCloud directory data. Skipping entry"));
					}
		...
		{
			// There was an error.
			UE_LOG_ONLINE_CLOUD(Error, TEXT("sceNpTusGetMultiSlotDataStatusAAsync returned an error. Error code: 0x%08x"), OperationResult);
			bWasSuccessful = false;
		}
	...
	{
		// There was an error.
		UE_LOG_ONLINE_CLOUD(Error, TEXT("sceNpTusPollAsync failed. Error code: 0x%08x"), Result);
...
void FOnlineAsyncTaskPS4EnumerateUserFiles::Finalize() 
{
	...
	if (WasSuccessful()) 
	{
		...
		if (FileMetaDataPtr) 
		{
			FUniqueNetIdString id(UserId->ToString(), PS4_SUBSYSTEM);
	...
	if (RequestId >= 0)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusDeleteRequest() failed. Error code: 0x%08x"), Result);
		}
...
void FOnlineAsyncTaskPS4ReadUserFile::Tick()
{
	...
	if (CurrentPhase == SEND_REQUEST)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusGetDataAAsync() failed. Error code: 0x%08x"), Result);
			// The call was not successfully launched, so return,
			// which leaves the call complete and unsuccessful
	...
	if (Result == 0)
	{
		...
		case READ_STATUS: // fetched status
			if (OperationResult < 0)
			{
				UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusGetDataAAsync() returned an error. Error code: 0x%08x"), OperationResult);
				bIsComplete = true;
				return;  // isComplete(true), wasSuccessful(false)
			}
	...
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusPollAsync() failed. Error code: 0x%08x"), Result);
		bIsComplete = true;
	}
}
...
void FOnlineAsyncTaskPS4WriteUserFile::WriteUserFile(const FUniqueNetId& InUserId, const FString& InFileToWrite, const TArray<uint8>& InContents)
{
	...
	if (Result < 0)
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusCreateRequest failed for filename \"%s\". Error code: 0x%08x"), *FileName, Result);
		return;  // bIsComplete(true), bWasSuccessful(false)
	}
	RequestId = Result;
...
void FOnlineAsyncTaskPS4WriteUserFile::Tick()
{
	...
	if (!bWasRequestSent)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusSetDataAAsync failed for filename \"%s\". Error code: 0x%08x"), *FileName, Result);
			bIsComplete = true;
			return;
		}
	...
	if (Result == 0)
	{
		...
		{
			UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusSetDataAAsync failed() for filename \"%s\". Error code: 0x%08x"), *FileName, OperationResult);
			bWasSuccessful = false;
		}
	...
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusPollAsync failed() for filename \"%s\". Error code: 0x%08x"), *FileName, Result);
		bWasSuccessful = false;
		bIsComplete = true;
	}
}
...
void FOnlineAsyncTaskPS4WriteUserFile::Finalize()
{
	...
	if (RequestId >= 0)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusDeleteRequest failed for filename \"%s\". Error code: 0x%08x"), *FileName, Result);
		}
...
void FOnlineUserCloudPS4::ConfigureSlots()
{
	...
	if (Count == 0)
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("No TUS slots configured. Edit PS4Engine.ini after configuring slots in the SMT for the Title User Storage"));
	}
...
bool FOnlineUserCloudPS4::Init()
{
	...
	if ( Result != SCE_OK )
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_NP_TUS) failed. Error code: 0x%08x"), Result);
		return false;
	}
...
void FOnlineUserCloudPS4::Shutdown()
{
	...
	if (Result != SCE_OK )
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceSysmoduleUnloadModule(SCE_SYSMODULE_NP_TUS) failed. Error code: 0x%08x"), Result);
	}
}
...
FCloudFilePS4 *FOnlineUserCloudPS4::FindCachedFileForSlot(const FUniqueNetId& UserId, int SlotId, bool CreateIfNeeded)
{
	...
	if (SlotId > 0)
	{
		...
		FUniqueNetIdString IdString(UserId.ToString(), PS4_SUBSYSTEM);
...
bool FOnlineUserCloudPS4::WriteUserFile(const FUniqueNetId& UserId, const FString& FileName, TArray<uint8>& FileContents, bool bCompressBeforeUpload)
{
...
{
	UE_LOG_ONLINE_CLOUD(Verbose, TEXT("PS4 UserCloud has %d files configured"), SlotCount);
	...
	{
		UE_LOG_ONLINE_CLOUD(Verbose, TEXT("User does not have any cached files"));
	}

	if (FileSetPtr)
	{
		UE_LOG_ONLINE_CLOUD(Verbose, TEXT("User has %d cached files."), FileSetPtr->Num());
	}
}
...
void FOnlineUserCloudPS4::DumpCloudFileState(const FUniqueNetId& UserId, const FString& FileName)
{
	if (FileName.Len() > 0)
	{
		UE_LOG_ONLINE_CLOUD(Log, TEXT("Cloud File State file %s:"), *FileName);
		{
			...
			if (SlotId <= 0)
			{
				UE_LOG_ONLINE_CLOUD(Log, TEXT("\tFile is not configured!"));
			}
			else
			{
				UE_LOG_ONLINE_CLOUD(Log, TEXT("\tFile is configured for slot %d"), SlotId);
			}

			FCloudFilePS4 *FilePtr = FindCachedFileForSlot(UserId, SlotId);
			if (FilePtr)
			{
				UE_LOG_ONLINE_CLOUD(Log, TEXT("\tFileCache: Filename:%s  CacheSize:%d  SlotId:%d  LastModifiedTick: %#lx"), 
					*FilePtr->FileName, FilePtr->Data.Num(), FilePtr->SlotId, FilePtr->TimeLastChanged.tick);
			}
			else
			{
				UE_LOG_ONLINE_CLOUD(Log, TEXT("\tNo cached content found!"));
			}
		}
	}
}
...
void FOnlineAsyncTaskPS4DeleteUserFile::IssueDeleteRequest()
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusDeleteMultiSlotDataAAsync() failed for file \"%s\". Error code: 0x%08x"), *FileName, Result);
		return;  // isComplete(true), wasSuccessful(false)
	}
...
void FOnlineAsyncTaskPS4DeleteUserFile::Tick()
{
	...
	if (Result == 0)
	{
		...
		{
			UE_LOG_ONLINE_CLOUD(Warning, TEXT("sceNpTusPollAsync failed() for deleting filename \"%s\". Error code: 0x%08x"), *FileName, OperationResult);
			bWasSuccessful = false;
		}
