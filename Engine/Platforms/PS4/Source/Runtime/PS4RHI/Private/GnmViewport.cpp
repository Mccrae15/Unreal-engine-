...
{
	GGnmManager.CreateBackBuffers(InSizeX, InSizeY);
}
...
void FGnmDynamicRHI::RHIResizeViewport(FRHIViewport* ViewportRHI,uint32 SizeX,uint32 SizeY,bool bIsFullscreen)
{
	check( IsInGameThread() );
...
void FGnmCommandListContext::RHIBeginDrawingViewport(FRHIViewport* ViewportRHI, FRHITexture* RenderTargetRHI)
{
	check(IsImmediate());
//	FGnmViewport* Viewport = ResourceCast(ViewportRHI);
...
void FGnmCommandListContext::RHIEndDrawingViewport(FRHIViewport* ViewportRHI, bool bPresent, bool bLockToVsync)
{
	check(IsImmediate());
//	FGnmViewport* Viewport = ResourceCast(ViewportRHI);
...
FTexture2DRHIRef FGnmDynamicRHI::RHIGetViewportBackBuffer(FRHIViewport* ViewportRHI)
{
//	FGnmViewport* Viewport = ResourceCast(ViewportRHI);
