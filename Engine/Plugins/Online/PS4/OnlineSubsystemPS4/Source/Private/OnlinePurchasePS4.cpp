...
public:
	...
	virtual void Tick() override
	{
		if (!bIsComplete && !bIsBlockingTaskDone)
		{
			...
			{
				UE_LOG_ONLINE(Warning, TEXT("Commerce::getServiceEntitlements returned 0x%08x (sync)"), Ret);
				return FailTask(Ret);
			}
		}
		...
		if (!bIsComplete)
		{
			...
			if (!Entitlements.isLocked())
			{
				...
				if (Entitlements.getReturnCode() != SCE_TOOLKIT_NP_V2_SUCCESS)
				{
					...
					UE_LOG_ONLINE(Warning, TEXT("Commerce::getServiceEntitlements returned 0x%08x (async)"), ErrorCode);
...
protected:
	virtual void FailTask(const int32 ErrorCodeInt)
	{
		...
		FormatArguments.Add(TEXT("ErrorCode"), FText::FromString(FString::Printf(TEXT("0x%0.8X"), ErrorCodeInt)));
		...
		RequestStatus.SetFromErrorMessage(ErrorMessageText, ErrorCodeInt);
	...
	{
		UE_LOG_ONLINE(Error, TEXT("FOnlinePurchasePS4::Checkout - %s"), *ErrorCodeString);
...
protected:
	...
	bool IsCheckoutComplete()
	{
		...
		else
		{
			...
			if (Ret < 0)
			{
				UE_LOG_ONLINE(Warning, TEXT("Commerce::displayCheckoutDialog returned 0x%08x"), Ret);
				FailTask(Ret);
			}
	...
	virtual void DoBlockingTask() override
	{
		...
		if (Ret != SCE_TOOLKIT_NP_V2_SUCCESS)
		{
			UE_LOG_ONLINE(Warning, TEXT("Commerce::getProducts returned 0x%08x"), Ret);
			return FailTask(Ret);
		}
...
protected:
	virtual void DoBlockingTask() override
	{
		...
		if (Ret != SCE_TOOLKIT_NP_V2_SUCCESS)
		{
			UE_LOG_ONLINE(Warning, TEXT("Commerce::Interface::voucherCodeInput returned 0x%08x"), Ret);
			return FailTask(Ret);
		}
	}
