	...
	if(!Direct3DDevice)
	{
		...
		if( IsRHIDeviceNVIDIA() )
		{
			...
			if (SLIStatus == NVAPI_OK)
			{
				...
				{
					GNumAlternateFrameRenderingGroups/*GNumActiveGPUsForRendering*/ = SLICaps.numAFRGroups;
					UE_LOG(LogD3D11RHI, Log, TEXT("Detected %i SLI GPUs Setting GNumActiveGPUsForRendering to: %i."), SLICaps.numAFRGroups, GNumAlternateFrameRenderingGroups/*GNumActiveGPUsForRendering*/);
				}
			}
			else
			{
		...
		{
			GNumAlternateFrameRenderingGroups/*GNumActiveGPUsForRendering*/ = GDX11ForcedGPUs;
