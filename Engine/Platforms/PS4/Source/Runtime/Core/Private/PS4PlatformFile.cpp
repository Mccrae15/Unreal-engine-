...
/**
 * Turn a UE4 filename into a PS4 filename. DO NOT EVER STORE THE RETURN FROM THIS! USE AS TEMPORARY/RVALUE ONLY!
 */
#define TO_PS4_FILENAME(Filename) (TCHAR_TO_UTF8(*NormalizeFileName(Filename)))
#define TO_PS4_DIRECTORYNAME(Filename) (TCHAR_TO_UTF8(*NormalizeDirectory(Filename)))
...
public:
	...
	{
		return sceFiosFHSyncSync(nullptr, FileHandle) == SCE_FIOS_OK;
	}

	virtual bool Truncate(int64 NewSize) override
	{
		return sceFiosFHTruncateSync(nullptr, FileHandle, NewSize) == SCE_FIOS_OK;
	}
...
FPS4PlatformFile::FPS4PlatformFile()
{
	...
	// initialize the AIO (Async IO) subsystem
	SceKernelAioParam AioParam;
	sceKernelAioInitializeParam(&AioParam);
	verify(sceKernelAioInitialize(&AioParam) == SCE_OK);
...
#if UE_BUILD_SHIPPING && (PRESERVE_LOG_BACKUPS_IN_SHIPPING == 0)

		// Limit the size of the /temp0/ directory to prevent log backups etc. from causing
		// full disk errors. We don't want to do this all the time, so check the size first
		// and only format if there is less than a certain threshold available.

		// Sony makes no guarantees about the /temp0/ directory between game launches, and
		// this is early boot code so formatting now doesn't make a lot of difference vs.
		// the OS having done this anyway.

		const size_t MinTempDataAvailableThresholdKb = 512 * 1024; // 512 MB

		size_t AvailableSpace = 0;
		if (sceAppContentTemporaryDataGetAvailableSpaceKb(&GPS4Temp0MountPoint, &AvailableSpace) == SCE_OK
			&& AvailableSpace < MinTempDataAvailableThresholdKb)
		{
			sceAppContentTemporaryDataFormat(&GPS4Temp0MountPoint);
		}
...
const FString* FPS4PlatformFile::GetOverrideLogDirectory()
{
#if PS4_FORCE_LOGS_SCREENSHOTS_TO_DEVICE

	// In non-shipping builds, use the standard log directory (nullptr = don't override).
	// The path handling in NormalizeFilename will ensure logs go to /data/
	return nullptr;

#else

	// In shipping, logs go to /temp0/
	static FString LogDir = GPS4TempDirectory + TEXT("/");
	return &LogDir;
...
{
	int Result = sceFiosDirectoryCreateSync(nullptr, TO_PS4_DIRECTORYNAME(Directory));
...
{
	int Result = sceFiosDirectoryDeleteSync(nullptr, TO_PS4_DIRECTORYNAME(Directory));
...
{
	int Result = sceFiosFileDeleteSync(nullptr, TO_PS4_FILENAME(Filename));
...
bool FPS4PlatformFile::DirectoryExists( const TCHAR* Directory )
{
	...
	bool bExists = sceFiosDirectoryExistsSync(nullptr, TO_PS4_DIRECTORYNAME(Directory));	
	return bExists;
}
...
{
	bool bExists = sceFiosFileExistsSync(nullptr, TO_PS4_FILENAME(Filename));	
	return bExists;
}
...
{
	int64 Size = sceFiosFileGetSizeSync(nullptr, TO_PS4_FILENAME(Filename));
...
FDateTime FPS4PlatformFile::GetAccessTimeStamp( const TCHAR* Filename )
{
	...
	if (sceFiosStatSync(nullptr, TO_PS4_FILENAME(Filename), &Stat) == SCE_FIOS_OK)
	{
...
FFileStatData FPS4PlatformFile::GetStatData(const TCHAR* FilenameOrDirectory)
{
	...
	if (sceFiosStatSync(nullptr, TO_PS4_FILENAME(FilenameOrDirectory), &Stat) == SCE_FIOS_OK)
	{
...
{
	return (sceFiosRenameSync(nullptr, TO_PS4_FILENAME(From), TO_PS4_FILENAME(To)) == SCE_FIOS_OK);
}
...
IFileHandle* FPS4PlatformFile::OpenWrite( const TCHAR* Filename, bool bAppend , bool bAllowRead )
{
	...
	// open the file
	int Result = sceFiosFHOpenSync(nullptr, &FileHandle, TO_PS4_FILENAME(Filename), &Params);
	...
	else
	{
		...
		//UE_LOG(LogSony, Warning, TEXT("Failed to open file for write: %s, error: 0x%x"), *NormalizeDirectory(Filename), Result);
	}
...
bool FPS4PlatformFile::SetReadOnly( const TCHAR* Filename, bool bNewReadOnlyValue )
{
	...
	if (sceKernelStat(TO_PS4_FILENAME(Filename), &Stat) == SCE_OK)
	{
		...
		return (sceKernelChmod(TO_PS4_FILENAME(Filename), Stat.st_mode) == SCE_OK);
	}
...
void FPS4PlatformFile::SetTimeStamp( const TCHAR* Filename, FDateTime DateTime )
{
	...
	if (sceKernelStat(TO_PS4_FILENAME(Filename), &Stat) == SCE_OK)
	{
		...
		Times[1].tv_sec = static_cast<time_t>((DateTime - PS4Epoch).GetTotalSeconds());

		// ignore microseconds for files
		Times[0].tv_usec = Times[1].tv_usec = 0;

		// update the time
		sceKernelUtimes(TO_PS4_FILENAME(Filename), Times);
	}
}
...
{
	return TO_PS4_FILENAME(Filename);
}
...
FString FPS4PlatformFile::NormalizeFileName(const TCHAR* FileName, bool bDirectory, bool bShortenForDeepFiles)
{
	...
	if (Result.StartsWith(TEXT("/host/")) || Result.StartsWith(TEXT("/av_contents/")))
	{
		// Leave "/host/..." and "/av_contents/..." paths unmodified.
		return Result;
	}
...
bool FPS4PlatformFile::IterateDirectoryCommon( const TCHAR* Directory, const TFunctionRef<bool(const FString&, const FString&, struct dirent*)>& Visitor, const TFunctionRef<bool(const FString&)>& VisitorDeepSubDir )
{
	...
	// open the directory	
	int32 Handle = sceKernelOpen(TO_PS4_DIRECTORYNAME(Directory), SCE_KERNEL_O_DIRECTORY | SCE_KERNEL_O_RDONLY, SCE_KERNEL_S_IRU);
...
FString FPS4PlatformFile::StripRelativePath(const FString& Path)
{
	...
	{
		RelativePath.RightInline(RelativePath.Len() - (DotDotSlashIdx + 3), false);
	...
	{
		RelativePath.RightInline(RelativePath.Len() - GPS4SandboxedFileRootDirectory.Len(), false);
	}

	static FString App0Str(TEXT("/app0"));
	if (RelativePath.StartsWith(App0Str, ESearchCase::CaseSensitive))
	{
		RelativePath.RightInline(RelativePath.Len() - 5, false);
	}
	...
	{
		RelativePath.RightInline(RelativePath.Len() - 1, false);
	}

	if (RelativePath.EndsWith(Slash, ESearchCase::CaseSensitive))
	{
		RelativePath.LeftChopInline(1, false);
	}
...
void GetParentPath(const FString& ChildPath, FString& OutParentPath)
{
	...
	{
		OutParentPath.LeftInline(Offset, false);
	}
	else
	{
		OutParentPath = "";
