...
public:
		...
		, RequestedOfferIds(InOfferIds)
	{
		...
		UE_LOG_ONLINE_STOREV2(Log, TEXT("Querying PS4 Store for %d Offers"), RequestedOfferIds.Num());

		// Fill in the product IDs array
		for (int32 OfferIndex = 0; OfferIndex < RequestedOfferIds.Num(); ++OfferIndex)
		{
			UE_LOG_ONLINE_STOREV2(Log, TEXT("  Offer %d: %s"), OfferIndex, *(RequestedOfferIds[OfferIndex]));
	...
	virtual bool HandleResult(NpToolkit::Commerce::Products const& Result) override
	{
		...
		if (bWrongOfferCount)
		{
			UE_LOG_ONLINE_STOREV2(Warning, TEXT("Incorrect amount of PS4 Store offers found; expected %d, found %d offers"), RequestedOfferIds.Num(), Result.numProducts);
