...
typedef TSharedPtr<class FOnlineTournamentPS4, ESPMode::ThreadSafe> FOnlineTournamentPS4Ptr;

using FNpWebApiUserContext = int32;
...
protected:
	...
	/** Interface to the online tournament services */
	FOnlineTournamentPS4Ptr TournamentInterface;

	/** Online async task runnable */
	FOnlineAsyncTaskManagerPS4* OnlineAsyncTaskThreadRunnable;
	...
	/** Delegate handle for the join tournament event */
	FDelegateHandle OnTournamentJoinEventDelegateHandle;
	/** Delegate handle for the join tournament match event */
	FDelegateHandle OnTournamentJoinMatchEventDelegateHandle;
	/** Delegate handle for the join tournament team match event */
	FDelegateHandle OnTournamentJoinTeamMatchEventDelegateHandle;
...
public:
	 ...
	 */
	FNpWebApiUserContext GetUserWebApiContext(const FUniqueNetIdPS4& NetUserIdPS4);
	...
	*/
	...
	/**
	 * @return true if the SubSystem has been successfully initialized
	 */
	const bool IsFullyInitialized() const { return bFullyInitialized; }

PACKAGE_SCOPE:
	FOnlineAsyncTaskManagerPS4* GetAsyncTaskManager() { return OnlineAsyncTaskThreadRunnable; }
	...
	FORCEINLINE void CreateAndDispatchAsyncEvent(TArguments&&... Arguments)
	{
		...
		static_assert(TIsDerivedFrom<TOnlineAsyncEvent, FOnlineAsyncEvent<FOnlineSubsystemPS4>>::IsDerived, "Argument TOnlineAsyncEvent must derive from FOnlineAsyncEvent");
