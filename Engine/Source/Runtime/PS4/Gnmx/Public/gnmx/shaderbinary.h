...
namespace sce
{
	namespace Gnmx
	{
		...
		public:
			...
			uint8_t m_dispatchDrawIndexDeallocNumBits	: 5;	///< 0 if not dispatch draw or [1:31] for dispatch draw. Match index value (<c>0xFFFF & (0xFFFF << m_dispatchDrawIndexDeallocNumBits)</c>) if 16-bit IRB or (<c>0xFFFFFFFF & (0xFFFFFFFF << m_dispatchDrawIndexDeallocNumBits)</c>) if 32-bit IRB.
			...
			uint8_t m_version;									///< 0 if pre-versioning non-dispatch-draw or dispatch draw IRB-only with DispatchDrawTriangleCullData format input data; 1 for first revised version, etc. (v7.2: added from reserved bits) 
