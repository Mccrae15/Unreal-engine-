...
bool UBlueprintJsonLibrary::JsonHasField(const FBlueprintJsonObject& JsonObject, const FString& FieldName, FBlueprintJsonValue& ValueOut)
{
	if (JsonObject.Object.IsValid())
	{
		if (JsonObject.Object->HasField(FieldName))
		{
			ValueOut.Value = JsonObject.Object->GetField<EJson::None>(FieldName);
			return true;
		}
	}
	return false;
