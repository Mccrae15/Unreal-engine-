...
{
	NumUVChannels = FMath::Clamp(NumUVChannels, 1, (int32)GeometryCollectionUV::MAX_NUM_UV_CHANNELS);
...
void UFractureAutoUVSettings::ChangeNumUVChannels(int32 Delta)
{
	...
	{
		UFractureToolAutoUV* AutoUVTool = Cast<UFractureToolAutoUV>(OwnerTool);
		AutoUVTool->UpdateUVChannels(Target);
	}
}

// The underlying box project algo isn't backported yet
...
{
	// This feature isn't backported yet
...
bool UFractureToolAutoUV::SaveGeneratedTexture(TImageBuilder<FVector4f>& ImageBuilder, FString ObjectBaseName, const UObject* RelativeToAsset, bool bPromptToSave, bool bAllowReplace)
{
...
void UFractureToolAutoUV::BakeTextureForComponent(UGeometryCollectionComponent* Component, TFunction<void(int32, const FText&)> Progress)
{
	...
	TImageBuilder<FVector4f> ImageBuilder;
