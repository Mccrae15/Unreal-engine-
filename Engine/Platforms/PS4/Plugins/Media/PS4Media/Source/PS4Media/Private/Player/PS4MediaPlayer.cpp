...
bool FPS4MediaPlayer::InitializePlayer(const TSharedPtr<FArchive, ESPMode::ThreadSafe>& Archive, const FString& Url, bool Precache, const FMediaPlayerOptions* PlayerOptions)
{
	...
	Async(Execution, [Archive, CallbacksPtr = TWeakPtr<FPS4MediaCallbacks, ESPMode::ThreadSafe>(Callbacks), Precache, Url, CurrentShouldLoop]()
	{
		TSharedPtr<FPS4MediaCallbacks, ESPMode::ThreadSafe> PinnedCallbacks = CallbacksPtr.Pin();

		if (PinnedCallbacks.IsValid())
		{
			PinnedCallbacks->Initialize(Archive, Url, Precache, CurrentShouldLoop);
		}
	});
