...
void FOnlineAsyncTaskPS4_CreateSession::DoWork()
{
	...
	if (!Data.bWasSuccessful && Data.Session)
	{
		...
		FOnlineAsyncTaskPS4_DestroySession DestroyTask(PS4Subsystem, UserWebApiContext, *Data.Session, FOnDestroySessionCompleteDelegate());
		DestroyTask.DoWork();
		SendResults();
		return;
	}
...
void FOnlineAsyncTaskPS4_DestroySession::DoWork()
{
	...
	if (AsyncTaskManager)
	{
		auto NewEvent = new FAsyncEventPS4_DestroySessionComplete(PS4Subsystem, Session.SessionName, bWasSuccessful, CompletionDelegate);
		AsyncTaskManager->AddToOutQueue(NewEvent);
	}
}
...
void FOnlineAsyncTaskPS4_JoinSession::DoWork()
{
	...
	{
		FAsyncEventPS4_JoinSessionComplete* NewEvent = new FAsyncEventPS4_JoinSessionComplete(PS4Subsystem, Session.SessionName, DesiredSession, Result);
		AsyncTaskManager->AddToOutQueue(NewEvent);
	}
}
...
*/
	if (!Session.SessionInfo.IsValid())
	{
		...
		FAsyncEventPS4_LeaveSessionComplete* const NewEvent = new FAsyncEventPS4_LeaveSessionComplete(PS4Subsystem, Session.SessionName, false, CompletionDelegate);
		AsyncTaskManager->AddToOutQueue(NewEvent);
		return;
	}
	...
	FAsyncEventPS4_LeaveSessionComplete* const NewEvent = new FAsyncEventPS4_LeaveSessionComplete(PS4Subsystem, Session.SessionName, bWasSuccessful, CompletionDelegate);
	AsyncTaskManager->AddToOutQueue(NewEvent);
}
...
FOnlineAsyncTaskPS4_LeaveRoom::FOnlineAsyncTaskPS4_LeaveRoom(FOnlineSubsystemPS4& PS4Subsystem, FNamedOnlineSession& InSession, const FOnDestroySessionCompleteDelegate& InDestroySessionDelegate, SceNpMatching2RequestId InRequestId)
	: FOnlineAsyncTaskPS4(&PS4Subsystem)
	, Session(InSession)
...
FOnlineAsyncTaskPS4_SendSessionInvite::FOnlineAsyncTaskPS4_SendSessionInvite(FOnlineSubsystemPS4& PS4Subsystem, const FUniqueNetIdPS4Ref& InUserId, const FNamedOnlineSession& InSession, const TArray<TSharedRef<const FUniqueNetId>>& InFriends)
	: FOnlineAsyncTaskPS4(&PS4Subsystem)
{
