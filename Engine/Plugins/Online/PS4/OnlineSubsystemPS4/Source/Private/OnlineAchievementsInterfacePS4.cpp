...
public:
		...
		, Result(false)
	{
		...
		{
			Result.SetFromErrorCode(RequestId);
		}
	}
	...
	virtual void Tick() override
	{
		if (!Response.isLocked())
		{
			...
			else
			{
				...
				Result.SetFromErrorCode(ReturnCode);
			}
			UE_LOG_ONLINE(Log, TEXT("FOnlineAsyncTaskPS4RegisterTrophyPack: User=[%s] registering with the trophy service completed with result=[%s]"), *LocalUserId->ToDebugString(), Result.ToLogString());

			FOnlineAchievementsPS4Ptr AchievementsPtr = StaticCastSharedPtr<FOnlineAchievementsPS4>(Subsystem->GetAchievementsInterface());
			check(AchievementsPtr.IsValid()); // Must have achievements to create this task
			ETrophyServiceRegistrationStatus NewStatus = Result.bSucceeded ? ETrophyServiceRegistrationStatus::Registered : ETrophyServiceRegistrationStatus::Failed;
...
public:
	...
	virtual void TickTrophyRequest() override
	{
		if (!UnlockedTrophiesResponse.isLocked())
		{
			...
			UE_LOG_ONLINE(Log, TEXT("FOnlineAsyncTaskPS4QueryUnlockedTrophies: User=[%s] query unlocked trophies completed with result=[%s]"), *LocalUserId->ToDebugString(), Result.ToLogString());
	...
	virtual void Finalize() override
	{
		...
		if (Result.bSucceeded)
		{
			...
			UE_LOG_ONLINE(VeryVerbose, TEXT("FOnlineAsyncTaskPS4QueryUnlockedTrophies: user=[%s] has %d unlocked trophies"), *LocalUserId->ToDebugString(), NumUnlockedTrophies);
			...
			for (int32 UnlockedTrophyIndex = 0; UnlockedTrophyIndex < NumUnlockedTrophies; ++UnlockedTrophyIndex)
			{
				...
				if (Achievement)
				{
					UE_LOG_ONLINE(VeryVerbose, TEXT("FOnlineAsyncTaskPS4QueryUnlockedTrophies: user=[%s] has trophy=%d unlocked"), *LocalUserId->ToDebugString(), static_cast<int32>(TrophyId));
					Achievement->Progress = 100.0;
				}
				else
				{
					UE_LOG_ONLINE(Verbose, TEXT("FOnlineAsyncTaskPS4QueryUnlockedTrophies: Unlocked trophyid=%d not found in the trophy map"), static_cast<int32>(TrophyId));
				}
			}
	...
	virtual void TriggerDelegates() override
	{
		...
		CompletionDelegate.ExecuteIfBound(*LocalUserId, Result.bSucceeded);
	}
...
public:
	...
	void StartTrophiesQuery()
	{
		...
		UE_LOG_ONLINE(Verbose, TEXT("FOnlineAsyncTaskPS4QueryTrophyDescriptions: User=[%s] query trophy descriptions summary completed with %d trophies, beginning the individual trophy lookup"), *LocalUserId->ToDebugString(), NumTrophies);
		...
		for (int32 TrophyIndex = 0; TrophyIndex < NumTrophiesToQuery; ++TrophyIndex)
		{
			...
			if (RequestId < SCE_TOOLKIT_NP_V2_SUCCESS)
			{
				UE_LOG_ONLINE(Warning, TEXT("FOnlineAsyncTaskPS4QueryTrophyDescriptions: User=[%s] query trophy description for trophyid=%d failed to start with result=0x%08x"), *LocalUserId->ToDebugString(), TrophyIndex, RequestId);
		...
		for (FTrophyPackTrophyQuery& TrophyPackTrophyQuery : TrophyPackTrophyQueries)
		{
			...
			if (ResponseState == NpToolkit::Core::ResponseState::ready)
			{
				...
				if (ReturnCode == SCE_TOOLKIT_NP_V2_SUCCESS)
				{
					...
					UE_LOG_ONLINE(VeryVerbose, TEXT("FOnlineAsyncTaskPS4QueryTrophyDescriptions: Queried trophy details for trophyid=%d title=[%s] description=[%s] hidden=%d"),
				...
				{
					UE_LOG_ONLINE(Warning, TEXT("FOnlineAsyncTaskPS4QueryTrophyDescriptions: Queried trophy details for trophyid=%d failed with result=0x%08x"), static_cast<int32>(TrophyPackTrophyQuery.TrophyId), ReturnCode);
			...
			if (ResponseState == NpToolkit::Core::ResponseState::reset && TrophyIdsAwaitingQuery.Num() > 0)
			{
				...
				{
					UE_LOG_ONLINE(Warning, TEXT("FOnlineAsyncTaskPS4QueryTrophyDescriptions: User=[%s] query trophy description for trophyid=%d failed to start with result=0x%08x"), *LocalUserId->ToDebugString(), NextTrophyId, RequestId);
...
public:
		...
		, CompletionDelegate(InCompletionDelegate)
	{
		...
		if (UE_LOG_ACTIVE(LogOnline, Verbose))
		{
			UE_LOG_ONLINE(Verbose, TEXT("FOnlineAsyncTaskPS4UnlockTrophies: Unlocking %d trophies"), TrophyIds.Num());
			int32 TrophyIdCounter = 0;
			for (const SceNpTrophyId TrophyId : TrophyIds)
			{
				UE_LOG_ONLINE(Verbose, TEXT("  %d=%d"), TrophyIdCounter, static_cast<int32>(TrophyId));
				++TrophyIdCounter;
			}
		}
	}
	...
	virtual void TickTrophyRequest() override
	{
		if (!Response.isLocked())
		{
			...
			UE_LOG_ONLINE(Log, TEXT("FOnlineAsyncTaskPS4UnlockTrophies: User=[%s] unlock trophy=%d completed with returncode=0x%08x"), *LocalUserId->ToDebugString(), static_cast<int32>(TrophyId), ReturnCode);
...
protected:
	...
	void UnlockNextTrophy()
	{
		if (TrophyIds.IsValidIndex(UnlockingTrophyIndex))
		{
			...
			UE_LOG_ONLINE(Log, TEXT("Starting unlock request for trophyid=%d"), static_cast<int32>(TrophyId));
			...
			if (RequestId < SCE_TOOLKIT_NP_V2_SUCCESS)
			{
				...
				UE_LOG_ONLINE(Warning, TEXT("Failed to start unlock request for trophyid=%d with result=0x%08x"), static_cast<int32>(TrophyId), RequestId);
				Result.SetFromErrorCode(RequestId);
				FailedToUnlockTrophyIds.Add(TrophyId);
			}
		}
	...
	: PS4Subsystem(InSubsystem)
{
	...
	{
		UE_LOG_ONLINE(Error, TEXT("FOnlineSubsystemPS4::FOnlineAchievementsPS4: Load module failed with result=0x%08x"), Result);
	}
...
void FOnlineAchievementsPS4::Shutdown()
{
	if (bTrophyModuleLoaded)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE(Error, TEXT("FOnlineSubsystemPS4::Shutdown: Unload module failed with result=0x%08x"), Result);
		}
		bTrophyModuleLoaded = false;
	}
}

bool FOnlineAchievementsPS4::LoadAndInitFromJsonConfig(const TCHAR* const JsonConfigName)
{
	bool bResult = false;
	...
	if (FFileHelper::LoadFileToString(JsonText, *JsonConfigFilename))
	{
		...
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4: Failed to parse json filename=[%s] jsontext=[%s]"), *JsonConfigFilename, *JsonText);
		}
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4: Failed to load config filename=[%s]"), *JsonConfigFilename);
	}
...
void FOnlineAchievementsPS4::WriteAchievements(const FUniqueNetId& PlayerId, FOnlineAchievementsWriteRef& WriteObject, const FOnAchievementsWrittenDelegate& Delegate)
{
	...
	for (const TPair<FName, FVariantData> It : WriteObject->Properties)
	{
		...
		const SceNpTrophyId* const TrophyId = AchievementsConfig.AchievementMap.Find(AchievementId);

		if (TrophyId == nullptr)
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4::WriteAchievements: No mapping for achievement=[%s]"), *AchievementId);
			continue;
		}
		...
		if (GetCachedAchievement(PlayerId, AchievementId, ExistingAchievement) == EOnlineCachedResult::Success)
		{
			...
			if (bAlreadyUnlocked)
			{
				UE_LOG_ONLINE(VeryVerbose, TEXT("FOnlineAchievementsPS4::WriteAchievements: User=[%s] already has achievement=[%s] unlocked trophyid=%d, not unlocking again"), *PS4User.ToDebugString(), *AchievementId, static_cast<int32>(*TrophyId));
				continue;
			}
		}
...
EOnlineCachedResult::Type FOnlineAchievementsPS4::GetCachedAchievement(const FUniqueNetId& PlayerId, const FString& AchievementId, FOnlineAchievement& OutAchievement)
{
	...
	if (Achievements)
	{
		...
		if (TrophyId)
		{
			...
			{
				UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4::GetCachedAchievement: No trophy entry found for achievement=[%s]"), *AchievementId);
			}
		}
		else
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4::GetCachedAchievement: No mapping for achievement=[%s]"), *AchievementId);
		}
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4::GetCachedAchievement: Achievements have not been read for player=[%s], call QueryAchievements first"), *PlayerId.ToDebugString());
	}
	return Result;
}
...
EOnlineCachedResult::Type FOnlineAchievementsPS4::GetCachedAchievementDescription(const FString& AchievementId, FOnlineAchievementDesc& OutAchievementDesc)
{
	...
	if (TrophyId)
	{
		...
		{
			UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4::GetCachedAchievementDescription: Achievements have not been read for id=[%s] trophyid=%d"), *AchievementId, static_cast<int32>(*TrophyId));
		}
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineAchievementsPS4::GetCachedAchievementDescription: No mapping for achievement=[%s]"), *AchievementId);
	}
	return Result;
}
...
{
	UE_LOG_ONLINE(Warning, TEXT("ResetAchievements is not implemented for PS4"));
	return false;
};
...
{
#if !UE_BUILD_SHIPPING
	UE_LOG_ONLINE(Log, TEXT("FOnlineAchievementsPS4: Begin"));
	UE_LOG_ONLINE(Log, TEXT("  bTrophyModuleLoaded=%d"), static_cast<int32>(bTrophyModuleLoaded));
	...
	UE_LOG_ONLINE(Log, TEXT("  RegistrationStatus: Begin"));
	for (const TPair<SceUserServiceUserId, ETrophyServiceRegistrationStatus>& Status : RegistrationStatus)
	{
		UE_LOG_ONLINE(Log, TEXT("    %d=[%s]"), static_cast<int32>(Status.Key), LexToCString(Status.Value));
	}
	UE_LOG_ONLINE(Log, TEXT("  RegistrationStatus: End"));

	UE_LOG_ONLINE(Log, TEXT("  AchievementsConfig (JSON config file): Begin"));
	for (const TPair<FString, int32>& ConfigAchievement : AchievementsConfig.AchievementMap)
	{
		UE_LOG_ONLINE(Log, TEXT("    [%s]=%d"), *ConfigAchievement.Key, ConfigAchievement.Value);
	}
	UE_LOG_ONLINE(Log, TEXT("  AchievementsConfig: End"));

	UE_LOG_ONLINE(Log, TEXT("  UserAchievements: Begin"));
	for (const TPair<SceUserServiceUserId, FOnlineUserAchievementsMapPS4>& User : UserAchievements)
	{
		UE_LOG_ONLINE(Log, TEXT("    User=%d: Begin"), static_cast<int32>(User.Key));
		for (const TPair<SceNpTrophyId, FOnlineAchievement>& Achievement : User.Value)
		{
			UE_LOG_ONLINE(Log, TEXT("      TrophyId=%d"), static_cast<int32>(Achievement.Key));
			UE_LOG_ONLINE(Log, TEXT("      Value=[%s]"), *Achievement.Value.ToDebugString());
		}
		UE_LOG_ONLINE(Log, TEXT("    User=%d: End"), static_cast<int32>(User.Key));
	}
	UE_LOG_ONLINE(Log, TEXT("  UserAchievements: End"));

	UE_LOG_ONLINE(Log, TEXT("  AchievementDescriptions: Begin"));
	for (const TPair<SceNpTrophyId, FOnlineAchievementDesc>& Description : AchievementDescriptions)
	{
		UE_LOG_ONLINE(Log, TEXT("    TrophyId=%d"), static_cast<int32>(Description.Key));
		UE_LOG_ONLINE(Log, TEXT("    Value=[%s]"), *Description.Value.ToDebugString());
	}
	UE_LOG_ONLINE(Log, TEXT("  AchievementDescriptions: End"));

	UE_LOG_ONLINE(Log, TEXT("FOnlineAchievementsPS4: End"));
#endif // !UE_BUILD_SHIPPING
}
...
void FOnlineAchievementsPS4::SetUserTrophyRegistrationStatus(const FUniqueNetIdPS4& UserId, const ETrophyServiceRegistrationStatus Status)
{
	...
	{
		UE_LOG_ONLINE(Log, TEXT("Setting trophy registration status for user=[%s] from [%s] to [%s]"), *UserId.ToDebugString(), LexToCString(PreviousStatus), LexToCString(Status));
...
void FOnlineAchievementsPS4::UpdateAchievementProgress(const FUniqueNetIdPS4& UserId, const SceNpTrophyId TrophyId, const double NewProgress)
{
	...
	if (FoundUserAchievements)
	{
		...
		if (Achievement)
		{
			UE_LOG_ONLINE(VeryVerbose, TEXT("UpdateAchievementProgress: user=[%s] achievementid=%d updated to new progress=%f"), *UserId.ToDebugString(), static_cast<int32>(TrophyId), NewProgress);
			Achievement->Progress = NewProgress;
		}
		else
		{
			UE_LOG_ONLINE(Warning, TEXT("UpdateAchievementProgress: user=[%s] achievementid=%d not found"), *UserId.ToDebugString(), static_cast<int32>(TrophyId));
		}
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("UpdateAchievementProgress: user=[%s] not found"), *UserId.ToDebugString());
	}
}
