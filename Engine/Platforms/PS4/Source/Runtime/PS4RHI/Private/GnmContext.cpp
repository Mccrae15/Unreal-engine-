...
FGnmCommandListContext::FGnmCommandListContext(bool bInIsImmediate)
	...
	, CurrentVertexShaderStage(Gnm::kShaderStageCs)
	...
	, CachedNumSamples(Gnm::kNumSamples1)
	...
	, EndOfSubmissionTimestamp(nullptr)
{
	...
	CachedRenderOverrideControl.init();
...
{	
	...
	GnmContext->setRenderOverrideControl(CachedRenderOverrideControl);
	...
	SetupMSAARenderState(CachedNumSamples);
...
void FGnmCommandListContext::InitializeStateForFrameStart()
{
	...
	if (CVarInstancedStereo && CVarInstancedStereo->GetValueOnAnyThread())
	{
...
static inline Gnm::DataFormat TranslateElementType(EVertexElementType Type)
{
	...
	case VET_PackedNormal:	return Gnm::kDataFormatR8G8B8A8Snorm;
	...
	case VET_UInt:			return Gnm::kDataFormatR32Uint;
...
void FGnmCommandListContext::SetupMSAARenderState(Gnm::NumSamples NumAASamples)
{
	if (NumAASamples != CachedNumSamples)
{
		CachedNumSamples = NumAASamples;

		Gnm::DepthEqaaControl EqaaReg;
		EqaaReg.init();
		EqaaReg.setPsSampleIterationCount(Gnm::kNumSamples1);
		EqaaReg.setMaskExportNumSamples(CachedNumSamples);
		EqaaReg.setMaxAnchorSamples(CachedNumSamples);
		EqaaReg.setAlphaToMaskSamples(CachedNumSamples);
		if (CachedNumSamples > Gnm::kNumSamples1)
	{
	...
	else
	{
		...
		GnmContext->setAaDefaultSampleLocations(CachedNumSamples);
	}
}

void FGnmCommandListContext::SetCurrentRenderTarget(uint32 RTIndex, FRHITexture* RTTexture, bool CMASKRequired, uint32 MipIndex, uint32 SliceIndex)
{
	...
	if (RTTexture)
	{
		...
		if (RT)
		{
			SetupMSAARenderState(RT->NumSamples);
		}
	}
...
void FGnmCommandListContext::SetCurrentDepthTarget(FRHITexture* DepthTexture, bool HTILERequired, bool bReadOnly)
{
	...
	if (DepthTexture)
	{
		...
		SetupMSAARenderState(Depth->NumSamples);
	}
	else
	{
...
void FGnmCommandListContext::SetTextureForStage(FRHITexture* NewTextureRHI, uint32 TextureIndex, Gnm::ShaderStage Stage)
{
	if (NewTextureRHI)
	{
...
void FGnmCommandListContext::SetSRVForStage(FRHIShaderResourceView* SRVRHI, uint32 TextureIndex, Gnm::ShaderStage Stage)
{
	...
	if (SRV)
	{
		...
		else
		{
			...
			if (IsValidRef(SRV->SourceMultiBufferRHI))
			{
				char* BufferData = (char*)SRV->SourceMultiBufferResource->GetCurrentBuffer(false);
				SRV->Buffer.setBaseAddress(BufferData + SRV->BufferOffset);
			}
			// set the texture into the texture register
...
{
	// cache it
	for(FPixelShaderUAVToFlushPair& Flush : PixelShaderUAVToFlush)
	{
		if(UAVAddress == Flush.Pointer)
		{
			check(Flush.Size == Size);
			return;
		}
	}
...
void FGnmCommandListContext::StoreBoundUAVs(bool bFlushGfx, bool bFlushCompute)
{
	for (int i = 0; i < BoundUAVs.Num(); ++i)
	{
		if (BoundUAVs[i])
		{
			...
			BoundUAVs[i]->SetResourceDirty(true, GGnmManager.GetFrameCount());
		}
	}
...
void FGnmCommandListContext::FlushBeforeComputeShader()
{
...
#if 0
	// The code below doesn't actually do anything - using RB flushes for UAVs.
	// We can't use waitForGraphicsWrites to wait on a UAV write, only an RTV write.
	{
	...
	// tell the CP to flush the L1$ and L2$
	GnmContext->flushShaderCachesAndWait(Gnm::kCacheActionWriteBackAndInvalidateL1andL2, 0, Gnm::kStallCommandBufferParserDisable);
...
{
	TShaderMapRef<FClearReplacementCS_Buffer_Uint> ComputeShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
	...
	GetContext().setRwBuffers(Gnm::kShaderStageCs, ComputeShader->GetResourceParamIndex(), 1, &DestinationBuffer);

	SetShaderValue(RHICmdList, ShaderRHI, ComputeShader->GetClearValueParam(), Value);

	RHICmdList.DispatchComputeShader(FMath::DivideAndRoundUp(NumDwords, ComputeShader->ThreadGroupSizeX), 1, 1);
...
*/
void FGnmCommandListContext::EliminateFastClear(FRHITexture* Target)
{	
#if USE_CMASK
...
void FGnmCommandListContext::DecompressFmaskSurface(FRHITexture* Target)
{
	// We are bypassing the cue so make sure there is enough space for the upcoming commands
	GnmContext->CommandBufferReserve();
...
*  so we don't trash the current state.
*/
void FGnmCommandListContext::DecompressDepthTarget(FRHITexture* DepthTexture)
{
#if USE_HTILE
	// We are bypassing the cue so make sure there is enough space for the upcoming commands
	GnmContext->CommandBufferReserve();
	...
	*Label = 0;
	...
	SetupMSAARenderState(Surface.NumSamples);
...
void FGnmCommandListContext::ResummarizeHTile(FRHITexture* DepthTexture)
{
#if USE_HTILE
	FGnmSurface& Surface = GetGnmSurfaceFromRHITexture(DepthTexture);
	...
	GnmContext->pushMarker("ResummarizeHTile");

	// Wait for all graphics depth writes to complete
	GnmContext->waitForGraphicsWrites(
		DepthTarget->getHtileAddress256ByteBlocks(),
		DepthTarget->getHtileSliceSizeInBytes() >> 8,
		Gnm::kWaitTargetSlotDb,
		Gnm::kCacheActionNone, Gnm::kExtendedCacheActionFlushAndInvalidateDbCache,
		Gnm::kStallCommandBufferParserDisable);

	GnmContext->waitForGraphicsWrites(
		DepthTarget->getZWriteAddress256ByteBlocks(),
		DepthTarget->getZSliceSizeInBytes() >> 8,
		Gnm::kWaitTargetSlotDb,
		Gnm::kCacheActionNone, Gnm::kExtendedCacheActionFlushAndInvalidateDbCache,
		Gnm::kStallCommandBufferParserDisable);
	...
	// Depth related render state config
	DCB->setDepthStencilDisable();
	DCB->setDepthRenderTarget(DepthTarget);
	const uint32 Width = DepthTarget->getWidth();
	const uint32 Height = DepthTarget->getHeight();
	SetViewportUncached(0, 0, 0, Width, Height, 1.0f, 0.5f, 0.5f);
	DCB->setScreenScissor(0, 0, Width, Height);
	DCB->setWindowScissor(0, 0, Width, Height, Gnm::kWindowOffsetDisable);
	DCB->setGenericScissor(0, 0, Width, Height, Gnm::kWindowOffsetDisable);
	...
	// Enable HTILE resummarization
	Gnm::DbRenderControl DBRenderControl;
	DBRenderControl.init();
	DBRenderControl.setHtileResummarizeEnable(true);
	DCB->setDbRenderControl(DBRenderControl);

	// Read all Z tiles (force the Z data to be read unless it is being overwritten)
	Gnm::RenderOverrideControl ROControl;
	ROControl.init();
	ROControl.setForceZValid(true);
	DCB->setRenderOverrideControl(ROControl);
	...
	// Make it safe to sample depth in a subsequent pass
	volatile uint32* Label = (uint32*)GnmContext->allocateFromCommandBuffer(sizeof(uint32), Gnm::kEmbeddedDataAlignment8);
	*Label = 0;
...
void FGnmCommandListContext::RestoreCachedDCBState()
{
	...
	DCB->setDepthStencilControl(CachedDepthStencilState);
	...
	DCB->setRenderOverrideControl(CachedRenderOverrideControl);
...
void FGnmCommandListContext::SetResourcesFromTables(const ShaderType* RESTRICT Shader, Gnm::ShaderStage ShaderStage)
{
	...
	while (DirtyBits)
	{
		...
		if(ShaderStage == Gnm::kShaderStagePs)
		{
			FGnmContextCommon::SetShaderResourcesFromBuffer_UAV(*this, ShaderStage, Buffer, Shader->ShaderResourceTable.UnorderedAccessViewMap.GetData(), BufferIndex);
		}
	}
...
void* FGnmCommandListContext::AllocateFromTempFrameBuffer(uint32 Size, uint32 Alignment)
{
	...
	{
		return TempFrameAllocator.Allocate(Size, Alignment);
	}

	return GGnmManager.AllocateFromTempRingBuffer(Size, Alignment);
}

uint64* FGnmCommandListContext::AllocateBeginCmdListTimestamp()
{
	ensure(StartOfSubmissionTimestamp == nullptr);
	StartOfSubmissionTimestamp = reinterpret_cast<uint64*>(DCBAllocator.Allocate(sizeof(uint64), 8));
...
uint64* FGnmCommandListContext::AllocateEndCmdListTimestamp()
{
	...
	EndOfSubmissionTimestamp = reinterpret_cast<uint64*>(DCBAllocator.Allocate(sizeof(uint64), 8));
