...
FGPUFenceRHIRef FGnmDynamicRHI::RHICreateGPUFence(const FName &Name)
{
	return new FGnmGPUFence(Name);
}

FStagingBufferRHIRef FGnmDynamicRHI::RHICreateStagingBuffer(FVertexBufferRHIParamRef VertexBuffer)
{
	return new FGnmStagingBuffer(VertexBuffer);
}

void* FGnmDynamicRHI::RHILockStagingBuffer(FStagingBufferRHIParamRef StagingBuffer, uint32 Offset, uint32 SizeRHI)
{
	FGnmStagingBuffer* Buffer = ResourceCast(StagingBuffer);
	return Buffer->Lock(Offset, SizeRHI);
}

void FGnmDynamicRHI::RHIUnlockStagingBuffer(FStagingBufferRHIParamRef StagingBuffer)
{
	FGnmStagingBuffer* Buffer = ResourceCast(StagingBuffer);
	Buffer->Unlock();
}

void* FGnmDynamicRHI::LockStagingBuffer_RenderThread(class FRHICommandListImmediate& RHICmdList, FStagingBufferRHIParamRef StagingBuffer, uint32 Offset, uint32 SizeRHI)
{
	return LockVertexBuffer_RenderThread(RHICmdList, StagingBuffer->GetBackingBuffer(), Offset, SizeRHI, RLM_ReadOnly);
}

void FGnmDynamicRHI::UnlockStagingBuffer_RenderThread(class FRHICommandListImmediate& RHICmdList, FStagingBufferRHIParamRef StagingBuffer)
{
	UnlockVertexBuffer_RenderThread(RHICmdList, StagingBuffer->GetBackingBuffer());
}
...
bool FGnmDynamicRHI::RHIGetRenderQueryResult(FRenderQueryRHIParamRef QueryRHI,uint64& OutNumPixels,bool bWait)
{
	...
	if (!Query->bResultIsCached)
	{
		...
		else if (Query->QueryType == RQT_AbsoluteTime)
		{
			...
			if (bSuccess)
			{
				uint64 GPUTiming = (*Query->TimeStampResults);
