...
{
/*
	UE_LOG(LogSony, Log, TEXT("WebRequest: User %d"), UserIndex);
	UE_LOG(LogSony, Log, TEXT("          : Action '%s'"), *Action);
	UE_LOG(LogSony, Log, TEXT("          : URL '%s'"), *URL);
	UE_LOG(LogSony, Log, TEXT("          : Params:"));
	for (auto It = Params.CreateConstIterator(); It; ++It)
	{
		UE_LOG(LogSony, Log, TEXT("  %s = '%s'"), *It.Key(), *It.Value());
	}
*/
...
bool FPS4CompanionServer::StartServer()
{
	...
	// start with defaults
	SceCompanionHttpdOptParam2 Param;
	sceCompanionHttpdOptParamInitialize2(&Param);
	...
	SIZE_T DataSize = TempDirectory.Len() + 1;
	Param.workDirectory = new ANSICHAR[DataSize];
	TCString<ANSICHAR>::Strcpy(const_cast<ANSICHAR*>(Param.workDirectory), DataSize, TCHAR_TO_ANSI(*TempDirectory));

	// initialize the server
	int32_t Ret = sceCompanionHttpdInitialize2(&Param);
	if (Ret != SCE_OK)
	{
		return false;
	}
										  ...
										  SceCompanionHttpdResponse* HttpResponse, void* UserData)
{
	...
	// @todo: The UserId is possibly different than the UserIndex (see PS4InputInterface.cpp)
	...
	if (sceCompanionHttpdGetEvent(&Event) == SCE_OK)
	{
		...
		if (Event.event == SCE_COMPANION_HTTPD_EVENT_CONNECT)
		{
			if (NumConnections == 0)
			{
				...
				if (Result != SCE_OK) 
				{
					UE_LOG(LogSony, Log, TEXT("Failed to sceCompanionHttpdStart, ret = %x"), Result);
				}
				Result = sceCompanionHttpdRegisterRequestCallback(RequestCallback, this);
				if (Result != SCE_OK) 
				{
					UE_LOG(LogSony, Log, TEXT("Failed to sceCompanionHttpdRegisterRequestCallback, ret = %x"), Result);
				}
			}
			NumConnections++;
		}
		else if (Event.event == SCE_COMPANION_HTTPD_EVENT_DISCONNECT)
		{
			...
			if (NumConnections == 0)
			{
				...
				if (Result != SCE_OK)
				{
					UE_LOG(LogSony, Log, TEXT("Failed to sceCompanionHttpdStop, ret = %x"), Result);
				}
			}
		}
	}
