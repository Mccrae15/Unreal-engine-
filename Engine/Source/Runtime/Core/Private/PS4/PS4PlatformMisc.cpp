...
ENetworkConnectionType FPS4Misc::GetNetworkConnectionType()
{
...
bool FPS4Misc::HasActiveWiFiConnection()
{
...
void FPS4Misc::BeginNamedEvent(const struct FColor& Color, const TCHAR* Text)
{
	if (IsRunningOnDevKit() && sceRazorCpuIsCapturing())
	{
		...
		{
			sceRazorCpuPushMarker(TCHAR_TO_ANSI(Text), GTlsProfilerColorStack.Last().ToPackedABGR(), 0);
		}
		else
		{
...
void FPS4Misc::BeginNamedEvent(const struct FColor& Color, const ANSICHAR* Text)
{
	if (IsRunningOnDevKit() && sceRazorCpuIsCapturing())
	{
		if (Color.DWColor() == 0 && GTlsProfilerColorStack.Num())
		{
			sceRazorCpuPushMarker(Text, GTlsProfilerColorStack.Last().ToPackedABGR(), 0);
		}
		else
		{
...
void FPS4Misc::BeginNamedEventStatic(const struct FColor& Color, const TCHAR* Text)
{
...
void FPS4Misc::BeginNamedEventStatic(const struct FColor& Color, const ANSICHAR* Text)
{
	if (IsRunningOnDevKit() && sceRazorCpuIsCapturing())
	{
		if (Color.DWColor() == 0 && GTlsProfilerColorStack.Num())
		{
			sceRazorCpuPushMarkerStatic(Text, GTlsProfilerColorStack.Last().ToPackedABGR(), 0);
		}
		else
		{
			sceRazorCpuPushMarkerStatic(Text, Color.ToPackedABGR(), 0);
		}
	}
}
...
void FPS4Misc::BeginProfilerColor(const struct FColor& Color)
{
	GTlsProfilerColorStack.Push(Color);
}

void FPS4Misc::EndProfilerColor()
{
...
bool FPS4Misc::GetDownloadAreaSizeAndFreeSpace(const FString& InPath, uint64& TotalNumberOfBytes, uint64& NumberOfFreeBytes)
{
...
bool FPS4Misc::GetDiskTotalAndFreeSpace(const FString& InPath, uint64& TotalNumberOfBytes, uint64& NumberOfFreeBytes)
{
