...
void FWebApiPS4Task::DoWork()
{
	...
	if (FNpWebApiThrottle::GetRateLimitStatus(ApiGroup) == EOnlinePSNRateLimitStatus::RateLimited)
	{
		...
		Result.NumericErrorCode = EHttpResponseCodes::TooManyRequests;
	...
	else
	{
		...
		Result.NumericErrorCode = ResponseInformationOption.httpStatus;
		...
		if (Result.NumericErrorCode == EHttpResponseCodes::TooManyRequests)
		{
			FNpWebApiThrottle::DetermineNextWebRequestTime(ApiGroup, RequestId);
		}
	}
...
{
	return Result.NumericErrorCode;
}
