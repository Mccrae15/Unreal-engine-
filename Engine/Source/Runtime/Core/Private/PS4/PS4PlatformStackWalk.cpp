...
#include "PS4/PS4PlatformStackWalk.h"
#include "Misc/App.h"
#if PS4_ENABLE_ASLR_STACKWALK
#include <libdbg.h>
#endif
...
#if PS4_ENABLE_ASLR_STACKWALK
TArray<FPS4PlatformStackWalk::FPS4SegmentInfo> FPS4PlatformStackWalk::SelfSegments;
#endif
...
void FPS4PlatformStackWalk::LoadSymbolInfo()
{
	...
	const FString PathOptions[] = { 
		TEXT("/app0/symbols/"),
		FString(TEXT("/app0")) / FApp::GetProjectName() / TEXT("build/ps4/symbols/"),
		GFileRootDirectory / GSandboxName / TEXT("symbols/"),
		GFileRootDirectory / GSandboxName / FApp::GetProjectName() / TEXT("build/ps4/symbols/"),
	};
...
{
#if PS4_ENABLE_ASLR_STACKWALK
	for (int Index = 0; Index < FPS4PlatformStackWalk::SelfSegments.Num(); ++Index)
	{
		FPS4PlatformStackWalk::FPS4SegmentInfo& Segment = FPS4PlatformStackWalk::SelfSegments[Index];
