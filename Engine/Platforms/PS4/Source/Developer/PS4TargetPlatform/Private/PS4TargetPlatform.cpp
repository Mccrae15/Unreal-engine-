...
FPS4TargetPlatform::FPS4TargetPlatform()
{
	...
	// we only need these when running the editor or programs. There is also an issue where they may not cleanup
	// correctly which casues issues during automated tests
	// Note - UFE returns 'Game' for this check
#if !IS_PROGRAM
	const FString EngineMode = FPlatformMisc::GetEngineMode();
	if (EngineMode != TEXT("Game") && EngineMode != TEXT("Server"))
#endif
	{
		FString CmdExe = FPaths::EngineDir() / TEXT("Binaries/DotNet/PS4/PS4DevKitUtil");
...
bool FPS4TargetPlatform::GenerateStreamingInstallManifest(const TMultiMap<FString, int32>& ChunkMap, const TSet<int32>& PakchunkIndicesInUse) const
{
	...
	for (auto ChunkIDIt = PakchunkIndicesInUse.CreateConstIterator(); ChunkIDIt; ++ChunkIDIt)
	{
	...
	FString ChunkHeader = FString::Printf(
				...
				TEXT(" 			<chunks supported_languages=\"en\" default_language=\"en\">\r\n"), 1000, PakchunkIndicesInUse.Num());		//@todo ps4: hardcoded SDK version.  Use ORBIS_SDK_VERSION from sdk_version.h?
...
void FPS4TargetPlatform::GetTextureFormats( const UTexture* InTexture, TArray< TArray<FName> >& OutFormats) const
{
	// @todo ps4: Do we support BC6H and BC7? (see last param)
	GetDefaultTextureFormatNamePerLayer(OutFormats.AddDefaulted_GetRef(), this, InTexture, PS4EngineSettings, true, true);
	for (FName& DefaultFormat : OutFormats.Last())
	{
...
{
	static const FName NAME_AT9(TEXT("AT9"));
	...
	const int32 ValidChannelCountsForAT9[5] = { 1, 2, 4, 6, 8 };

	for (const int32& ValidChannelCount : ValidChannelCountsForAT9)
	{
		if (Wave->NumChannels == ValidChannelCount)
		{
			return NAME_AT9;
		}
	}
	
	// If we got here, we had a non-standard channel count, so we fall back to ADPCM.
	return NAME_ADPCM;
}
