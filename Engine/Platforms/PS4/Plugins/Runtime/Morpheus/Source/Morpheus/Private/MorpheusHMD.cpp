...
public:
	...
	{
		FPlatformControllers::FControllerState& ControllerState = Controllers.GetControllerState(0);
...
class FMorpheusPlugin : public IMorpheusPlugin
{
	...
	virtual void StartupModule() override
	{
		...
		FString PluginShaderDir = FPaths::Combine(IPluginManager::Get().FindPlugin(TEXT("Morpheus"))->GetBaseDir(), TEXT("Shaders"));
...
void FMorpheusHMD::FMorpheusBridge::UpdateViewport(const FViewport& Viewport, FRHIViewport* ViewportRHI)
{
	...
	ENQUEUE_RENDER_COMMAND(SetUnscaledEyeTexture)(
		[Bridge, RT](FRHICommandListImmediate& RHICmdList)
		{
...
bool FMorpheusHMD::OnStartGameFrame(FWorldContext& InWorldContext)
{
	...
	const AWorldSettings* const WorldSettings = InWorldContext.World() ? InWorldContext.World()->GetWorldSettings() : nullptr;
	if (WorldSettings)
	{
		WorldToMeterScale = WorldSettings->WorldToMeters;
	}
	...
	else
	{
		...
		const bool bIsTrackingCameraConnected = PS4Tracker->IsCameraConnected();
		...
		if (bStereoEnabled)
		{
			...
			else if (!bIsTrackingCameraConnected)
			{
				// DelegateDefined cancel behavior means that something else (eg blueprints) must handle the problem if the user
				// cancels the dialog, so we want it to go up the first time, but not subsequent times.
				if (!bSuppressHMDSetupDialogAfterCancel && !sceCommonDialogIsUsed())
	...
	case SCE_COMMON_DIALOG_STATUS_RUNNING:
		{
			...
			{
				UE_LOG(LogMorpheusHMD, Log, TEXT("UpdateHMDSetupDialog() terminating because IsEngineExitRequested()"));
				EndHMDSetupDialog();
				return true;
			}
		}
	...
	case SCE_COMMON_DIALOG_STATUS_FINISHED:
		{
			...
			if (Ret != SCE_OK)
			{
				if (Ret == SCE_COMMON_DIALOG_ERROR_UNEXPECTED_FATAL)
				{
					UE_LOG(LogMorpheusHMD, Warning, TEXT("sceHmdSetupDialogGetResult failed: 0x%08X, SCE_COMMON_DIALOG_ERROR_UNEXPECTED_FATAL.  You may need to up/downgrade your PSVR HMD firmware (from devkit debug menu)."), Ret);
				}
				else
				{
					UE_LOG(LogMorpheusHMD, Warning, TEXT("sceHmdSetupDialogGetResult failed: 0x%08X."), Ret);
				}
				EndHMDSetupDialog();
				return true;
			}
...
bool FMorpheusHMD::OpenHMD()
{
	...
	*/
	...
	{
		check(PS4Application);
		OnVRHeadsetRecenterDefaultDelegateHandle = PS4Application->OnVRHeadsetRecenterDefaultDelegate.AddRaw(this, &FMorpheusHMD::OnVRHeadsetRecenterDefaultDelegateReceived);
	}

#else

	FString SceRootDir = FPlatformMisc::GetEnvironmentVariable(TEXT("SCE_ROOT_DIR"));
	if (SceRootDir.Len() == 0)
	{
		UE_LOG(LogMorpheusHMD, Warning, TEXT("SCE_ROOT_DIR is not set. Unable to load hmd_client.dll. Morpheus HMD will be unavailable."));
	}
	else
	{
		FString HMDClientDLLDir = FPaths::Combine(*SceRootDir, TEXT("ORBIS"), TEXT("Tools"), TEXT("HMD Server"), TEXT("x86_64"));
		FString HMDClientDLLPath = FPaths::Combine(*HMDClientDLLDir, TEXT("hmd_client.dll"));
		UE_LOG(LogMorpheusHMD, Display, TEXT("Loading hmd_client.dll from \"%s\"."), *HMDClientDLLPath);

		HMDClientDLLHandle = LoadLibraryW(*HMDClientDLLPath);
		if (HMDClientDLLHandle == nullptr)
		{
			UE_LOG(LogMorpheusHMD, Warning, TEXT("Failed to loading hmd_client.dll from \"%s\". Morpheus HMD will be unavailable."), *HMDClientDLLPath);
		}
		else
		{
...
void FMorpheusHMD::CloseHmd()
{
	...
	{
		FPS4Application::GetPS4Application()->OnVRHeadsetRecenterDefaultDelegate.Remove(OnVRHeadsetRecenterDefaultDelegateHandle);
	}
...
#endif
	PS4Tracker->SetPredictionTiming(FlipToDisplayLatency, RenderFrameTime, bIs60Render120ScanoutMode);
}
...
FXRRenderBridge* FMorpheusHMD::GetActiveRenderBridge_GameThread(bool /* unused */)
{
	check(IsInGameThread());

#if PLATFORM_PS4
	return MorpheusBridge;
#else
	return nullptr;
#endif
}
...
{
	return !!(MORPHEUS_ENGINE_DISTORTION);
}
...
{
	FMorpheusHMD* const Self = this;
	ENQUEUE_RENDER_COMMAND(SetupOcclusionMeshesCmd)([=](FRHICommandListImmediate& RHICmdList)
	{
		Self->HiddenAreaMeshes[0].BuildMesh(LeftEyeHiddenAreaPositions, HiddenAreaVertexCount, FHMDViewMesh::MT_HiddenArea);
		Self->HiddenAreaMeshes[1].BuildMesh(RightEyeHiddenAreaPositions, HiddenAreaVertexCount, FHMDViewMesh::MT_HiddenArea);
		Self->VisibleAreaMeshes[0].BuildMesh(LeftEyeVisibleAreaPositions, VisibleAreaVertexCount, FHMDViewMesh::MT_VisibleArea);
		Self->VisibleAreaMeshes[1].BuildMesh(RightEyeVisibleAreaPositions, VisibleAreaVertexCount, FHMDViewMesh::MT_VisibleArea);
	});
}
...
void FMorpheusHMD::OnVRHeadsetRecenterDefaultDelegateReceived()
{
...
void FMorpheusHMD::FetchTrackingData(FTrackingFrame& TrackingFrame, FQuat& CurrentOrientation, FVector& CurrentPosition, bool bUpdateLastTrackingData, bool bEarlyPoll, bool bPollImmediately)
{
	...
	TrackingFrame.WorldToMetersScale = WorldToMeterScale;
