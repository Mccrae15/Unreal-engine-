...
#if ENABLE_GPU_DEBUGGER
	if (DebugResourceHandle != 0)
	{
		sce::Gnm::unregisterResource(DebugResourceHandle);
		DebugResourceHandle = 0;
	}
#endif
}
...
#if ENABLE_GPU_DEBUGGER
	//Registers the shader for use with PS4 GPU debugger
	sce::Gnm::registerResource(&InShader->DebugResourceHandle, GGnmManager.GetOwnerHandle(), InShader->ShaderMemory.GetPointer(), ShaderInfo.m_gpuShaderCodeSize, TCHAR_TO_ANSI(*InShader->ShaderName), sce::Gnm::kResourceTypeShaderBaseAddress, 0);
#endif
}
...
void ExtractPlatformShader(const uint8* &PlatformShaderCode, uint32& PlatformShaderCodeSize, TArrayView<const uint8> InShaderCode)
{
...
FVertexShaderRHIRef FGnmDynamicRHI::RHICreateVertexShader(TArrayView<const uint8> InShaderCode, const FSHAHash& Hash)
{
	...
	TArrayView<const uint8> UnpackedShaderCode = MakeArrayView(PlatformShaderCode, PlatformShaderCodeSize);
	...
	FMemoryReaderView Ar( UnpackedShaderCode, true );
...
FPixelShaderRHIRef FGnmDynamicRHI::RHICreatePixelShader(TArrayView<const uint8> InShaderCode, const FSHAHash& Hash)
{
...
FHullShaderRHIRef FGnmDynamicRHI::RHICreateHullShader(TArrayView<const uint8> InShaderCode, const FSHAHash& Hash)
{ 
	FShaderCodeReader ShaderCode(InShaderCode);
...
FDomainShaderRHIRef FGnmDynamicRHI::RHICreateDomainShader(TArrayView<const uint8> InShaderCode, const FSHAHash& Hash)
{ 
	FShaderCodeReader ShaderCode(InShaderCode);
...
FGeometryShaderRHIRef FGnmDynamicRHI::RHICreateGeometryShader(TArrayView<const uint8> InShaderCode, const FSHAHash& Hash)
{ 
	FShaderCodeReader ShaderCode(InShaderCode);
...
FComputeShaderRHIRef FGnmDynamicRHI::RHICreateComputeShader(TArrayView<const uint8> InShaderCode, const FSHAHash& Hash)
{ 
	const uint8* PlatformShaderCode;
	uint32 PlatformShaderCodeSize;
	...
	if( ShaderRequiresScratchBuffer( Shader->Shader ) )
	{
		...
		uint32 ScratchBufferSize = FGnmManager::ScratchBufferMaxNumWaves * Num1KbyteScratchChunksPerWave * 1024;
			...
			UE_LOG(LogSony, Log, TEXT("Compute Shader Scratch Usage: DWPerThread is %d, Num1KbyteScratchChunksPerWave is %d, scratch buffer size is %d"), ScratchSizeInDWPerThread, Num1KbyteScratchChunksPerWave, ScratchBufferSize );
		}
	...
	:	CacheLink(InVertexDeclarationRHI,InVertexShaderRHI,InPixelShaderRHI,InHullShaderRHI,InDomainShaderRHI,InGeometryShaderRHI,this)
{
	...
	if (InVertexShader && InVertexDeclaration)
	{
		...
		for (uint32 InputIndex = 0; InputIndex < InVertexShader->Program.m_numInputAttributes; InputIndex++)
		{
			...
			if (Attrib.m_psslSemantic == Shader::Binary::kSemanticUserDefined)
			{
				...
				{
					UE_LOG(LogSony, Fatal, TEXT("Found unknown user attribute %s in a vertex program!"), ANSI_TO_TCHAR(Attrib.getSemanticName()));
				}
		...
		if (InVertexShader->ExportShader)
		{
			...
			Gnmx::generateEsFetchShader((uint32*)CPUFetchShader.GetData(), &FetchShaderModifier, InVertexShader->ExportShader, InstancingData, UE_ARRAY_COUNT(InstancingData));
		}
		else
		{
			check(InGeometryShader == NULL);
			...
			CPUFetchShader.AddUninitialized(FetchShaderSize);
			Gnmx::generateVsFetchShader((uint32*)CPUFetchShader.GetData(), &FetchShaderModifier, InVertexShader->Shader, InstancingData, UE_ARRAY_COUNT(InstancingData));
		}
	...
	if( MaxScratchSizeInDWPerThread )
	{
		...
		UE_LOG(LogSony, Log, TEXT("Graphic Shader Scratch Usage: DWPerThread is %d, Num1KbyteScratchChunksPerWave is %d, scratch buffer size is %d"), MaxScratchSizeInDWPerThread, Num1KbyteScratchChunksPerWave, ScratchBufferSize );
