...
CSV_DEFINE_CATEGORY(HttpPS4, false);
...
FString FPS4HTTPRequest::GetURIPart(uint32 URIPartToGet) const
{
	char *RebuildUri;
...
FString FPS4HTTPRequest::GetURL() const
{
	return GetURIPart(SCE_HTTP_URI_BUILD_WITH_ALL);
}

FString FPS4HTTPRequest::GetURLParameter(const FString& ParameterName) const
{
	return GetURIPart(PS4UriParameterName(ParameterName));
}

FString FPS4HTTPRequest::GetHeader(const FString& HeaderName) const
{
	const FString* Header = RequestHeaders.Find(HeaderName);
	return Header != NULL ? *Header : TEXT("");
}

TArray<FString> FPS4HTTPRequest::GetAllHeaders() const
{
...
FString FPS4HTTPRequest::GetContentType() const
{
	return GetHeader(TEXT("Content-Type"));
}

int32 FPS4HTTPRequest::GetContentLength() const
{
	return RequestPayload.IsValid() ? RequestPayload->GetContentLength() : 0;
}

const TArray<uint8>& FPS4HTTPRequest::GetContent() const
{
	static const TArray<uint8> EmptyContent;
	return RequestPayload.IsValid() ? RequestPayload->GetContent() : EmptyContent;
}

FString FPS4HTTPRequest::GetVerb() const
{
	return RequestVerb;
}
...
bool FPS4HTTPRequest::SetContentAsStreamedFile(const FString& Filename)
{
...
bool FPS4HTTPRequest::SetContentFromStream(TSharedRef<FArchive, ESPMode::ThreadSafe> Stream)
{
...
EHttpRequestStatus::Type FPS4HTTPRequest::GetStatus() const
{
...
{
	CSV_SCOPED_TIMING_STAT(HttpPS4, TickSendState);
...
{
	CSV_SCOPED_TIMING_STAT(HttpPS4, TickThreadedRequest);
...
int32 FPS4HTTPRequest::PS4Verb() const
{
...
bool FPS4HTTPRequest::StartRequest()
{
	...
	const int32 PS4HttpVerb = PS4Verb();
	if (PS4HttpVerb != -1)
	{
		RequestId = sceHttpCreateRequestWithURL(ConnectionId, PS4Verb(), TCHAR_TO_ANSI(*GetURL()), RequestPayload->GetContentLength());
	}
	else
	{
	...
	{
		UE_LOG(LogHttp, Warning, TEXT("HttpCreateRequestWithURL failed. The value specified in method is invalid: %i. Verb=[%s]"), PS4HttpVerb, *RequestVerb);
		return false;
	}
...
float FPS4HTTPRequest::GetElapsedTime() const
{
	return ElapsedTime;
}
...
void FPS4HTTPRequest::AbortRequest()
{
...
FString FPS4HTTPResponse::GetURIPart(uint32 URIPartToGet) const
{
	size_t MallocSize, UseSize;
...
FString FPS4HTTPResponse::GetURL() const
{
	return GetURIPart(SCE_HTTP_URI_BUILD_WITH_ALL);
}


FString FPS4HTTPResponse::GetURLParameter(const FString& ParameterName) const
{
	return GetURIPart(PS4UriParameterName(ParameterName));
}

FString FPS4HTTPResponse::GetHeader(const FString& HeaderName) const
{
	FString Result(TEXT(""));
	if (!bIsReady)
	{
...
TArray<FString> FPS4HTTPResponse::GetAllHeaders() const
{
	TArray<FString> Result;
	if (!bIsReady)
	{
...
FString FPS4HTTPResponse::GetContentType() const
{
	return GetHeader(TEXT("Content-Type"));
}

int32 FPS4HTTPResponse::GetContentLength() const
{
	return ContentLength;
}

const TArray<uint8>& FPS4HTTPResponse::GetContent() const
{
	if (!bIsReady)
	{
...
int32 FPS4HTTPResponse::GetResponseCode() const
{
	return ResponseCode;
}

FString FPS4HTTPResponse::GetContentAsString() const
{
