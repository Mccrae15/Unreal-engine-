...
void FOnlineAsyncTaskPS4SessionDestroy::DoWork()
{
	...
	AsyncTask.GetTask().SetRequest(TEXT("sessionInvitation"), URL, SceNpWebApiHttpMethod::SCE_NP_WEBAPI_HTTP_METHOD_DELETE);
	...
	if (bWasSuccessful)
	{
		UE_LOG_ONLINE(Verbose, TEXT("Destroy Session successful Name=[%s] SessionId=[%s]"), *Session->SessionName.ToString(), *SessionId);
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("Failed to Destroy Session Name=[%s] SessionId=[%s] HttpStatus=[%d] Error=[%s]"), *Session->SessionName.ToString(), *SessionId, StatusCode, *TaskResult.GetErrorString());
	}
...
void FOnlineAsyncTaskPS4SessionJoin::DoWork()
{
	...
	Task.GetTask().SetRequest(TEXT("sessionInvitation"), URL, SceNpWebApiHttpMethod::SCE_NP_WEBAPI_HTTP_METHOD_POST);
...
void FOnlineAsyncTaskPS4SessionLeave::DoWork()
{
	...
	{
		UE_LOG_ONLINE(Warning, TEXT("FOnlineAsyncTaskPS4SessionLeave: AsyncTaskManager is null - missing task manager."));
		return;
	}
	...
	if (bWasSuccessful)
	{
		UE_LOG_ONLINE(Verbose, TEXT("Leave Session successful Name=[%s] SessionId=[%s]"), *Session->SessionName.ToString(), *SessionId);
	}
	else
	{
		UE_LOG_ONLINE(Warning, TEXT("Failed to leave session Name=[%s] SessionId=[%s] HttpStatus=[%d] Error=[%s]"), *Session->SessionName.ToString(), *SessionId, StatusCode, *TaskResult.GetErrorString());
	}
		...
		if (PS4SessionInfo.IsValid())
		{
			if (PS4SessionInfo->SessionType == EPS4Session::RoomSession && PS4SessionInfo->RoomId != SCE_NP_MATCHING2_INVALID_ROOM_ID)
			{
				...
				if (OnlineSessionPS4Ptr.IsValid())
				{
					...
					{
						UE_LOG_ONLINE(Warning, TEXT("Failed to leave a room - 0x%x"), LeaveRoomReturnCode);
						bIsComplete = true;
					}
					else
					{
				...
				{
					UE_LOG_ONLINE(Warning, TEXT("Failed to leave a room - Invalid session info"));
					bIsComplete = true;
				}
			}
			else
			{
				UE_LOG_ONLINE(Warning, TEXT("Failed to leave a room - Session Type (%d) or Room ID (%lu) invalid"), static_cast<int32>(PS4SessionInfo->SessionType), static_cast<uint64>(PS4SessionInfo->RoomId));
				bIsComplete = true;
			}
		}
		else
...
bool FOnlineSessionPS4::DoLeaveRoom(FNamedOnlineSession& Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	if (AsyncTaskManager)
	{
		UE_LOG_ONLINE(Verbose, TEXT("FOnlineSessionPS4::DoLeaveRoom: Leave Room queued"));
		Session.SessionState = EOnlineSessionState::Destroying;
...
{
	UE_LOG_ONLINE(Verbose, TEXT("Attempting to Destroy Session %s"), *SessionName.ToString());
	...
	else if (Session->SessionState == EOnlineSessionState::Destroying)
	{
		...
		UE_LOG_ONLINE(Warning, TEXT("Session %s is already pending destruction"), *SessionName.ToString());
	...
	else
	{
		if (PS4Subsystem->AreRoomsEnabled())
		{
			...
			{
				UE_LOG_ONLINE(Warning, TEXT("Unable to leave room for Session %s"), *SessionName.ToString());
			}
		}
		else
		{
			...
			{
				UE_LOG_ONLINE(Warning, TEXT("Unable to leave room or destroy session for Session %s"), *SessionName.ToString());
			}
		}
	}
...
bool FOnlineSessionPS4::DoLeaveOrDestroySession(FNamedOnlineSession& Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	{
		UE_LOG_ONLINE(Verbose, TEXT("Actually destroying session %s as we are host"), *Session.SessionName.ToString());
	...
	else
	{
		...
		{
			UE_LOG_ONLINE(Verbose, TEXT("Leaving session %s as we are letting another player become host"), *Session.SessionName.ToString());
		}
		else
		{
			UE_LOG_ONLINE(Verbose, TEXT("Leaving session %s as we not the host"), *Session.SessionName.ToString());
		}
...
bool FOnlineSessionPS4::StartMatchmaking(const TArray< TSharedRef<const FUniqueNetId> >& LocalPlayers, FName SessionName, const FOnlineSessionSettings& NewSessionSettings, TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
	...
	{
		UE_LOG_ONLINE(Warning, TEXT("Matchmaking cannot start with invalid PlayerId: %s."), *SearchingPlayerId->ToDebugString());
		TriggerOnCancelMatchmakingCompleteDelegates(SessionName, false);
		return false;
	}
