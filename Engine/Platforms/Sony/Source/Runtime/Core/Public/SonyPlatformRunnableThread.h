 ...
 */
class FSonyRunnableThread : public FRunnableThreadPThread
{
protected:

	virtual ~FSonyRunnableThread()
	{
	...
	virtual bool SetThreadAttr(pthread_attr_t *ThreadAttr, uint32 InStackSize, EThreadCreateFlags InCreateFlags) override
	{
		return FRunnableThreadPThread::SetThreadAttr(ThreadAttr, InStackSize, InCreateFlags);
	}
};
