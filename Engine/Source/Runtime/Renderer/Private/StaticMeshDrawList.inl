	...
	)
{
	...
	if (!bDrawnShared)
	{
		...
		if ((View.bVRProjectEnabled || PolicyContext.bIsSinglePassStereo) && (DrawingPolicyLink->MultiResBoundShaderStateInput.VertexShaderRHI != nullptr))
		{
			BoundShaderStateInput = DrawingPolicyLink->MultiResBoundShaderStateInput;
		}
		else if (ensure(DrawingPolicyLink->BoundShaderStateInput.VertexShaderRHI != nullptr))
		{
	...
	)
{
	...
	for (int32 Index = FirstPolicy; Index <= LastPolicy; Index++)
	{
		...
		{
			// Single view
			if (InstancedStereo == InstancedStereoPolicy::Disabled && !PolicyContext.bIsSinglePassStereo)
			{
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		...
		if (this->PerDrawingPolicyCounts)
		{
			...
			while (Start <= this->LastPolicy)
			{
				...
				if (Start <= this->LastPolicy)
				{
					...
					if (!this->PolicyContext.bIsInstancedStereo && !this->PolicyContext.bIsSinglePassStereo)
					{
					...
					{
						if (this->PolicyContext.bIsSinglePassStereo)
						{
							this->Caller.template DrawVisibleInner<InstancedStereoPolicy::Disabled>(this->RHICmdList, *this->StereoView.LeftView, this->PolicyContext, this->DrawRenderState, nullptr, nullptr, &StereoView, Start, BatchEnd, true);
						}
						else
						{
		...
		else
		{
			...
			else
			{
				if (this->PolicyContext.bIsSinglePassStereo)
				{
					this->Caller.template DrawVisibleInner<InstancedStereoPolicy::Disabled>(this->RHICmdList, *this->StereoView.LeftView, this->PolicyContext, this->DrawRenderState, nullptr, nullptr, &StereoView, this->FirstPolicy, this->LastPolicy, true);
				}
				else
				{
		...
		if (ParallelCommandListSet.bBalanceCommands)
		{
			...
			if (bNeedScan)
			{
				...
				ParallelForWithPreWork(NumPolicies, 
					[this, &PerDrawingPolicyCounts, &StaticMeshVisibilityMap, &BatchVisibilityArray, StereoView, PolicyContext](int32 Index)
					{
						int32 Count = 0;
						...
						FPlatformMisc::Prefetch(&DrawingPolicyLink->CompactElements.GetData()->MeshId);
						const bool bIsSinglePassStereo = (StereoView != nullptr) && PolicyContext.bIsSinglePassStereo;
						const bool bIsInstancedStereo = (StereoView != nullptr) && (!bIsSinglePassStereo);
						for (int32 ElementIndex = 0; ElementIndex < NumElements; ElementIndex++, CompactElementPtr++)
						{
							bool bIsVisible = false;

							// Single view
							if (!bIsInstancedStereo && !bIsSinglePassStereo && StaticMeshVisibilityMap->AccessCorrespondingBit(FRelativeBitReference(CompactElementPtr->MeshId)))
							{
								bIsVisible = true;
							}
							...
							// Stereo pair, we need to test both eyes
								...
								(bIsInstancedStereo || bIsSinglePassStereo) &&
