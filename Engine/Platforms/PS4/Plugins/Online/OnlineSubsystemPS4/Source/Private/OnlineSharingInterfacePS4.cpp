...
public:
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4ShareStatusUpdate bWasSuccessful: %d"), WasSuccessful());
	}
	...
	void StartTask(const FString& UserName, const FOnlineStatusUpdate& StatusUpdate)
	{
		...
		{
			UE_LOG_ONLINE_SHARING(Error, TEXT("Could not read Json file for activity feed. File = %s"), *JsonTemplateFullPath);
		...
		UE_LOG_ONLINE_SHARING(Display, TEXT("POST user feed body = \n%s"), *JsonStr);

		WebApiTask.GetTask().SetRequestBody(JsonStr);
		WebApiTask.GetTask().SetRequest(ENpApiGroup::ActivityFeed, TEXT("/v1/users/me/feed"), SCE_NP_WEBAPI_HTTP_METHOD_POST);
	...
	virtual void TriggerDelegates() override
	{
		...
		UE_LOG_ONLINE_SHARING(Display, TEXT("POST USER FEED response body = %s"), *WebApiTask.GetTask().GetResponseBody());
...
public:
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskPS4ReadNewsFeed bWasSuccessful: %d"), WasSuccessful());
	}
	...
	virtual void Tick() override
	{
		...
		if (!CurrentWebApiTask.IsValid())
		{
			...
			CurrentWebApiTask->GetTask().SetRequest(ENpApiGroup::ActivityFeed, Url, SCE_NP_WEBAPI_HTTP_METHOD_GET);
...
private:
	...
	{
		UE_LOG_ONLINE_SHARING(Display, TEXT("GET USER FEED %d, response body = %s"), PostCounter, *ResponseStr);
