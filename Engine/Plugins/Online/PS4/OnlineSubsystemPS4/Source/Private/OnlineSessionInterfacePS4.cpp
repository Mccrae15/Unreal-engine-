...
bool FOnlineSessionPS4::DoLeaveRoom(FNamedOnlineSession& Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	if (AsyncTaskManager)
	{
		...
		FOnlineAsyncTaskPS4_LeaveRoom* const NewTask = new FOnlineAsyncTaskPS4_LeaveRoom(*PS4Subsystem, Session, CompletionDelegate, RequestId);
...
bool FOnlineSessionPS4::SendSessionInviteToFriends(const FUniqueNetId& LocalUserId, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	...
	{
		FOnlineAsyncTaskPS4_SendSessionInvite* NewTask = new FOnlineAsyncTaskPS4_SendSessionInvite(*PS4Subsystem, FUniqueNetIdPS4::Cast(LocalUserId.AsShared()), *Session, Friends);
		AsyncTaskManager->AddToInQueue(NewTask);
		return true;
	}
