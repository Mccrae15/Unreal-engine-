...
void FRCPassPostProcessTonemap::Process(FRenderingCompositePassContext& Context)
{
	...
	else
	{
		...
		else
		{
			...
			{	// psilva: should ESimpleRenderTargetMode::EUninitializedColorExistingDepth change to EExistingColorAndDepth? I think so, hence the change.
				SetRenderTarget(Context.RHICmdList, DestRenderTarget.TargetableTexture, SceneContext.GetSceneDepthTexture(), ESimpleRenderTargetMode::EExistingColorAndDepth, FExclusiveDepthStencil::DepthRead_StencilNop);
				FGraphicsPipelineStateInitializer GraphicsPSOInit;
				Context.RHICmdList.ApplyCachedRenderTargets(GraphicsPSOInit);
