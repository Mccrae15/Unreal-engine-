...
SceVoiceQoSRemoteId RemoteEndpointManager::AllocateRemoteEndpoint()
{
	...
	if (Count + 1 <= MaxCount)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSCreateRemoteEndpoint() failed. Error code: %#x"), Result);
...
void RemoteEndpointManager::ReleaseRemoteEndpoint(SceVoiceQoSRemoteId RemoteEndpoint)
{
	if (Count > 0 && RemoteEndpoint != SCE_VOICE_QOS_INVALID_REMOTE_ID)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSDeleteRemoteEndpoint() failed. Error code: %#x"), Result);
		}
		else
		{
			--Count;
	...
	LocalConnection(NULL)
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSCreateLocalEndpoint() failed for SceUserServiceUserId %d. Error code: %#x"), UserId, Result);
		LocalEndpoint = SCE_VOICE_QOS_INVALID_LOCAL_ID;
	}
...
LocalTalkerPS4::~LocalTalkerPS4()
{
	...
	if (LocalEndpoint != SCE_VOICE_QOS_INVALID_LOCAL_ID)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSDeleteLocalEndpoint() failed. Error code: %#x"), Result);
		}
		LocalEndpoint = SCE_VOICE_QOS_INVALID_LOCAL_ID;
	}
}
...
void LocalTalkerPS4::WriteVoicePacket(FVoicePacketPS4 *VoicePacket)
{
	...
	else
	{
		...
		if (NewRemoteEndpoint == SCE_VOICE_QOS_INVALID_REMOTE_ID)
		{
			...
			UE_LOG_ONLINE_VOICE(Verbose, TEXT("Dropped packet from %s! No free endpoints."), *VoicePacket->GetSenderName());
		}
		else
		{
			VoiceConnectionPS4Ptr Connection = MakeShareable(new VoiceConnectionPS4(LocalEndpoint, NewRemoteEndpoint));
			if (Connection->ConnectId != SCE_VOICE_QOS_INVALID_CONNECTION_ID)
			{
...
TSharedPtr<class FVoicePacket> LocalTalkerPS4::ReadVoicePacket()
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_VOICE(Log, TEXT("sceVoiceQoSReadPacket() failed for connection id %d. Error code %#x"), LocalConnection->ConnectId, Result);
		return NULL;
	}
...
void LocalTalkerPS4::StartNetworkedVoice()
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_VOICE(Log, TEXT("sceVoiceQoSSetLocalEndpointAttribute() failed for connection id %d. Error code %#x"), LocalConnection->ConnectId, Result);
	}
}
	...
	DeadAirCount(0)
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSConnect() failed. Error code: %#x"), Result);
		ConnectId = SCE_VOICE_QOS_INVALID_CONNECTION_ID;
	}
}
...
VoiceConnectionPS4::~VoiceConnectionPS4()
{
	if (ConnectId != SCE_VOICE_QOS_INVALID_CONNECTION_ID)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSDisconnect() failed. Error code: %#x"), Result);
		}
		ConnectId = SCE_VOICE_QOS_INVALID_CONNECTION_ID;
	}
}
...
void VoiceConnectionPS4::WriteVoicePacket(FVoicePacketPS4 *VoicePacket)
{
	if (ConnectId != SCE_VOICE_QOS_INVALID_CONNECTION_ID)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSWritePacket() failed. Error code: %#x"), Result);
		}
		else
		{
			...
			if (WriteLength != VoicePacket->Length)
			{
				...
				UE_LOG_ONLINE_VOICE(Error, TEXT("sceVoiceQoSDisconnect() only wrote %d bytes when %d were available."), WriteLength, VoicePacket->Length);
			}
...
bool FOnlineVoicePS4::Init()
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG_ONLINE_VOICE(Warning, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_VOICEQOS) failed. Error code: 0x%08x"), Result);
		return false;
	}
	...
	if (!VoiceMemory)
	{
		UE_LOG_ONLINE_VOICE(Warning, TEXT("Failed to allocate %d bytes for QoS voice memory"), SCE_VOICE_MEMORY_CONTAINER_SIZE);
		return false;
	}
	...
	if (ErrorCode != SCE_OK)
	{
		UE_LOG_ONLINE_VOICE(Warning, TEXT("sceVoiceQoSInit() failed. Error code: 0x%08x"), ErrorCode);
		return false;
	}
...
void FOnlineVoicePS4::Shutdown()
{
	if (bIsInitialized)
	{
		...
		if (Result != SCE_OK)
		{
			UE_LOG_ONLINE_VOICE(Warning, TEXT("sceVoiceQoSEnd() failed. Error code: 0x%08x"), Result);
		}
		bIsInitialized = false;
	}
	...
	if (sceSysmoduleIsLoaded(SCE_SYSMODULE_VOICEQOS))
	{
		...
		if (Result != SCE_OK )
		{
			UE_LOG_ONLINE_VOICE(Warning, TEXT("sceSysmoduleUnloadModule(SCE_SYSMODULE_VOICEQOS) failed. Error code: 0x%08x"), Result);
		}
	}
}
...
void FOnlineVoicePS4::ProcessTalkingDelegates(float DeltaTime)
{
	...
	for (int32 LocalUserIndex = 0; LocalUserIndex < LocalTalkers.Num(); LocalUserIndex++)
	{
		...
		//UE_LOG_ONLINE_VOICE(Error, TEXT("Trigger %sTALKING"), Talker->bIsSpeaking ? TEXT("") : TEXT("NOT"));
	}
}
...
IVoiceEnginePtr FOnlineVoicePS4::CreateVoiceEngine()
{
