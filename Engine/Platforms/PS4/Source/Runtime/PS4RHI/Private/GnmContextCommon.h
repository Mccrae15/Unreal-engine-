...
public:
	...
	template <typename TGnmContext>
	static FORCEINLINE int32 SetShaderResourcesFromBuffer_UAV(TGnmContext& CommandListContext, Gnm::ShaderStage ShaderStage, FGnmUniformBuffer* RESTRICT Buffer, const uint32* RESTRICT ResourceMap, int32 BufferIndex);
...
template <typename TGnmContext>
inline int32 FGnmContextCommon::SetShaderResourcesFromBuffer_UAV(TGnmContext& CommandListContext, Gnm::ShaderStage ShaderStage, FGnmUniformBuffer* RESTRICT Buffer, const uint32* RESTRICT ResourceMap, int32 BufferIndex)
{
	const TRefCountPtr<FRHIResource>* RESTRICT Resources = Buffer->ResourceTable.GetData();
	int32 NumSetCalls = 0;
	uint32 BufferOffset = ResourceMap[BufferIndex];
	if (BufferOffset > 0)
	{
		...
		do
		{
			...
			FGnmUnorderedAccessView* ResourcePtr = (FGnmUnorderedAccessView*)Resources[ResourceIndex].GetReference();

			CommandListContext.BindUAV(ResourcePtr, BindIndex, ShaderStage);
