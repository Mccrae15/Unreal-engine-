...
private:
	...
	struct FDeviceState
	{
		...
		int32					LastTrackingResult; // Used to avoid log spam when a device is disconnected
...
void FPS4Tracker::Initialize()
{
	...
	if (Result != SCE_OK)
	{
		UE_LOG(LogSony, Warning, TEXT("sceSysmoduleLoadModule(SCE_SYSMODULE_MESSAGE_DIALOG) failed (result 0x%08x).  PS4 Tracker will probably not be able to pop up the camera disconnected dialog."), Result);
		return;
	}
	...
	const EGnmMemType TrackerOnionMemType = EGnmMemType::GnmMem_CPU;
	...
	case SCE_COMMON_DIALOG_STATUS_RUNNING:
		{
			...
			{
				UE_LOG(LogPS4Tracker, Log, TEXT("UpdateCameraSetupDialog() terminating because IsEngineExitRequested()"));
				EndCameraSetupDialog();
				return true;
			}
...
int32 FPS4Tracker::AcquireTracker(int32 DeviceHandle, int32 ControllerIndex, EDeviceType DeviceType)
{
	...
	for (int32 TrackerHandle = 0; TrackerHandle < UE_ARRAY_COUNT(DeviceStates); ++TrackerHandle)
	{
...
int32 FPS4Tracker::CreateTracker(int32 DeviceHandle, int32 ControllerIndex, EDeviceType DeviceType)
{
	...
	for (; TrackerHandle < UE_ARRAY_COUNT(DeviceStates); ++TrackerHandle)
	{
		auto& DeviceState = DeviceStates[TrackerHandle];
	...
	if (TrackerHandle >= UE_ARRAY_COUNT(DeviceStates))
	{
		UE_LOG(LogPS4Tracker, Warning, TEXT("Tried to create a tracker for %i (HMD,Left,Pad,Right,Gun), but we already have %i trackers, which is the maximum.  Failing."), (int)DeviceType, UE_ARRAY_COUNT(DeviceStates));
		return INVALID_TRACKER_HANDLE;
	}
...
void FPS4Tracker::ReleaseTracker(int32 TrackerHandle)
{
	...
	check(TrackerHandle < UE_ARRAY_COUNT(DeviceStates));
...
//=============================================================================
void FPS4Tracker::OnWorldTickStart(UWorld* World, ELevelTick TickType, float DeltaSeconds)
{
...
{
	for (int i = 0; i < UE_ARRAY_COUNT(DeviceStates); ++i)
	{
...
void FPS4Tracker::PollTrackingData(FDeviceState& DeviceState, FTrackingData& TrackingData, uint64 PredictionTimeOffset, bool bEarlyPoll)
{
	...
	else
	{
		...
		else if (Ret == SCE_VR_TRACKER_ERROR_TIMESTAMP_OUT_OF_RANGE)
		{
			...
			// The error would cease when the hug ended. 
			// The inside of a game studio is probably something of a stress test for wireless controller communication.
			// This error will also be returned if the device has been turned off, for instance through the quick access menu, in which case for 
			// MotionControllers it is not possible to discern from other out of range errors (See above for the solution for the HMD.)
			// Therefore, this message is logged the first time until sceVrTrackerGetResult() returns something else for the device.
			if (Ret != DeviceState.LastTrackingResult)
			{
	...
	DeviceState.LastTrackingResult = Ret;
}


#elif PLATFORM_WINDOWS && HAS_MORPHEUS_HMD_SDK
