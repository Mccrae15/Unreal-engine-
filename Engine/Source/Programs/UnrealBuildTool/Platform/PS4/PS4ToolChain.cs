...
namespace UnrealBuildTool
{
	...
	class PS4ToolChain : UEToolChain
	{
		...
		private ICollection<FileItem> GenerateSymbols(FileItem Executable, LinkEnvironment LinkEnvironment, List<Action> Actions)
		{
			...
			if (!LinkEnvironment.bCreateMapFile)
			{
				Action SymbolTableAction = new Action(ActionType.Link);
				...
				SymbolTableAction.CommandArguments = string.Format("\"{0}\" \"{1}\" \"{2}\"", NMPath, SelfPath, InputNMOutputFilename);
				...
				Actions.Add(SymbolTableAction);
			}

			// Symbol Tool Stage
			Action PostBuildAction = new Action(ActionType.Link);
		...
		public override ICollection<FileItem> PostBuild(FileItem Executable, LinkEnvironment BinaryLinkEnvironment, List<Action> Actions)
		{
			...
			{
				OutputFiles.AddRange(GenerateSymbols(Executable, BinaryLinkEnvironment, Actions));
			}
