...
void FOnlineAsyncTaskPS4SessionDestroy::DoWork()
{
	...
	if (bWasSuccessful)
	{
		UE_LOG_ONLINE_SESSION(Verbose, TEXT("Destroy Session successful Name=[%s] SessionId=[%s]"), *Session->SessionName.ToString(), *SessionId);
	}
	else
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("Failed to Destroy Session Name=[%s] SessionId=[%s] HttpStatus=[%d] Error=[%s]"), *Session->SessionName.ToString(), *SessionId, StatusCode, *TaskResult.GetErrorString());
	}
	...
	if (AsyncTaskManager)
	{
		auto NewEvent = new FAsyncEventSessionDestroyTaskCompleted(PS4Subsystem, Session->SessionName, bWasSuccessful, CompletionDelegate);
		AsyncTaskManager->AddToOutQueue(NewEvent);
	}
}
...
void FOnlineAsyncTaskPS4SessionLeave::DoWork()
{
	...
	if (Session == nullptr)
	{
		...
		FAsyncEventSessionLeaveTaskCompleted* const NewEvent = new FAsyncEventSessionLeaveTaskCompleted(PS4Subsystem, NAME_None, false, CompletionDelegate);
		AsyncTaskManager->AddToOutQueue(NewEvent);
		return;
	}
	...
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("FOnlineAsyncTaskPS4SessionLeave: Session->SessionInfo pointer is null - we are unable to leave %s. This might mean the session has already been left."), *Session->SessionName.ToString());
		FAsyncEventSessionLeaveTaskCompleted* const NewEvent = new FAsyncEventSessionLeaveTaskCompleted(PS4Subsystem, Session->SessionName, false, CompletionDelegate);
		AsyncTaskManager->AddToOutQueue(NewEvent);
		return;
	}
	...
	if (bWasSuccessful)
	{
		UE_LOG_ONLINE_SESSION(Verbose, TEXT("Leave Session successful Name=[%s] SessionId=[%s]"), *Session->SessionName.ToString(), *SessionId);
	}
	else
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("Failed to leave session Name=[%s] SessionId=[%s] HttpStatus=[%d] Error=[%s]"), *Session->SessionName.ToString(), *SessionId, StatusCode, *TaskResult.GetErrorString());
	}

	FAsyncEventSessionLeaveTaskCompleted* const NewEvent = new FAsyncEventSessionLeaveTaskCompleted(PS4Subsystem, Session->SessionName, bWasSuccessful, CompletionDelegate);
	AsyncTaskManager->AddToOutQueue(NewEvent);
}
...
void FOnlineAsyncTaskPS4GetSession::Tick()
{
	if (!bInitialized)
	{
		...
		FAsyncTask<FWebApiPS4Task> task(PS4Subsystem->GetUserWebApiContext(UserId.Get()));
		task.GetTask().SetRequest(ENpApiGroup::SessionAndInvitation, FString(TEXT("/v1/sessions/")) + OnlineSessionSearch->SearchResults[0].Session.SessionInfo->GetSessionId().ToString() + FString(TEXT("?fields=@default,members")), SceNpWebApiHttpMethod::SCE_NP_WEBAPI_HTTP_METHOD_GET);
		task.StartSynchronousTask();
		bWasSuccessful = task.GetTask().GetHttpStatusCode() == 200;
		bIsComplete = true;
...
void FOnlineAsyncTaskPS4SessionGetChangeableSessionData::DoWork()
{
	...
	if (LIKELY(AsyncTaskManager != nullptr))
	{
		...
		FAsyncEventSessionGetChangeableSessionDataTaskCompleted* const NewEvent = new FAsyncEventSessionGetChangeableSessionDataTaskCompleted(PS4Subsystem, UserId, MoveTemp(SessionId), MoveTemp(ResponseBody), Callback, bSucceeded);
		AsyncTaskManager->AddToOutQueue(NewEvent);
	}
}
	...
	if (!bInitialized)
	{
		...
		if (PS4SessionInfo.IsValid())
		{
			if (PS4SessionInfo->SessionType == EPS4Session::RoomSession && PS4SessionInfo->RoomId != SCE_NP_MATCHING2_INVALID_ROOM_ID)
			{
				...
				FOnlineSessionPS4Ptr OnlineSessionPS4Ptr = StaticCastSharedPtr<FOnlineSessionPS4>(PS4Subsystem->GetSessionInterface());
...
bool FOnlineSessionPS4::DoSearchRooms(const TSharedRef<FOnlineSessionSearch>& SearchSettings, const FUniqueNetIdPS4& SearchingPlayerId)
{
	...
	{
		// We don't have a valid world
		UE_LOG_ONLINE_SESSION(Error, TEXT("FOnlineSessionPS4::DoCreateJoinRoom: Matching2Context: 0x%x in bad state: %i."), SearchingPlayerContextId, (int)GetUserMatching2ContextState(SearchingPlayerContextId));
		return false;
	}
...
bool FOnlineSessionPS4::DoLeaveRoom(FNamedOnlineSession& Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	if (AsyncTaskManager)
	{
		...
		FOnlineAsyncTaskPS4LeaveRoom* const NewTask = new FOnlineAsyncTaskPS4LeaveRoom(PS4Subsystem, &Session, CompletionDelegate, RequestId);
...
bool FOnlineSessionPS4::DoLeaveSession(FNamedOnlineSession* Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	(new FAutoDeleteSessionLeaveTask(PS4Subsystem, PS4Subsystem->GetUserWebApiContext(*PlayerId), Session, CompletionDelegate))->StartBackgroundTask();
...
void FOnlineSessionPS4::OnSessionInviteAccepted(const FString& LocalUserIdStr, const FString& SessionId)
{
	...
	if (PlayerId->IsValid())
	{
		if (PS4Subsystem->AreRoomsEnabled())
		{
			(new AutoDeleteSessionGetChangeableSessionDataTask(PS4Subsystem, PS4Subsystem->GetUserWebApiContext(*PlayerId), PlayerId, SessionId, FOnlineAsyncTaskPS4SessionGetChangeableSessionData::FOnSessionGetChangeableSessionDataComplete::CreateRaw(this, &FOnlineSessionPS4::HandleGetChangeableDataCompleteSessionInviteRoomsEnabled)))->StartBackgroundTask();
		}
		else
		{
			FOnlineAsyncTaskManagerPS4 * AsyncTaskManager = PS4Subsystem->GetAsyncTaskManager();
			if (AsyncTaskManager != nullptr)
			{
				...
				FOnlineAsyncTaskPS4SessionGetChangeableSessionData::FOnSessionGetChangeableSessionDataComplete Delegate = FOnlineAsyncTaskPS4SessionGetChangeableSessionData::FOnSessionGetChangeableSessionDataComplete::CreateRaw(this, &FOnlineSessionPS4::HandleGetChangeableDataCompleteSessionInviteRoomsDisabled);
...
bool FOnlineSessionPS4::IsChatDisabled(const FUniqueNetIdPS4& User)
{
	...
	if (ret < 0)
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("Failed to create a request in IsChatEnabled() with error code [%x]"), ret);
	}
	else
	{
		...
		if (ret < 0)
		{
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Checking Parental Controls failed with error code [%x]"), ret);
		}
		else
		{
...
bool FOnlineSessionPS4::DoLeaveOrDestroySession(FNamedOnlineSession& Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	if (Session.bHosting && !bUsingHostMigration)
	{
		...
		(new FAutoDeleteSessionDestroyTask(PS4Subsystem, PS4Subsystem->GetUserWebApiContext(PlayerIdPS4), &Session, CompletionDelegate))->StartBackgroundTask();
	}
	else
	{
		...
		(new FAutoDeleteSessionLeaveTask(PS4Subsystem, PS4Subsystem->GetUserWebApiContext(PlayerIdPS4), &Session, CompletionDelegate))->StartBackgroundTask();
	}
