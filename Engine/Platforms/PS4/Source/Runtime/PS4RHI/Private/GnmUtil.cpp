...
void VerifyRenderTargets(FGnmCommandListContext& GnmCommandContext, FGnmBoundShaderState* BoundShaderState)
{
	if( GDoRenderTargetVerification == true && BoundShaderState->PixelShader )
	{
		...
		for( uint32 RenderTargetIndex = 0; RenderTargetIndex < MaxSimultaneousRenderTargets; RenderTargetIndex++, ShaderOutputMask <<= 4 )
		{
			...
			FRHITexture* RT = GnmCommandContext.GetCurrentRenderTargetTexture(RenderTargetIndex);
			if (RT == NULL)
			{
				continue;
			}
			...
			case sce::Gnm::kPsTargetOutputModeR32:
				{
					...
					bIsValid = IsValidRenderTargetFormat( RenderTargetFormat, ValidRenderTargetFormats, UE_ARRAY_COUNT( ValidRenderTargetFormats ));
			...
			checkf(bIsValid, TEXT("Incompatible surface format [%s] with PS output [%s] and TargetChannelType %i, detected for render target slot %d.\nThe shader must indicate the correct output format in ModifyCompilationEnvironment with OutEnvironment.SetRenderTargetOutputFormat\n"), *GetRenderTargetFormatString(RenderTargetDataFormat.getRenderTargetFormat()), *GetTargetOutputModeString(TargetOutputMode), TargetChannelType, RenderTargetIndex);
...
const FGnmAuxBuffer& GetSocialScreenAuxBuffer()
{
	return GGnmManager.GetSocialScreenAuxBuffer();
}
