...
IMPLEMENT_MATERIAL_SHADER_TYPE(template<>,TDistortionMeshFastGS<FDistortMeshAccumulatePolicy>, TEXT("/Engine/Private/DistortAccumulateVS.usf"), TEXT("VRProjectFastGS"), SF_Geometry);
...
*/
template<class DistortMeshPolicy>
FBoundShaderStateInput TDistortionMeshDrawingPolicy<DistortMeshPolicy>::GetBoundShaderStateInput(ERHIFeatureLevel::Type InFeatureLevel, bool bMultiRes /* = false */) const
{
	...
	)
{
	...
	if(bDistorted && !bBackFace)
	{
		...
		CommitGraphicsPipelineState(RHICmdList, DrawingPolicy, DrawRenderStateLocal, DrawingPolicy.GetBoundShaderStateInput(View.GetFeatureLevel(), View.bVRProjectEnabled));
