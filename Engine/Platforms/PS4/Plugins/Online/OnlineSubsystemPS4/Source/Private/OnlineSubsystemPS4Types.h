...
using FUniqueNetIdPS4Ref = TSharedRef<const class FUniqueNetIdPS4>;
...
public:
	...
	{
		//static const TSharedRef<const FUniqueNetId> EmptyId = MakeShared<const FUniqueNetIdPS4>(SCE_USER_SERVICE_USER_ID_INVALID, nullptr);
		static const TSharedRef<const FUniqueNetId> EmptyId = MakeShareable(new FUniqueNetIdPS4(SCE_USER_SERVICE_USER_ID_INVALID, nullptr));
		return EmptyId;
	}
	...
	static TSharedRef<FUniqueNetIdPS4> FindOrCreate(const NpToolkit::Core::OnlineUser& OnlineUser);
	...
	inline TSharedRef<const FUniqueNetIdPS4> AsShared() const { return Cast(FUniqueNetId::AsShared()); }
	...
	virtual inline bool Compare(FUniqueNetId const& Other) const override final
	{
		...
		return GetType() == Other.GetType() &&
			reinterpret_cast<uintptr_t>(this) == reinterpret_cast<uintptr_t>(&Other);
	}
};
