...
public class PS4Platform : Platform
{
	...
	private static int GetChunkIndexForFile(String LocalDirectory, String Filename, Dictionary<FileReference, int> CustomFileChunkMapping)
	{
		...
		if(CustomFileChunkMapping.ContainsKey(localFilePath))
		{
	...
	private static void RecurseStagingDirectory(string LocalDirectory, string PS4Directory, int Depth, List<ChunkLanguageEntry> ChunkLanguageEntries, Dictionary<FileReference, int> CustomFileChunkMapping, PerTitlePackageParameters TitleParams, StringBuilder FilesContents, UnrealTargetConfiguration TargetConfiguration, bool bPlayGoEmulation, bool bCompressPakFiles, string TargetExecutable, bool bGeneratingPatch, bool bGeneratingRemaster, out int NumChunks, ref List<string> DirNameList)
	{
		NumChunks = 1;
		...
		foreach (string FullFile in Files)
		{
			...
			int FileChunk = GetChunkIndexForFile(LocalDirectory, File, CustomFileChunkMapping);
		...
		foreach (string FullDir in Dirs)
		{
			...
			int RecurseChunks;
			RecurseStagingDirectory(LocalDirectory + Dir + "\\", PS4Directory + Dir + "/", Depth + 1, ChunkLanguageEntries, CustomFileChunkMapping, TitleParams, FilesContents, TargetConfiguration, bPlayGoEmulation, bCompressPakFiles, TargetExecutable, bGeneratingPatch, bGeneratingRemaster, out RecurseChunks, ref DirNameList);
	...
	private static Dictionary<FileReference, int> GetCustomChunkMapping(ProjectParams Params, DeploymentContext SC, string LocalDirectory)
	{
		...
		foreach (var chunkMapping in CustomChunkMapping)
		{
			...
			{
				FileChunkMapping.Add(new FileReference(FullPathPattern), chunkMapping.Value);
			}
		}
	...
	private static void BuildChunkContents(StringBuilder ChunkContents, List<ChunkLanguageEntry> ChunkLanguageEntries, int NumChunks, bool bPlaygoEmulation, ProjectParams Params, DeploymentContext SC, bool bForceDualLayer, out String[] OutChunkLayers)
	{
	...
	public override void Package(ProjectParams Params, DeploymentContext SC, int WorkingCL)
	{
		...
		Parallel.ForEach(TitleParameters, (TitleParams) =>
		{
			...
			String StagedTitleOverrideDirectory = Path.Combine(Params.BaseStageDirectory, "PS4", TitleParams.TitleID.ToLowerInvariant());

			// read the title id and passcode from the title.json file in the specified titleid directory
			JsonObject TitleObj = null;
			if (JsonObject.TryRead(new FileReference(Path.Combine(StagedTitleOverrideDirectory, "title.json")), out TitleObj))
			{
			...
			if (Directory.Exists(StagedTitleOverrideDirectory))
			{
			...
			Parallel.For(0, SC.StageTargetConfigurations.Count, TargetConfigurationIdx =>
			{
				...
				Dictionary<FileReference, int> CustomFileChunkMapping = GetCustomChunkMapping(Params, SC, LocalDirectory);
				List<string> DirNameList = new List<string>();
				RecurseStagingDirectory(LocalDirectory, "", 0, ChunkLanguageEntries, CustomFileChunkMapping, TitleParams, FilesContents, TargetConfiguration, bPlayGoEmulation, true, TargetExecutable, Params.GeneratePatch, Params.GenerateRemaster, out NumChunks, ref DirNameList);
				...
				String[] ChunkLayers;
				BuildChunkContents(ChunkContents, ChunkLanguageEntries, NumChunks, bPlayGoEmulation, Params, SC, bForceDualLayer, out ChunkLayers);
	...
	public override void PostStagingFileCopy(ProjectParams Params, DeploymentContext SC)
	{
		...
		//PlayGo handling
		{
			...
			if (SC.PlatformUsesChunkManifests && !Params.Distribution)
			{
				...
				Dictionary<FileReference, int> CustomFileChunkMapping = GetCustomChunkMapping(Params, SC, LocalDirectory);
				List<string> DirNameList = new List<string>();
				RecurseStagingDirectory(LocalDirectory, "", 0, ChunkLanguageEntries, CustomFileChunkMapping, PackageParameters, FilesContents, SC.StageTargetConfigurations[0], bPlayGoEmulation, false, TargetExecutable, Params.GeneratePatch, Params.GenerateRemaster, out NumChunks, ref DirNameList);

				StringBuilder ChunkContents = new StringBuilder();
				String[] ChunkLayers;
				BuildChunkContents(ChunkContents, ChunkLanguageEntries, NumChunks, bPlayGoEmulation, Params, SC, false, out ChunkLayers);
