...
int32 FPS4TargetDevice::GetProcessSnapshot( TArray<FTargetDeviceProcessInfo>& OutProcessInfos ) 
{
	...
	FProcHandle RunningProc = FPlatformProcess::CreateProc( *( FPaths::EngineDir() / TEXT( "Binaries/DotNET/PS4/PS4DevKitUtil.exe" )), *Params, true, false, false, NULL, 0, NULL, WritePipe );
...
bool FPS4TargetDevice::Launch( const FString& AppId, EBuildConfiguration BuildConfiguration, EBuildTargetType TargetType, const FString& Params, uint32* OutProcessId )
{
...
bool FPS4TargetDevice::Run( const FString& ExecutablePath, const FString& Params, uint32* OutProcessId )
{
	...
	RunningProc = FPlatformProcess::CreateProc( *( FPaths::EngineDir() / TEXT( "Binaries/DotNET/PS4/PS4DevKitUtil.exe" ) ), *CommandLine, true, false, false, NULL, 0, NULL, WritePipe );
...
void FPS4TargetDevice::ExecuteConsoleCommand( const FString& ExecCommand ) const
{
	void* WritePipe;
	void* ReadPipe;
	FPlatformProcess::CreatePipe( ReadPipe, WritePipe );

	FString CommandLine = (TEXT("sendinput target=")) + CachedHostName + TEXT(" Args=") + ExecCommand + TEXT(" ArgsEnd");
	FProcHandle RunningProc = FPlatformProcess::CreateProc( *( FPaths::EngineDir() / TEXT( "Binaries/DotNET/PS4/PS4DevKitUtil.exe" ) ), *CommandLine, true, false, false, NULL, 0, NULL, WritePipe );
	FPlatformProcess::WaitForProc( RunningProc );
	...
	{
		FString ProcessOutput = FPlatformProcess::ReadPipe( ReadPipe );
		UE_LOG( LogPS4TargetDevice, Warning, TEXT("Problem sending console command:\r\n%s"), *ProcessOutput );
	}
...
ITargetDeviceOutputPtr FPS4TargetDevice::CreateDeviceOutputRouter(FOutputDevice* Output) const
{
	FPS4TargetDeviceOutputPtr DeviceOutputPtr = MakeShareable(new FPS4TargetDeviceOutput());
